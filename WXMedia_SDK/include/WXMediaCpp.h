...
//static inline int16_t S16Level(int16_t a, int Level) {
//	int sum = a * Level / 100;
...
//}
//
//static int   GetS16Max(int16_t* buf, int count) {
//	int iMax = 0;
//	for (int i = 0; i < count; i++) {
//		if (abs(buf[i]) > iMax)
//			iMax = abs(buf[i]);
...
//static int   GetF32Max(float* buf, int count) {
//	float fMax = 0.0;
//	for (int i = 0; i < count; i++) {
//		if (fabs(buf[i]) > fMax)
//			fMax = fabs(buf[i]);
//	}
//	return (int)(fMax * 32767);
//}
...
//
//static void GetXY(int srcWidth, int srcHeight, int dstWidth, int dstHeight, int& desX, int& desY) {
//	desX = 0;
//	desY = 0;
...
//static AVInputFormat* GetInputFotmat(WXCTSTR wszFileName) {
//	AVInputFormat* inFmt = nullptr;
//	WXString strExt = WXBase::GetFileNameSuffix(wszFileName);
//	if (wcsicmp(strExt.str(), L"webm") == 0) {
//		inFmt = av_find_input_format("webm");
//	}
//	else if (wcsicmp(strExt.str(), L"wxm") == 0) {
//		inFmt = av_find_input_format("wxm");
//
//
//	}
//	else if (wcsicmp(strExt.str(), L"ts") == 0) {
