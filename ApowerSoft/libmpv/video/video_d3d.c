...
#pragma comment(lib, "dxva2.lib")  // 对于某些 Direct3D 功能可能需要
...
static void d3d9_free_av_device_ref(AVHWDeviceContext *ctx)
{
    ...
    if (hwctx->devmgr) {
        ...
        hwctx->devmgr->lpVtbl->Release(hwctx->devmgr);
		hwctx->devmgr = NULL;
    }
...
AVBufferRef *d3d9_wrap_device_ref(IDirect3DDevice9 *device)
{
   ...
   // IDirect3DDeviceManager9_ResetDevice(hwctx->devmgr, device, reset_token);
	hwctx->devmgr->lpVtbl->ResetDevice(hwctx->devmgr, device, reset_token);
