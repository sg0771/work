...
static void update_playback_state(struct vo_w32_state *w32)
{
    ...
    if (!pstate->playing || !pstate->taskbar_progress) {
        w32->taskbar_list3->lpVtbl->SetProgressState(w32->taskbar_list3, w32->window,
                                       TBPF_NOPROGRESS);
        return;
    }

    w32->taskbar_list3->lpVtbl->SetProgressValue(w32->taskbar_list3, w32->window,
...
static void update_window_state(struct vo_w32_state *w32)
{
    ...
    if (w32->taskbar_list) {
        w32->taskbar_list->lpVtbl->MarkFullscreenWindow(w32->taskbar_list,
...
static void *gui_thread(void *ptr)
{
    ...
    if (SUCCEEDED(OleInitialize(NULL))) {
        ...
        {
            if (FAILED(w32->taskbar_list->lpVtbl->HrInit(w32->taskbar_list))) {
                w32->taskbar_list->lpVtbl->Release(w32->taskbar_list);
        ...
        {
            if (FAILED(w32->taskbar_list3->lpVtbl->HrInit(w32->taskbar_list3))) {
                w32->taskbar_list3->lpVtbl->Release(w32->taskbar_list3);
