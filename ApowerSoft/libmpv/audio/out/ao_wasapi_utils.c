...
{
    struct wasapi_state *state = ao->priv;
    HRESULT hr = state->pAudioClient->lpVtbl->IsFormatSupported(state->pAudioClient,
...
static bool find_formats_shared(struct ao *ao, WAVEFORMATEXTENSIBLE *wformat)
{
    ...
    hr = state->pAudioClient->lpVtbl->GetMixFormat(state->pAudioClient, &mix_format);
    EXIT_ON_ERROR(hr);
    ...
    hr = state->pAudioClient->lpVtbl->IsFormatSupported(state->pAudioClient,
...
static HRESULT init_clock(struct wasapi_state *state) {
    HRESULT hr = state->pAudioClient->lpVtbl->GetService(state->pAudioClient,
                                         &IID_IAudioClock,
                                         (void **)&state->pAudioClock);
    EXIT_ON_ERROR(hr);
    hr = state->pAudioClock->lpVtbl->GetFrequency(state->pAudioClock, &state->clock_frequency);
    EXIT_ON_ERROR(hr);
...
static void init_session_display(struct wasapi_state *state) {
    ...
    hr = state->pSessionControl->lpVtbl->SetIconPath(state->pSessionControl, path, NULL);
    if (FAILED(hr)) {
    ...
    hr = state->pSessionControl->lpVtbl->SetDisplayName(state->pSessionControl,
...
static void init_volume_control(struct wasapi_state *state)
{
    ...
    if (state->share_mode == AUDCLNT_SHAREMODE_EXCLUSIVE) {
        ...
        hr = state->pDevice->lpVtbl->Activate(state->pDevice,
        ...
        hr = state->pEndpointVolume->lpVtbl->QueryHardwareSupport(state->pEndpointVolume,
                                                       &state->vol_hw_support);
        EXIT_ON_ERROR(hr);
    } else {
        MP_DBG(state, "IAudioClient::Initialize pAudioVolume\n");
        hr = state->pAudioClient->lpVtbl->GetService(state->pAudioClient,
...
static HRESULT fix_format(struct ao *ao, bool align_hack)
{
    ...
    HRESULT hr = state->pAudioClient->lpVtbl->GetDevicePeriod(state->pAudioClient,&devicePeriod,
                                              NULL);
    ...
    hr = state->pAudioClient->lpVtbl->Initialize(state->pAudioClient,
                                 state->share_mode,
    ...
    hr = state->pAudioClient->lpVtbl->SetEventHandle(state->pAudioClient, state->hWake);
    EXIT_ON_ERROR(hr);

    MP_DBG(state, "IAudioClient::Initialize IAudioClient_GetBufferSize\n");
    hr = state->pAudioClient->lpVtbl->GetBufferSize(state->pAudioClient,
                                    &state->bufferFrameCount);
    EXIT_ON_ERROR(hr);
...
static char* get_device_name(struct mp_log *l, void *talloc_ctx, IMMDevice *pDevice)
{
    ...
    hr = pProps->lpVtbl->GetValue(pProps, &mp_PKEY_Device_FriendlyName,
                                 &devname);
    EXIT_ON_ERROR(hr);
...
{
    LPWSTR deviceID;
    HRESULT hr = pDevice->lpVtbl->GetId(pDevice, &deviceID);
    if (FAILED(hr)) {
...
static struct enumerator *create_enumerator(struct mp_log *log)
{
    ...
    hr = e->pEnumerator->lpVtbl->EnumAudioEndpoints(
        e->pEnumerator, eRender, DEVICE_STATE_ACTIVE, &e->pDevices);
    EXIT_ON_ERROR(hr);

    hr = e->pDevices->lpVtbl->GetCount(e->pDevices, &e->count);
    EXIT_ON_ERROR(hr);
...
static struct device_desc *device_desc_for_num(struct enumerator *e, UINT i)
{
    ...
    HRESULT hr = e->pDevices->lpVtbl->Item(e->pDevices, i, &pDevice);
    if (FAILED(hr)) {
...
{
    IMMDevice *pDevice = NULL;
    HRESULT hr = e->pEnumerator->lpVtbl->GetDefaultAudioEndpoint(
                           ...
                           IMMDevice **ppDevice, LPWSTR deviceID)
{
    ...
    hr = pEnumerator->lpVtbl->GetDevice(pEnumerator, deviceID, ppDevice);
    EXIT_ON_ERROR(hr);
...
retry:
    if (state->deviceID) {
        ...
        hr = state->pDevice->lpVtbl->Activate(state->pDevice, &IID_IAudioClient,
                                         CLSCTX_ALL, NULL,
        ...
        hr = res->lpVtbl->QueryInterface(res, &IID_IAudioClient,
                                     (void **)&state->pAudioClient);
...
void wasapi_thread_uninit(struct ao *ao)
{
    ...
    if (state->pAudioClient)
        state->pAudioClient->lpVtbl->Stop(state->pAudioClient);
