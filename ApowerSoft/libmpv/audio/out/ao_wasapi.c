...
#include <initguid.h>
 ...
 // 如果标准头文件中没有定义这些GUID，手动定义它们

 // IMMDeviceEnumerator 相关
#ifndef IID_IMMDeviceEnumerator
DEFINE_GUID(IID_IMMDeviceEnumerator,
    0xA95664D2, 0x9614, 0x4F35, 0xA7, 0x46, 0xDE, 0x8D, 0xB6, 0x36, 0x17, 0xE6);
#endif

#ifndef IID_IMMNotificationClient
...
#ifndef IID_IAudioSessionControl
...
static HRESULT get_device_delay(struct wasapi_state *state, double *delay_us) {
    ...
    hr = state->pAudioClock->lpVtbl->GetPosition(state->pAudioClock, &position, &qpc_position);
    EXIT_ON_ERROR(hr);
...
static bool thread_feed(struct ao *ao)
{
    ...
    UINT32 padding;
    hr = state->pAudioClient->lpVtbl->GetCurrentPadding(state->pAudioClient, &padding);
    EXIT_ON_ERROR(hr);
    bool refill = false;
    ...
    BYTE *pData;
    hr = state->pRenderClient->lpVtbl->GetBuffer(state->pRenderClient,
                                      frame_count, &pData);
    EXIT_ON_ERROR(hr);
    ...
    hr = state->pRenderClient->lpVtbl->ReleaseBuffer(state->pRenderClient,
                                          frame_count, 0);
    EXIT_ON_ERROR(hr);
...
static void thread_reset(struct ao *ao)
{
    ...
    hr = state->pAudioClient->lpVtbl->Stop(state->pAudioClient);
    if (FAILED(hr))
        MP_ERR(state, "IAudioClient_Stop returned: %s\n", mp_HRESULT_to_str(hr));

    hr = state->pAudioClient->lpVtbl->Reset(state->pAudioClient);
    if (FAILED(hr))
...
static void thread_resume(struct ao *ao)
{
    ...
    HRESULT hr = state->pAudioClient->lpVtbl->Start(state->pAudioClient);
    if (FAILED(hr)) {
...
static DWORD __stdcall AudioThread(void *lpParameter)
{
    ...
    while (true) {
        ...
        intptr_t thread_state = atomic_load(&state->thread_state);
        switch (thread_state) {
...
static int thread_control_exclusive(struct ao *ao, enum aocontrol cmd, void *arg)
{
    ...
    switch (cmd) {
    case AOCONTROL_GET_VOLUME:
        state->pEndpointVolume->lpVtbl->GetMasterVolumeLevelScalar(
    ...
    case AOCONTROL_SET_VOLUME:
        ...
        state->pEndpointVolume->lpVtbl->SetMasterVolumeLevelScalar(
            state->pEndpointVolume, volume, NULL);
        return CONTROL_OK;
    case AOCONTROL_GET_MUTE:
        state->pEndpointVolume->lpVtbl->GetMute(state->pEndpointVolume, &mute);
        *(bool *)arg = mute;
        return CONTROL_OK;
    case AOCONTROL_SET_MUTE:
        mute = *(bool *)arg;
        state->pEndpointVolume->lpVtbl->SetMute(state->pEndpointVolume, mute, NULL);
        return CONTROL_OK;
    }
    return CONTROL_UNKNOWN;
}
...
static int thread_control_shared(struct ao *ao, enum aocontrol cmd, void *arg)
{
    ...
    switch(cmd) {
    case AOCONTROL_GET_VOLUME:
        state->pAudioVolume->lpVtbl->GetMasterVolume(state->pAudioVolume, &volume);
        *(ao_control_vol_t *)arg = (ao_control_vol_t){
            .left  = 100.0f * volume,
            .right = 100.0f * volume,
        };
    ...
    case AOCONTROL_SET_VOLUME:
        volume = ((ao_control_vol_t *)arg)->left / 100.f;
        state->pAudioVolume->lpVtbl->SetMasterVolume(state->pAudioVolume, volume, NULL);
        return CONTROL_OK;
    case AOCONTROL_GET_MUTE:
        state->pAudioVolume->lpVtbl->GetMute(state->pAudioVolume, &mute);
        *(bool *)arg = mute;
        return CONTROL_OK;
    case AOCONTROL_SET_MUTE:
        mute = *(bool *)arg;
        state->pAudioVolume->lpVtbl->SetMute(state->pAudioVolume, mute, NULL);
        return CONTROL_OK;
    }
    return CONTROL_UNKNOWN;
}
...
static int thread_control(struct ao *ao, enum aocontrol cmd, void *arg)
{
    ...
    case AOCONTROL_UPDATE_STREAM_TITLE:
        ...
        do {
            state->pSessionControl->lpVtbl->SetDisplayName(state->pSessionControl, title, NULL);

            SAFE_DESTROY(tmp, CoTaskMemFree(tmp));
            state->pSessionControl->lpVtbl->GetDisplayName(state->pSessionControl, &tmp);
