                           ...
                           char *name)
{
    ...
    struct ao *aoobj = talloc_ptrtype(NULL, aoobj);
    talloc_steal(aoobj, log);
    *aoobj = (struct ao) {
        .driver = desc.p,
        .probing = probing,
        .global = global,
        ...
        .log = mp_log_new(aoobj, log, name),
        .def_buffer = opts->audio_buffer,
        .client_name = talloc_strdup(aoobj, opts->audio_client_name),
    };
    talloc_free(opts);
    aoobj->priv = m_config_group_from_desc(aoobj, aoobj->log, global, &desc, name);
    if (!aoobj->priv)
        goto error;
    ao_set_gain(aoobj, 1.0f);
    return aoobj;
error:
    talloc_free(aoobj);
    return NULL;
