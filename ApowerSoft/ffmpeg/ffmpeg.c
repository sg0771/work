...
static char* UTF16ToGB(const wchar_t* strUTF16) {
    int len = WideCharToMultiByte(CP_ACP, 0, strUTF16, -1, NULL, 0, NULL, NULL);
    if (len <= 0)
        return "";
    char* str= (char* )malloc(len);
    WideCharToMultiByte(CP_ACP, 0, strUTF16, -1, &str[0], len, NULL, NULL);
    return str;
}
...
int wmain(int argc, wchar_t **argvw)
{
    ...
    wprintf(L"当前程序路径：%ws", exe_path);
    // 方法2：使用 Windows API 辅助函数（PathRemoveFileSpec）
    wchar_t dirPath[MAX_PATH];
    wcscpy(dirPath, exe_path);  // 复制原路径
    PathRemoveFileSpecW(dirPath);  // 移除文件名部分
    wprintf(L"libffmpeg 当前程序路径：%ws", dirPath);
