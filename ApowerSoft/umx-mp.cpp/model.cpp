...
bool umxcpp::load_umx_model(const std::wstring &model_file,
                            struct umx_model *model)
{
    model->load_progress += 0.1f;
    FILE *f = _wfopen(model_file.c_str(), L"rb");
    if (!f)
    {
        return false;
    }
    ...
    // load weights
    {
        ...
        for (;;)
        {
            ...
            //std::cout << "Loading tensor " << name << " with shape [" << ne[0]<< ", " << ne[1] << "]" << std::endl;
    ...
    //fprintf(stdout, "Loaded model (%u tensors, %6.2f MB)\n", n_loaded, total_size / 1024.0 / 1024.0);
