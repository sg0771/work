...
{
  return (unsigned long)AThread::GetCurrentThreadId();
}
...
void DllLibCurlGlobal::Unload()
{
  ...
  if(g_curlReferences == 1)
    g_curlTimeout = SystemClockMillis();
#endif
}
...
void DllLibCurlGlobal::CheckIdle()
{
  ...
  {
    if( !it->m_busy && (SystemClockMillis() - it->m_idletimestamp) > idletime )
    {
  ...
  if(g_curlReferences == 1 && SystemClockMillis() - g_curlTimeout > idletime)
    Unload();
#endif
}
...
void DllLibCurlGlobal::easy_release(CURL_HANDLE** easy_handle, CURLM** multi_handle)
{
  ...
  for(it = m_sessions.begin(); it != m_sessions.end(); ++it)
  {
    if( it->m_easy == easy && (multi == NULL || it->m_multi == multi) )
    {
      ...
      it->m_busy = false;
      it->m_idletimestamp = SystemClockMillis();
      return;
    }
  }
}
