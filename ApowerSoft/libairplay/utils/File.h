...
public:
  ...
  size_t LoadFile(const CURL &file, auto_buffer& outputBuffer);
   ...
   */
  size_t Read(void* bufPtr, size_t bufSize);
   ...
   */
  size_t Write(const void* bufPtr, size_t bufSize);
  void Flush();
  ...
  size_t LoadFile(const std::string &filename, auto_buffer& outputBuffer);
