...
public:
  ...
  {
    const unsigned ts = SystemClockMillis();
  ...
  void Pause()
  {
    m_pause = SystemClockMillis();
  }

  void Resume()
  {
    m_stamp += SystemClockMillis() - m_pause;
    m_pause  = 0;
  }
...
//    else
   {
	   size_t front = 20971520;//g_advancedSettings.m_cacheMemBufferSize;///////////////////////////////////////////////////////////////////////////////////////////////
	   size_t back = std::max<size_t>(20971520/*g_advancedSettings.m_cacheMemBufferSize*/ / 4, 1024 * 1024);/////////////////////////////////////////////////////////////
//      if (useDoubleCache)
//      {
//        front = front / 2;
//        back = back / 2;
...
{
	//WriteLogNew("filecache::process begin");
  if (!m_pCache)
  {
	 // WriteErrorLogNew("filecache::process no cache strategy");
  ...
  if (buffer.get() == NULL)
  {
	  //WriteErrorLogNew("filecache::process failed to allocate read buffer");
  ...
  while (!m_bStop)
  {
    ...
    while (m_writeRate)
    {
		if (m_writePos - m_readPos < m_writeRate * 4.0f/*g_advancedSettings.m_readBufferFactor*/)
      {
        limiter.Reset(m_writePos);
        break;
      }

		if (limiter.Rate(m_writePos) < m_writeRate * 4.0f/*g_advancedSettings.m_readBufferFactor*/)
        break;
    ...
    if (iRead == 0)
    {
	  ...
	  {
		  //WriteLogNew("filecache::process exit process");
		  break;
	  }
    }
    else if (iRead < 0)
  ...
  //WriteErrorLogNew("filecache::process exit");
}
...
size_t CFileCache::Read(void* lpBuf, size_t uiBufSize)
{
  CSingleLock lock(m_sync);
  if (!m_pCache)
  {
