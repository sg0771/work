...
void CCurlFile::SetCommonOptions(CReadState* state)
{
  ...
  std::string strTempPath = "C:\\Users\\apple\\AppData\\Roaming\\Kodi\\cache";
  strCookieFile = "C:\\Users\\apple\\AppData\\Roaming\\Kodi\\cache\\cookies.dat";
  ...
  if (m_userAgent.length() > 0)
    ...
    curl_easy_setopt(h, CURLOPT_USERAGENT, ""/*g_advancedSettings.m_userAgent.c_str()*/);
  ...
  if (m_connecttimeout == 0)
	  m_connecttimeout = 10; //g_advancedSettings.m_curlconnecttimeout;
  ...
  if (m_lowspeedtime == 0)
	  m_lowspeedtime = 20;// g_advancedSettings.m_curllowspeedtime;
...
size_t write_data4(void *ptr, size_t size, size_t nmemb, FILE *stream)
{
...
bool CCurlFile::Download(const std::string& strURL, const std::string& strFileName, LPDWORD pdwSize)
{
	...
	CURLcode res;
	{
		...
		//curl_easy_setopt(m_state->m_easyHandle, CURLOPT_TIMEOUT, 10/*g_advancedSettings.m_curlconnecttimeout*/);
			...
			|| result == CURLE_RECV_ERROR /* some silly shoutcast servers */)
		{
			...
			curl_easy_setopt(m_state->m_easyHandle, CURLOPT_XFERINFOFUNCTION, transfer_abort_callback);
#else
			curl_easy_setopt(m_state->m_easyHandle, CURLOPT_PROGRESSFUNCTION, transfer_abort_callback);
#endif
			curl_easy_setopt(m_state->m_easyHandle, CURLOPT_NOPROGRESS, 0);
		...
		res = curl_easy_setopt(m_state->m_easyHandle, CURLOPT_WRITEFUNCTION, write_data4);
		if (res != CURLE_OK)
		{
			fclose(fp);
			g_curlInterface.easy_release(&m_state->m_easyHandle, NULL);
		...
		/*  根据curl_easy_setopt()设置的传输选项，实现回调函数以完成用户特定任务  */
		res = curl_easy_setopt(m_state->m_easyHandle, CURLOPT_WRITEDATA, fp);
		if (res != CURLE_OK)
		{
			fclose(fp);
			g_curlInterface.easy_release(&m_state->m_easyHandle, NULL);
		...
		//
		curl_easy_setopt(m_state->m_easyHandle, CURLOPT_NOSIGNAL, 1L);
		curl_easy_setopt(m_state->m_easyHandle, CURLOPT_LOW_SPEED_LIMIT, 1L);
		curl_easy_setopt(m_state->m_easyHandle, CURLOPT_LOW_SPEED_TIME, 5L);
		curl_easy_setopt(m_state->m_easyHandle, CURLOPT_HEADER, 0L);
		curl_easy_setopt(m_state->m_easyHandle, CURLOPT_NOBODY, 0L);
		curl_easy_setopt(m_state->m_easyHandle, CURLOPT_FOLLOWLOCATION, 1L);

		res = curl_easy_perform(m_state->m_easyHandle);                               // 调用curl_easy_perform()函数完成传输任务
		fclose(fp);
...
size_t CCurlFile::Write(const void* lpBuf, size_t uiBufSize)
{
  if (!(m_opened && m_forWrite) || m_inError)
    return -1;
  ...
  return (size_t)m_state->m_filePos;
}
...
bool CCurlFile::Exists(const CURL& url)
{
  ...
  {
    long code;
    curl_easy_getinfo(m_state->m_easyHandle, CURLINFO_RESPONSE_CODE, &code);
  }
  ...
  while ((unsigned int)m_buffer.getMaxReadSize() < want && m_buffer.getMaxWriteSize() > 0 )
  {
    ...
    if (!m_stillRunning)
    {
      if (result == CURLM_OK)
      {
        ...
        if (++retry > 2/*g_advancedSettings.m_curlretries*/)
        {
