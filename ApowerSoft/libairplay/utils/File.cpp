...
size_t CFile::Read(void *lpBuf, size_t uiBufSize)
{
  if (!m_pFile)
    return -1;
  ...
  if(m_pBuffer)
  {
    ...
    {
      const size_t nBytes = m_pBuffer->sgetn(
    ...
    {
      const size_t nBytes = m_pBuffer->sgetn((char*)lpBuf, uiBufSize);
      if (m_bitStreamStats && nBytes>0)
        m_bitStreamStats->AddSampleBytes(nBytes);
      return nBytes;
    }
  ...
  try
  {
    if(m_flags & READ_TRUNCATED)
    {
      const size_t nBytes = m_pFile->Read(lpBuf, uiBufSize);
      if (m_bitStreamStats && nBytes>0)
        m_bitStreamStats->AddSampleBytes(nBytes);
      return nBytes;
    }
    else
    {
      ...
      {
        const size_t curr = m_pFile->Read((char*)lpBuf+done, uiBufSize-done);
        if (curr <= 0)
        {
          if (curr < 0 && done == 0)
            return -1;
...
size_t CFile::Write(const void* lpBuf, size_t uiBufSize)
{
  if (!m_pFile)
    return -1;
  if (lpBuf == NULL && uiBufSize != 0)
...
size_t CFile::LoadFile(const std::string &filename, auto_buffer& outputBuffer)
{
  const CURL pathToUrl(filename);
  return LoadFile(pathToUrl, outputBuffer);
}

size_t CFile::LoadFile(const CURL& file, auto_buffer& outputBuffer)
{
  ...
  */
  ...
  while (true)
  {
    ...
    size_t read = Read(outputBuffer.get() + total_read, outputBuffer.size() - total_read);
    if (read < 0)
    {
      outputBuffer.clear();
      return -1;
...
CFileStreamBuffer::int_type CFileStreamBuffer::underflow()
{
  ...
  size_t size = m_file->Read(m_buffer+backsize, m_frontsize);
