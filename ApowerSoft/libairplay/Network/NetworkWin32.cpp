...
std::string CNetworkInterfaceWin32::GenerateMacAddress() {
	static int isFirst = 1;
	static std::string strTime = "00:00:00:00:00:00";
	if (isFirst != 1) {
		return strTime;
	}
	...
	streamTime << sTime;
	strTime = streamTime.str();

	int remainder = 12 - strTime.length();
	...
	if (remainder >= 0) {
		...
		strTime = strTime.substr(strTime.length() - 12, strTime.length());
	}

	for (size_t i = 2; i < strTime.length(); i = i + 3)
	{
		strTime.insert(i, ":");
	}
...
void CNetworkWin32::queryInterfaceList()
{
  ...
  if ((GetAdaptersInfo(adapterInfo, &ulOutBufLen)) == NO_ERROR)
  {
	...
	const int nSize = 256;
	bool bOpen = false;
	LONG err = regKey.Open(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Control\\Class\\{4D36E972-E325-11CE-BFC1-08002bE10318}", KEY_READ);
	if (err == 0)
	{
		bOpen = true;
	}
    ...
    while (adapter)
    {
		bool bRealNetCard = false;
		//check if or not virtual network card
		std::string adname = adapter->AdapterName;
		...
		if (bOpen)
		{
			...
			for (DWORD i = 0; regKey.EnumKey(i, subKeyName, &dwLength) != ERROR_NO_MORE_ITEMS; dwLength = MAX_PATH, i++)
			{
				CRegKey	regKey;
				std::wstring subKey = L"SYSTEM\\CurrentControlSet\\Control\\Class\\{4D36E972-E325-11CE-BFC1-08002bE10318}\\";
				subKey += subKeyName;
				if (regKey.Open(HKEY_LOCAL_MACHINE, subKey.c_str(), KEY_READ) == 0)
				{
					char szStrValue[nSize] = { 0 };
					ULONG unSize = nSize;
					DWORD dwCharacter = 0;
					if (regKey.QueryStringValue(L"NetCfgInstanceId", (LPTSTR)szStrValue, &unSize) == 0 &&
                        stricmp(adname.c_str(), szStrValue) == 0)
					{
						if (regKey.QueryDWORDValue(L"Characteristics", dwCharacter) == 0 && (dwCharacter & 4) > 0)
						{
							bRealNetCard = true;
							break;
						}
