...
static HANDLE init_event(const wchar_t *name, DWORD pid)
{
	...
	if (!handle)
		wxlog("Failed to get event '%s': %lu", name, GetLastError());
	return handle;
}
...
static HANDLE init_mutex(const wchar_t *name, DWORD pid)
{
	...
	if (!handle)
		wxlog("Failed to open mutex '%s': %lu", name, GetLastError());
	return handle;
}
...
static inline bool init_mutexes(void)
{
	...
	if (!tex_mutexes[0]) {
		OutputDebugStringA("WXMedia init_mutex1 Error");
		return false;
	}

	tex_mutexes[1] = init_mutex(MUTEX_TEXTURE2, pid);
	if (!tex_mutexes[1]) {
		OutputDebugStringA("WXMedia init_mutex2 Error");
		return false;
	}
...
static inline bool init_system_path(void)
{
	...
	if (!ret) {
		wxlog("Failed to get windows system path: %lu", GetLastError());
		return false;
	}
...
static inline void log_current_process(void)
{
	...
	if (len > 0) {
		process_name[len] = 0;
		wxlog("Hooked to process: %s", process_name);
	}

	wxlog("(half life scientist) everything..  seems to be in order");
}
...
static inline bool init_hook_info(void)
{
	...
	if (!filemap_hook_info) {
		wxlog("Failed to create hook info file mapping: %lu",
				GetLastError());
		return false;
	}

	global_hook_info = MapViewOfFile(filemap_hook_info, FILE_MAP_ALL_ACCESS,
			0, 0, sizeof(struct hook_info) + MAX_CAMERA_WIDTH * MAX_CAMERA_HEIGHT * 4);
	if (!global_hook_info) {
		wxlog("Failed to map the hook info file mapping: %lu",
				GetLastError());
		return false;
	}
...
static DWORD WINAPI dummy_window_thread(LPVOID *unused)
{
	...
	if (!RegisterClass(&wc)) {
		wxlog("Failed to create temp D3D window class: %lu",
				GetLastError());
		return 0;
	}
	...
	if (!dummy_window) {
		wxlog("Failed to create temp D3D window: %lu", GetLastError());
		return 0;
	}
...
static inline void init_dummy_window_thread(void)
{
	HANDLE thread = CreateThread(NULL, 0, dummy_window_thread, NULL, 0,
			NULL);
	if (!thread) {
		wxlog("Failed to create temp D3D window thread: %lu",
				GetLastError());
		return;
	}
...
static inline bool attempt_hook(void)
{
	...
	if (!d3d9_hooked) {
		if (!d3d9_hookable()) {
			wxlog("no D3D9 hook address found!\n");
			d3d9_hooked = true;
		} else {
	...
	if (!dxgi_hooked) {
		if (!dxgi_hookable()) {
			wxlog("no DXGI hook address found!\n");
			dxgi_hooked = true;
		} else {
	...
	if (sizeof(void*) == 4 &&  !d3d8_hooked) {
		if (!d3d8_hookable()) {
			wxlog("no D3D8 hook address found!\n");
			d3d8_hooked = true;
		} else {
...
void hlog_hr(const char *text, HRESULT hr)
{
	...
	if (buffer) {
		wxlog("%s (0x%08lX): %s", text, hr, buffer);
		LocalFree(buffer);
	} else {
		wxlog("%s (0x%08lX)", text, hr);
	}
}
...
static inline bool init_shared_info(size_t size)
{
	...
	if (!shmem_file_handle) {
		wxlog("init_shared_info: Failed to create shared memory: %d",
				GetLastError());
		return false;
	}
	...
	shmem_info = MapViewOfFile(shmem_file_handle, FILE_MAP_ALL_ACCESS,
			0, 0, size);
	if (!shmem_info) {
		wxlog("init_shared_info: Failed to map shared memory: %d",
				GetLastError());
		return false;
	}
...
static inline bool init_shmem_thread(uint32_t pitch, uint32_t cy)
{
	...
	if (!thread_data.copy_event) {
		wxlog("init_shmem_thread: Failed to create copy event: %d",
				GetLastError());
		return false;
	}
	...
	if (!thread_data.stop_event) {
		wxlog("init_shmem_thread: Failed to create stop event: %d",
				GetLastError());
		return false;
	}
	...
	if (!thread_data.copy_thread) {
		wxlog("init_shmem_thread: Failed to create thread: %d",
				GetLastError());
		return false;
	}
	return true;
...
//共享内存
bool capture_init_shmem(struct shmem_data **data, HWND window, uint32_t cx, uint32_t cy,
		uint32_t pitch, uint32_t format, bool flip)
{
	...
	uint32_t  total_size     = aligned_header + aligned_tex * 2 + 32 + MAX_CAMERA_WIDTH * MAX_CAMERA_HEIGHT * 4;
	uintptr_t align_pos;

	if (!init_shared_info(total_size)) {
		wxlog("capture_init_shmem: Failed to initialize memory");
		return false;
	}
	...
	*data = shmem_info;
	...
	if (!SetEvent(signal_ready)) {
		wxlog("capture_init_shmem: Failed to signal ready: %d",
				GetLastError());
		return false;
	}
...
TCHAR g_szFileName[MAX_PATH] = { 0 };//EXE名字
BOOL WINAPI DllMain(HINSTANCE hinst, DWORD reason, LPVOID unused1)
{
	TCHAR szPathName[MAX_PATH] = { 0 };
	GetModuleFileName(NULL, szPathName, MAX_PATH);//获取DLL名字

	//TCHAR szFileName[MAX_PATH] = { 0 };//EXE名字
	memset(g_szFileName,0,MAX_PATH * sizeof(TCHAR));
	_tsplitpath_s(szPathName,
		NULL, 0,
		NULL, 0,
		g_szFileName, _MAX_FNAME,
		NULL, 0);

	if (_wcsicmp(g_szFileName, L"helper32") == 0 ||
		_wcsicmp(g_szFileName, L"helper64") == 0)
		return TRUE;

	//wxlogW(L"Exe is %ws", szPathName);
	...
	if (reason == DLL_PROCESS_ATTACH) {
		...
		if (!success) {
			wxlog("Failed to get current thread handle");
		}
		...
		if (!init_system_path()) {
			wxlog("init_system_path");
			return false;
		}
		if (!init_hook_info()) {
			wxlog("init_hook_info");
			return false;
		}
		if (!init_mutexes()) {
			wxlog("init_mutexes");
		...
		GetModuleFileNameW(hinst, name, MAX_PATH);//获取DLL名字
		LoadLibraryW(name);
		...
		WPARAM wparam, LPARAM lparam)
{
	...
	if (hooking && msg->message == (WM_USER + 432)) {
		...
		if (unhook_windows_hook_ex)
			unhook_windows_hook_ex((HHOOK)msg->lParam);//unhook 地址

		hooking = false;

		//wxlog("Un Hook !!! int dummy_debug_proc!!");
	}
