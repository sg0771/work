...
static bool setup_dxgi(IDXGISwapChain *swap)
{
	...
	if (SUCCEEDED(hr)) {
		...
		if (level >= D3D_FEATURE_LEVEL_11_0) {
			...
			global_hook_info->m_iType = (int)HOOK_TYPE_D3D11;
			wxlog("D3D11_1 hook");
			return true;
		}
	}
	...
	if (SUCCEEDED(hr)) {
		...
		device->Release();
		global_hook_info->m_iType = (int)HOOK_TYPE_D3D10;
		wxlog("D3D10 hook");
		return true;
	}
	...
	if (SUCCEEDED(hr)) {
		...
		device->Release();
		global_hook_info->m_iType = (int)HOOK_TYPE_D3D12;
		return true;
	}
#endif
...
bool hook_dxgi(void)
{
	...
	if (!compile) {
		wxlog("hook_dxgi: failed to find d3d compiler library");
		return true;
	}
