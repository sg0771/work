...
static bool d3d12_init_11on12(void)
{
	...
	if (!initialized_11 && !d3d11) {
		...
		if (!d3d11) {
			wxlog("d3d12_init_11on12: failed to load d3d11");
		}
		initialized_11 = true;
	}
	...
	if (!initialized_func && !create_11_on_12) {
		...
		if (!create_11_on_12) {
			wxlog("d3d12_init_11on12: Failed to get "
...
static bool d3d12_shtex_init(HWND window, bb_info &bb)
{
	...
	wxlog("d3d12 shared texture capture successful");
	return true;
}
		...
		bb_info &bb)
{
	...
	if (SUCCEEDED(hr)) {
		data.dxgi_1_4 = true;
		wxlog("We're DXGI1.4 boys!");
		swap3->Release();
	}

	wxlog("Buffer count: %d, swap effect: %d", (int)desc.BufferCount,
			(int)desc.SwapEffect);
	...
	if (bb.count > MAX_BACKBUFFERS) {
		wxlog("Somehow it's using more than the max backbuffers.  "
