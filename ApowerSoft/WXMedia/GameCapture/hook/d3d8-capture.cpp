...
struct d3d8_data {
	uint32_t                       cx = 0;
	uint32_t                       cy = 0;
	D3DFORMAT                      d3d8_format = D3DFMT_A8R8G8B8;
	DXGI_FORMAT                    dxgi_format = DXGI_FORMAT_B8G8R8A8_UNORM;
	struct shmem_data              *shmem_info = NULL;
	HWND                           window = NULL;
	uint32_t                       pitch = 0;
	IDirect3DSurface8              *copy_surfaces[NUM_BUFFERS] = { NULL };
	bool                           surface_locked[NUM_BUFFERS] = { false};
	int                            cur_surface = 0;
	CComPtr<IDirect3DSurface8>     m_pD3dSurface = NULL;//Ô¤ÀÀ´°¿Ú
...
static DXGI_FORMAT d3d8_to_dxgi_format(D3DFORMAT format)
{
	switch ((unsigned long)format) {
		...
		case D3DFMT_R5G6B5:   return DXGI_FORMAT_B5G6R5_UNORM;  //RGB565
...
static const char* d3d8_to_string(D3DFORMAT format)
{
	switch ((unsigned long)format) {
	case D3DFMT_X1R5G5B5: return "r5g5b5x1";
	case D3DFMT_A1R5G5B5: return "r5g5b5a1";
	case D3DFMT_R5G6B5:   return "r5g6b5";  //RGB565
...
static bool d3d8_get_window_handle(IDirect3DDevice8 *device)
{
	...
	HRESULT hr  = device->GetCreationParameters(&parameters);
	if (FAILED(hr)) {
...
static bool d3d8_init_format_backbuffer(IDirect3DDevice8 *device)
{
	...
	if (data.dxgi_format == DXGI_FORMAT_B5G6R5_UNORM) {
		data.m_iType = WX_RGB565;
	}else if (data.dxgi_format == D3DFMT_X1R5G5B5 || data.dxgi_format == D3DFMT_A1R5G5B5) {
		data.m_iType = WX_RGB555;
	}
...
static bool d3d8_shmem_init(IDirect3DDevice8 *device)
{
	...
	if (!capture_init_shmem(&data.shmem_info, data.window, 
	...
	//BackBuffer
	HRESULT hr = device->CreateImageSurface(
		data.cx, data.cy,
		data.d3d8_format,
		&data.m_pD3dSurface);

	wxlog("d3d8 memory capture successful  Size=%dx%d Pitch=%d Format=%s ",data.cx,data.cy, data.pitch, d3d8_to_string(data.d3d8_format));
	return true;
}
	...
	next_surface = (cur_surface == NUM_BUFFERS - 1) ? 0 : cur_surface + 1;

	 {
		if (shmem_texture_data_lock(next_surface)) {
			data.copy_surfaces[cur_surface]->UnlockRect();
			data.surface_locked[next_surface] = false;
			shmem_texture_data_unlock(next_surface);
		}
		hr = device->CopyRects(backbuffer, nullptr, 0, data.copy_surfaces[cur_surface], nullptr);
		if (SUCCEEDED(hr)) {
			IDirect3DSurface8 *target = data.copy_surfaces[cur_surface];
			D3DLOCKED_RECT rect;
			HRESULT hr;
			if (!data.surface_locked[cur_surface]) {
				hr = target->LockRect(&rect, nullptr, D3DLOCK_READONLY);
				if (SUCCEEDED(hr)) {

					WXCountFps();

					if (HookCheckCamera(data.m_iType)) {
						uint8_t *bufSrc = (uint8_t*)rect.pBits;
						int PitchSrc = rect.Pitch;
						HookDrawCamera(bufSrc, data.cx, data.cy, PitchSrc, 0, data.m_iType);
					}
					shmem_copy_data(cur_surface, rect.pBits);
				}
			}
		}
		...
		if (!global_hook_info->m_bCapture) {
			device->BeginScene();
			hr = device->CopyRects(backbuffer, nullptr, 0, data.m_pD3dSurface, nullptr);
			if (SUCCEEDED(hr)) {
				D3DLOCKED_RECT source;
				hr = data.m_pD3dSurface->LockRect(&source, 0, D3DLOCK_NOSYSLOCK);
				if (SUCCEEDED(hr)) {
					uint8_t *bufSrc = (uint8_t*)source.pBits;
					int PitchSrc = source.Pitch;
					HookDrawImage(bufSrc, data.cx, data.cy, PitchSrc, 0, data.m_iType);
					HookDrawString(bufSrc, data.cx, data.cy, PitchSrc, 0, data.m_iType);
					HookDrawCamera(bufSrc, data.cx, data.cy, PitchSrc, 0, data.m_iType);
					data.m_pD3dSurface->UnlockRect();
					hr = device->CopyRects(data.m_pD3dSurface, nullptr, 0, backbuffer, nullptr);
				}
			}
			device->EndScene();
		}
...
static void d3d8_capture(IDirect3DDevice8 *device,IDirect3DSurface8 *backbuffer){
	global_hook_info->m_iType = (int)HOOK_TYPE_D3D8;
		...
		void **present_addr)
{
	...
	wxlog("D3D8 value invalid, manually obtaining");

	create = (d3d8create_t)GetProcAddress(d3d8_module, "Direct3DCreate8");
	if (!create) {
		wxlog("Failed to load Direct3DCreate8");
		return false;
	}

	d3d8 = create(D3D_SDK_VERSION);
	if (!d3d8) {
		wxlog("Failed to create D3D8 context");
		return false;
	}
	...
	if (SUCCEEDED(hr)) {
		...
		*present_addr = (void*)vtable[15];
		...
		wxlog("Failed to create D3D8 device");
		return false;
	}
...
bool hook_d3d8(void)
{
	...
	if (global_hook_info->offsets.d3d8.present < d3d8_size) {
		...
		if (!manually_get_d3d8_present_addr(d3d8_module,
					&present_addr)) {
			wxlog("Failed to get D3D8 value");
			return true;
		}
	}

	if (!present_addr) {
		wxlog("Invalid D3D8 value");
		return true;
	}
