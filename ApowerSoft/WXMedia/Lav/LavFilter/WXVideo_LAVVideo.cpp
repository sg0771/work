...
#include "LAVVideo.h"
#include "WXVideo_Media.h"
#include <dvdmedia.h>
#include <algorithm>
#include "WXVideo_VideoInputPin.h"
#include "WXVideo_VideoOutputPin.h"
#include <algorithm>
#include "moreuuids.h"
#include "DSUtilLite_registry.h"
#include "common_defines.h"

#include <Shlwapi.h>

#include "WXVideo_VC1HeaderParser.h"
#include "WXVideo_MPEG2HeaderParser.h"
...
{
  *phr = S_OK;
  m_pInput = new CVideoInputPin((TCHAR*)TEXT("CVideoInputPin"), this, phr,(LPWSTR) L"Input");
...
{
  DSAutoLock cObjectLock(m_pLock);
...
HRESULT CLAVVideo::DecideBufferSize(IMemAllocator* pAllocator, ALLOCATOR_PROPERTIES* pProperties)
{
  ...
  pProperties->cBuffers = std::min(std::max(pProperties->cBuffers, (long)2) + decoderBuffs, decoderBuffersMax);
...
static const LPWSTR stream_ar_blacklist[] = {
  (LPWSTR)L".mkv", (LPWSTR)L".webm",
  (LPWSTR)L".mp4", (LPWSTR)L".mov", (LPWSTR)L".m4v",
};
...
HRESULT CLAVVideo::CreateDecoder(const CMediaType *pmt)
{
  ...
  AVCodecID codec = FindVideoCodecId(pmt);
  ...
  for(int i = 0; i < Codec_VideoNB; ++i) {
    const video_codec_config_t *config = get_codec_config((LAVVideoCodec)i);
    bool bMatched = false;
...
HRESULT CLAVVideo::EndOfStream()
{
  ...
  DSAutoLock DSAutoLock(&m_csReceive);
...
HRESULT CLAVVideo::CompleteConnect(PIN_DIRECTION dir, IPin *pReceivePin)
{
  ...
  if (dir == PINDIR_OUTPUT) {
    ...
    if (m_pInput->CurrentMediaType().subtype == MEDIASUBTYPE_MPEG2_VIDEO && !m_pSubtitleInput && (m_dwDecodeFlags & LAV_VIDEO_DEC_FLAG_DVD)) {
      m_pSubtitleInput = new CLAVVideoSubtitleInputPin((TCHAR * )TEXT("CLAVVideoSubtitleInputPin"), this, &m_csFilter, &hr, (LPWSTR)L"Subtitle Input");
      ASSERT(SUCCEEDED(hr));
