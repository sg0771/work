...
#include "moreuuids.h"
#include <algorithm>
#include "WXSplitter_PacketAllocator.h"
...
HRESULT CLAVOutputPin::DecideBufferSize(IMemAllocator* pAlloc, ALLOCATOR_PROPERTIES* pProperties)
{
  ...
  pProperties->cBuffers = std::max(pProperties->cBuffers, (long)(m_bPacketAllocator ? 20 : m_nBuffers));
...
HRESULT CLAVOutputPin::DeliverPacket(Packet *pPacket)
{
  ...
  if(pPacket->pmt) {
   ...
   fTimeValid = pPacket->rtStart != Packet::INVALID_TIME;
