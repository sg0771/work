...
AVCodecID FindAudioCodecId(const CMediaType *mt)
{
...
HRESULT CLAVAudio::ParseRealAudioHeader(const BYTE *extra, const size_t extralen)
{
  ...
  if (version == 3) {
    ...
    size_t ra_extralen = std::min((size_t)((extra + extralen) - (fmt+4)), (size_t)AV_RB32(fmt));
    if (ra_extralen > 0)  {
...
void CLAVAudio::UpdateVolumeStats(const BufferDetails &buffer)
{
  ...
  float * const fChAvg = (float *)av_malloc(buffer.wChannels * sizeof(float));
...
static audio_codec_config_t m_codec_config[] = {
...
const audio_codec_config_t *get_codec_config(LAVAudioCodec codec)
{
	audio_codec_config_t *config = &m_codec_config[codec];
