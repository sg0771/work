...
#include "dxva_SurfaceAllocator.h"
#include "moreuuids.h"
...
HRESULT VerifyD3D9Device(DWORD &dwIndex, DWORD dwDeviceId)
{
    ...
    IDirect3D9 *pD3D = LibInst::GetInst().mDirect3DCreate9(D3D_SDK_VERSION);
    if (!pD3D)
        return E_FAIL;
...
HRESULT CDecDXVA2::CreateD3DDeviceManager(IDirect3DDevice9 *pDevice, UINT *pReset, IDirect3DDeviceManager9 **ppManager)
{
    ...
    HRESULT hr =   LibInst::GetInst().mDXVA2CreateDirect3DDeviceManager9(&resetToken, &pD3DManager);
    if (FAILED(hr))
    {
...
HRESULT CDecDXVA2::InitD3D(UINT lAdapter)
{
    ...
    m_pD3D = LibInst::GetInst().mDirect3DCreate9(D3D_SDK_VERSION);
    if (!m_pD3D)
    {
...
HRESULT CDecDXVA2::InitD3DEx(UINT lAdapter)
{
    ...
    hr = LibInst::GetInst().mDirect3DCreate9Ex(D3D_SDK_VERSION, &pD3D9Ex);
    if (FAILED(hr))
    {
...
HRESULT CDecDXVA2::CheckHWCompatConditions(GUID decoderGuid)
{
    ...
    int max_ref_frames_dpb41 = std::min(11, 32768 / (width_mbs * height_mbs));
...
HRESULT CDecDXVA2::SetD3DDeviceManager(IDirect3DDeviceManager9 *pDevManager)
{
    ...
    if (m_pAVCtx)
    {
        ...
        UINT numSurfaces = std::max(config.ConfigMinRenderTargetBuffCount, (USHORT)1);
...
STDMETHODIMP CDecDXVA2::Flush()
{
    ...
    if (m_dwVendorId == VEND_ID_ATI && m_nCodecId == AV_CODEC_ID_H264 && m_pDecoder)
    {
        if (m_bNative && m_pDXVA2Allocator)
        {
           ...
           DSAutoLock allocatorLock(m_pDXVA2Allocator);
...
__forceinline bool CDecDXVA2::CopyFrame(LAVFrame *pFrame)
{
    ...
    if (pFrame->format != LAVPixFmt_P016) { //NV12 copy //Add By Tam
        for (int i = 0; i < pFrame->height; i++){
            memcpy(pFrame->data[0] + i * pFrame->stride[i], src[0] + i * src_pitch[0], pFrame->width);
        }
        for (int i = 0; i < pFrame->height / 2; i++) {
            memcpy(pFrame->data[1] + i * pFrame->stride[i], src[0] + i * src_pitch[1], pFrame->width);
        }
    }
