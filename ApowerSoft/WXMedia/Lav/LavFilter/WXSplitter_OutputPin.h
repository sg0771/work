...
#include "WXSplitter_StreamParser.h"
...
public:
  ...
  void SetNewMediaTypes(std::deque<CMediaType> pmts) { DSAutoLock lock(&m_csMT); m_mts = pmts; SetQueueSizes(); }
  void SendMediaType(CMediaType *mt) { DSAutoLock lock(&m_csMT); m_newMT = mt;}
  void SetStreamMediaType(CMediaType *mt) { DSAutoLock lock(&m_csMT); m_StreamMT = *mt; }
