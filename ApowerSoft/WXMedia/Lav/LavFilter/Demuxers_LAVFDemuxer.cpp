...
#include "Demuxers_LAVFUtils.h"
#include "Demuxers_LAVFStreamInfo.h"
#include "common_defines.h"
#include "Demuxers_LAVFVideoHelper.h"
#include "Demuxers_ExtradataParser.h"
#include "DSUtilLite.h"
#include "WXSplitter.h"
...
#include "DSUtilLite_CueSheet.h"

//#define AVFORMAT_OPEN_TIMEOUT 20  //打开网络流最大超时20秒
#define AVFORMAT_OPEN_TIMEOUT 10  //打开网络流最大超时10秒 
...
{
  DSAutoLock lock(m_pLock);
  AVDictionary* options = nullptr;
  HRESULT hr = S_OK;

  int bNetStream = FALSE;
  ...
  if (_strnicmp("http", fileName, 4) == 0) {
      bNetStream = TRUE;
  }
...
trynoformat:
  ...
  if (format) {
    ...
    if (wcsicmp(extension, L".webm") == 0) {
        inputFormat = av_find_input_format("webm");
    }
  ...
  //网络流webm处理
  if (bNetStream && strstr(fileName, "webm")) {
	  inputFormat = av_find_input_format("webm");
  }
  ret = avformat_open_input(&m_avFormat, fileName, inputFormat, &options);
  av_dict_free(&options);

  if (!bNetStream) { //本地文件打开失败时，尝试指定其它格式打开
      if (ret < 0) { //可能手动指定的格式不支持，按默认格式打开
          ret = avformat_open_input(&m_avFormat, fileName, NULL, NULL);
      }
      if (ret < 0) { //部分webm文件可能后缀不是webm，可能回打开失败
          inputFormat = av_find_input_format("webm");
  ...
  if (ret < 0) {
      ...
      m_strError = errbuf;
     // WXLogA("avformat_open_input [%s] error[%s]", fileName, errbuf);
    if (format) {
      format = nullptr;
...
STDMETHODIMP CLAVFDemuxer::InitAVFormat(LPCOLESTR pszFileName, BOOL bForce)
{
  ...
  if (m_avFormat->flags & AVFMT_FLAG_NETWORK || (m_avFormat->flags & AVFMT_FLAG_CUSTOM_IO && !m_avFormat->pb->seekable)) {
      ...
      av_opt_set_int(m_avFormat, "analyzeduration", std::max(m_pSettings->GetNetworkStreamAnalysisDuration() * 1000, (DWORD)200000), 0);
  }
  else {
  ...
  m_timeOpening = time(nullptr);
  int ret = avformat_find_stream_info(m_avFormat, nullptr); //URL堵塞， 可能比较慢
  if (ret < 0) {
  ...
  for (unsigned int idx = 0; idx < m_avFormat->nb_streams; ++idx) {
      ...
      if (m_avFormat->streams[idx]->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
          if (!(m_avFormat->streams[idx]->disposition & AV_DISPOSITION_ATTACHED_PIC))
          {
              //ffmpeg video stream
              AVDictionaryEntry* m = NULL;
              while ((m = av_dict_get(m_avFormat->streams[idx]->metadata, "rotate", m, AV_DICT_IGNORE_SUFFIX)) != NULL) {
    ...
    if (st->need_parsing == AVSTREAM_PARSE_FULL) {
      if (st->codecpar->codec_id == AV_CODEC_ID_DVB_SUBTITLE) { // DVB subtitles are not supported by the parser
