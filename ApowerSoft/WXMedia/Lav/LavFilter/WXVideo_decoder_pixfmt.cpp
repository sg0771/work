...
static void free_buffers(struct LAVFrame *pFrame)
{
  ...
  av_free(pFrame->data[1]);
  av_free(pFrame->data[2]);
  av_free(pFrame->data[3]);
  memset(pFrame->data, 0, sizeof(pFrame->data));

  av_free(pFrame->stereo[0]);
  av_free(pFrame->stereo[1]);
  av_free(pFrame->stereo[2]);
  av_free(pFrame->stereo[3]);
...
HRESULT AllocLAVFrameBuffers(LAVFrame *pFrame, ptrdiff_t stride)
{
  ...
  for (int plane = 0; plane < desc.planes; plane++) {
    ...
    pFrame->data[plane]   = (BYTE *)av_malloc(size + AV_INPUT_BUFFER_PADDING_SIZE);
  ...
  if (pFrame->flags & LAV_FRAME_FLAG_MVC) {
    for (int plane = 0; plane < desc.planes; plane++) {
      ...
      pFrame->stereo[plane] = (BYTE *)av_malloc(size + AV_INPUT_BUFFER_PADDING_SIZE);
