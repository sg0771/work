...
int precompute_coeffs(int inSize, int outSize, filter_t* filterp, int **boundsp, double **kkp)
{
	...
	kk = (double*)av_malloc(outSize * kmax * sizeof(double));
	if (!kk) {
		return 0;
	}

	// malloc check ok, kmax*sizeof(double) > 2*sizeof(int)
	bounds = (int*)av_malloc(outSize * 2 * sizeof(int));
	if (!bounds) {
		av_free(kk);
		return 0;
	}
...
int normalize_coeffs_8bpc(int outSize, int kmax, double *prekk, INT32 **kkp)
{
	...
	kk = (INT32*)av_malloc(outSize * kmax * sizeof(INT32));
	if (!kk) {
		return 0;
	}
...
HRESULT CResampleRGB32::Init()
{
	...
	if (m_bResampleHor && m_bResampleVer) {
		m_pTemp = (BYTE*)av_malloc(m_destW * m_srcH * 4);
		if (!m_pTemp) {
			FreeData();
