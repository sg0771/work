	...
	, m_cBuffers(cBuffers)
{
	...
	m_pInput = new CBaseVideoInputPin(L"CBaseVideoInputPin", this, phr, L"Video");
	if (!m_pInput) {
		*phr = E_OUTOFMEMORY;
	}
	if (FAILED(*phr)) {
	...
	m_pOutput = new CBaseVideoOutputPin(L"CBaseVideoOutputPin", this, phr, L"Output");
	if (!m_pOutput) {
		*phr = E_OUTOFMEMORY;
	}
	if (FAILED(*phr))  {
...
void CBaseVideoFilter::SetAspect(SIZE aspect)
{
...
HRESULT CBaseVideoFilter::ReconnectOutput(int width, int height, bool bForce/* = false*/, REFERENCE_TIME AvgTimePerFrame/* = 0*/, DXVA2_ExtendedFormat* dxvaExtFormat/* = nullptr*/)
{
	...
	if (bNeedReconnect) {
		...
		if (m_nDecoderMode != DM_MODE_SOFTWARE) {
			m_pOutput->SetMediaType(&mt);
		...
		if (m_nDecoderMode != DM_MODE_DXVA2) {
...
HRESULT CBaseVideoFilter::SetMediaType(PIN_DIRECTION dir, const CMediaType* pmt)
{
	if (dir == PINDIR_INPUT) {
		...
		m_nDecoderMode = DM_MODE_SOFTWARE;
	}
