...
CTextDimsKey::CTextDimsKey(const CTextDimsKey& textDimsKey)
	...
	, m_style(new STSStyle(*textDimsKey.m_style))
{
}
...
void CTextDimsKey::UpdateHash()
{
	...
	m_hash += m_hash << 5;
	m_hash += CStringElementTraits<CString>::Hash(m_style->fontNameW.c_str());
	m_hash += m_hash << 5;
...
bool CTextDimsKey::operator==(const CTextDimsKey& textDimsKey) const
{
	return m_str == textDimsKey.m_str
		   ...
		   && m_style->fontNameW == textDimsKey.m_style->fontNameW
