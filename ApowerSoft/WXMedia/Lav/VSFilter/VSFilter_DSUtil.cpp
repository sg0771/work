...
IBaseFilter* VS_FindFilter(LPCWSTR clsid, IFilterGraph* pFG)
{
	CLSID clsid2;
	CLSIDFromString(CComBSTR(clsid), &clsid2);
	return VS_FindFilter(clsid2, pFG);
}

IBaseFilter* VS_FindFilter(const CLSID& clsid, IFilterGraph* pFG)
{
...
IBaseFilter* VS_GetFilterFromPin(IPin* pPin)
{
	if (pPin) {
		CPinInfo pi;
...
{
	return GetCLSID(VS_GetFilterFromPin(pPin));
}
...
std::wstring VS_CStringFromGUID(const GUID& guid)
{
	WCHAR buff[40] = {};
	if (StringFromGUID2(guid, buff, 39) <= 0) {
		StringFromGUID2(GUID_NULL, buff, 39);
	}
	return CStringW(buff).GetString();
}
...
std::wstring ISO6391ToLanguage(LPCSTR code)
{
	CHAR tmp[2+1];
...
std::wstring ISO6392ToLanguage(LPCSTR code)
{
	CHAR tmp[3+1];
...
std::wstring ISO639XToLanguage(LPCSTR code, bool bCheckForFullLangName /*= false*/)
{
	CString lang;

	switch (size_t nLen = strlen(code)) {
		case 2:
			lang = ISO6391ToLanguage(code).c_str();
			break;
		case 3:
			lang = ISO6392ToLanguage(code).c_str();
...
LCID VS_ISO6391ToLcid(LPCSTR code)
{
	CHAR tmp[3+1];
	if (strncpy_s(tmp, code, 3) == 0 && tmp[0]) {
		_strlwr_s(tmp);
...
LCID VS_ISO6392ToLcid(LPCSTR code)
{
	CHAR tmp[3+1];
	if (strncpy_s(tmp, code, 3) == 0 && tmp[0]) {
		_strlwr_s(tmp);
...
std::wstring ISO6391To6392(LPCSTR code)
{
	CHAR tmp[2+1];
	if (strncpy_s(tmp, code, 2) == 0 && tmp[0]) {
		_strlwr_s(tmp);
		for (const auto& isolang : s_isolangs) {
			if (!strcmp(isolang.iso6391, tmp)) {
				return CString(isolang.iso6392).GetString();
			}
		}
	}
	return L"";
...
std::wstring ISO6392To6391(LPCSTR code)
{
	CHAR tmp[3+1];
	if (strncpy_s(tmp, code, 3) == 0 && tmp[0]) {
		_strlwr_s(tmp);
		for (const auto& isolang : s_isolangs) {
			if (!strcmp(isolang.iso6392, tmp)) {
				return CString(isolang.iso6391).GetString();
			}
		}
	}
	return L"";
...
unsigned int vs_lav_xiphlacing(unsigned char *s, unsigned int v)
{
	unsigned int n = 0;
...
std::string VobSubDefHeader(int w, int h, std::string palette)
{
	CStringA hdr;
	hdr.Format(
