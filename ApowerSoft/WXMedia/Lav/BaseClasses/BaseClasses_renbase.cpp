...
{
    DSAutoLock cObjectCreationLock(&m_ObjectCreationLock);
    if (m_pPosition) {
...
{
    DSAutoLock cRendererLock(&m_InterfaceLock);
...
{
    DSAutoLock cRendererLock(&m_RendererLock);
...
{
    DSAutoLock cInterfaceLock(&m_InterfaceLock);
    m_bInReceive = TRUE;
    ...
    DSAutoLock cSampleLock(&m_RendererLock);
...
HRESULT CBaseRenderer::NotifyEndOfStream()
{
    ...
    return NotifyEvent(EC_COMPLETE,S_OK,(LONG_PTR)(IBaseFilter *)this);  //�����Ž���
}
...
{
    DSAutoLock cRendererLock(&m_pRenderer->m_InterfaceLock);
    DSAutoLock cSampleLock(&m_pRenderer->m_RendererLock);
    ...
    if (FAILED(hr)) {
...

        {
            ...
            if (!IsStopped() && !IsFlushing() && !m_pRenderer->m_bAbort && !m_bRunTimeError) {
                ...
                {
                    DSAutoLock alRendererLock(&m_pRenderer->m_RendererLock);
...
STDMETHODIMP CBaseVideoRenderer::get_FramesDroppedInRenderer(__out int *pcFramesDropped)
{
    ...
    DSAutoLock cVideoLock(&m_InterfaceLock);
