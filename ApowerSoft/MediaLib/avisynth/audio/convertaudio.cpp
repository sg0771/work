...
intptr_t __stdcall ConvertAudio::SetCacheHints(int cachehints,int frame_range)
...
void __stdcall ConvertAudio::GetAudio(void* buf, __int64 start, __int64 count, IScriptEnvironment* env) 
{
  ...
  if (tempbuffer_size<count) {
    if (tempbuffer_size) av_free(tempbuffer);
    tempbuffer = (char *) av_malloc((int)count*src_bps*channels);
  ...
  if (dst_format == SAMPLE_FLOAT)  // Skip final copy, if samples are to be float
    ...
    if (floatbuffer_size < count) {
      if (floatbuffer_size) av_free(floatbuffer);
      floatbuffer = (SFLOAT*)av_malloc((int)count*channels*sizeof(SFLOAT));
