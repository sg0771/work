...
{
private:
    std::vector<soundtouch::SoundTouch*> samplers;
...
: GenericVideoFilter(ConvertAudio::Create(_child, SAMPLE_FLOAT, SAMPLE_FLOAT), __FUNCTION__),
  tempo(_tempo/100.0f), rate(_rate/100.0f), pitch(_pitch/100.0f)
{
  ...
  {for (unsigned n = 0; n < last_nch; n++)
    samplers.push_back(new soundtouch::SoundTouch());
  }
...
static void setSettings(soundtouch::SoundTouch* sampler, const AVSValue* args, IScriptEnvironment* env)
{
...
{
private:
    soundtouch::SoundTouch* sampler = nullptr;
...
//  last_nch = vi.AudioChannels();
  ...
  sampler = new soundtouch::SoundTouch();
...
AVSValue __cdecl AVSsoundtouch::Create(AVSValue args, void*, IScriptEnvironment* env) {
  ...
  if (args[0].AsClip()->GetVideoInfo().AudioChannels() == 2) {
    return new AVSStereoSoundTouch(args[0].AsClip(), 
      args[1].AsFloat(100.0f), 
      args[2].AsFloat(100.0f), 
      args[3].AsFloat(100.0f), 
	  &args[4],
      env);
  }
...
extern const AVSFunction Soundtouch_filters[] = {
  { "TimeStretch", "c[tempo]f[rate]f[pitch]f[sequence]i[seekwindow]i[overlap]i[quickseek]b[aa]i", AVSsoundtouch::Create }, //ML_Timeline.cpp 325
  { 0 }
};
