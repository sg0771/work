...
AVSValue ExpExceptionTranslator::Evaluate(IScriptEnvironment* env) 
{
  ...
  try {
    ...
    return av;
	  ...
	  //Log_info("IScriptEnvironment::NotFound");
    throw;
  }
  catch (AvisynthError) {
	  //Log_info("AvisynthError");
    throw;
  }
  catch (...) {
...
AVSValue ExpEqual::Evaluate(IScriptEnvironment* env) 
{
  ...
  if (x.IsBool() && y.IsBool()) {
    ...
    return !lstrcmpiA(x.AsString(), y.AsString());
  }
  else {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        WXLogA("I don't know what \"%s\" means", name);
        return 0;
      }
    }
  }
