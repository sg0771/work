...
Cache::Cache(PClip _child, IScriptEnvironment* env)
 : GenericVideoFilter(_child,__FUNCTION__ ), nextCache(NULL), cache(0), priorCache(NULL), Tick(0)
{
...
//h_total_frames = 0; // Hint cache not init'ed
  ...
  if (child->GetVersion() <= 5) {
    ...
    const intptr_t vmode = child->SetCacheHints(CACHE_GETCHILD_CACHE_MODE, h_policy); // Cache ask Child for desired video cache mode
    switch (vmode) {
    ...
    const intptr_t amode = child->SetCacheHints(CACHE_GETCHILD_AUDIO_MODE, h_audiopolicy); // Cache ask Child for desired audio cache mode
    switch (amode) {
      case 0: // Unsupported by child
        break;
    ...
    const intptr_t cost = child->SetCacheHints(CACHE_GETCHILD_COST, 0); // Cache ask Child for estimated processing cost.
    switch (cost) {
      case 0: // Unsupported by child
        break;
    ...
    const intptr_t thread = child->SetCacheHints(CACHE_GETCHILD_THREAD_MODE, 0); // Cache ask Child for thread safetyness.
    switch (thread) {
      case 0: // Unsupported by child
        break;
    ...
    const intptr_t access = child->SetCacheHints(CACHE_GETCHILD_ACCESS_COST, 0); // Cache ask Child for preferred access pattern.
    switch (access) {
      case 0: // Unsupported by child
        break;
...
int Cache::PokeCache(int key, int data, IScriptEnvironment* env)
{
  switch (key) {
...
//          LeaveCriticalSection(&cs_cache_V);
    ...
    if (!found) { // Cache miss
      ...
      const int range = (std::max(n-minframe, maxframe-n) * 2 + 1) << CACHE_SCALE_SHIFT;
...
// But the current cache might have 99% of what was requested??
    ...
    if (ac_too_small_count > 2 && (maxsamplecount < vi.AudioSamplesFromBytes(8192*1024))) {  // Max size = 8MB!
      ...
      int new_size = (int)(vi.BytesFromAudioSamples(std::max(count, cache_start+cache_count-start))+8192) & -8192; // Yes +1 to +8192 bytes
...
//      EnterCriticalSection(&cs_cache_A);
      ...
      cache_count = std::min(count,(long long) maxsamplecount); // Remember maxsamplecount gets updated
...
//      LeaveCriticalSection(&cs_cache_A);
  ...
  if ((start < cache_start) || (start > cache_start+maxsamplecount)) { //first sample is before cache or beyond linear reach -> restart cache
    ...
    cache_count = std::min(count, (long long)maxsamplecount);
    cache_start = start;
...
intptr_t __stdcall Cache::SetCacheHints(int cachehints, int frame_range) {
      ...
      if (frame_range/samplesize > maxsamplecount) { // Only make bigger
        ...
        if (newcache) {
          ...
          if (cache) {
            ...
            cache_count = std::min(cache_count, (long long)maxsamplecount);
  ...
  //    LeaveCriticalSection(&cs_cache_A);
    ...
    case CACHE_FORCE_GENERIC:
    {
      ...
      int _cache_init = std::min((int)MAX_CACHED_VIDEO_FRAMES, frame_range);
    ...
    case CACHE_WINDOW:
      ...
      if (frame_range > h_span)  // Use the largest size when we have multiple clients
        h_span = std::min((int)MAX_CACHE_WINDOW, frame_range);
...
AVSValue __cdecl Cache::Create_Cache(AVSValue args, void*, IScriptEnvironment* env)
{
  ...
  if (p) {  
    ...
    if (q != (intptr_t)(void *)p) return new Cache(p, env);
  }
  return p;
}
