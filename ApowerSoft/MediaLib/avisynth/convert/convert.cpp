...
class rgba2yuv : public GenericVideoFilter
{
public:
    rgba2yuv(PClip src, IScriptEnvironment* env)
    ...
    PVideoFrame __stdcall GetFrame(int n, IScriptEnvironment* env)
    {
        ...
        if (child->GetVideoInfo().IsRGB24())
        {
            libyuv::RGB24ToI420(srcp, src->GetRowSize(), dstY, Ypitch, dstU, UVpitch, dstV, UVpitch, vi.width, -vi.height);

        }
        else
        {
            libyuv::ARGBToI420(srcp, src->GetRowSize(), dstY, Ypitch, dstU, UVpitch, dstV, UVpitch, vi.width, -vi.height);
        }
    ...
    static AVSValue __cdecl Create(AVSValue args, void*, IScriptEnvironment* env)
    {
        ...
        return new rgba2yuv(clip, env);
    }


};

class yuv2rgba : public GenericVideoFilter
{
public:
    yuv2rgba(PClip src, IScriptEnvironment* env)
        : GenericVideoFilter(src, __FUNCTION__)
    {
    ...
    PVideoFrame __stdcall GetFrame(int n, IScriptEnvironment* env)
    {
        ...
        if (child->GetVideoInfo().pixel_type == CS_YV16)
        {
            libyuv::I422ToARGB(srcY, Ypitch, srcU, UVpitch, srcV, UVpitch, dstp, vi.RowSize(), vi.width, -vi.height);
        }
        else if (child->GetVideoInfo().pixel_type == CS_YV24)

        {
            libyuv::I444ToARGB(srcY, Ypitch, srcU, UVpitch, srcV, UVpitch, dstp, vi.RowSize(), vi.width, -vi.height);

        }
        else
        {
            libyuv::I420ToARGB(srcY, Ypitch, srcU, UVpitch, srcV, UVpitch, dstp, vi.RowSize(), vi.width, -vi.height);
        }
...
extern const AVSFunction Convert_filters[] = {       // matrix can be "rec601", rec709", "PC.601" or "PC.709"
  ...
  { "ConvertToRGB32", "c[matrix]s[interlaced]b[ChromaInPlacement]s[chromaresample]s", ConvertToRGB::Create32 },//used
  { "ConvertToY8",    "c[matrix]s", ConvertToY8::Create },//My_overlay.cpp
  { "ConvertToYV12",  "c[interlaced]b[matrix]s[ChromaInPlacement]s[chromaresample]s[ChromaOutPlacement]s", ConvertToYV12::Create },//ML_Timeline.cpp  µ¼³ö±àÂë
  
  { "ConvertToYV24",  "c[interlaced]b[matrix]s[ChromaInPlacement]s[chromaresample]s", ConvertToPlanarGeneric::CreateYV24},//MY_overlay.cpp
...
int getMatrix( const char* matrix, IScriptEnvironment* env) {
  if (matrix) {
    if (!lstrcmpiA(matrix, "rec601"))
      return Rec601;
    if (!lstrcmpiA(matrix, "rec709"))
      return Rec709;
    if (!lstrcmpiA(matrix, "PC.601"))
      return PC_601;
    if (!lstrcmpiA(matrix, "PC.709"))
      return PC_709;
    if (!lstrcmpiA(matrix, "PC601"))
      return PC_601;
    if (!lstrcmpiA(matrix, "PC709"))
      return PC_709;
    if (!lstrcmpiA(matrix, "AVERAGE"))
      return AVERAGE;
                           ...
                           : GenericVideoFilter(_child,__FUNCTION__ )
{
  ...
  if (matrix) {
    if (!lstrcmpiA(matrix, "rec709"))
      theMatrix = Rec709;
    else if (!lstrcmpiA(matrix, "PC.601"))
      theMatrix = PC_601;
    else if (!lstrcmpiA(matrix, "PC601"))
      theMatrix = PC_601;
    else if (!lstrcmpiA(matrix, "PC.709"))
      theMatrix = PC_709;
    else if (!lstrcmpiA(matrix, "PC709"))
      theMatrix = PC_709;
    else if (!lstrcmpiA(matrix, "rec601"))
      theMatrix = Rec601;
    else
      env->ThrowError("ConvertToRGB: invalid \"matrix\" parameter (must be matrix=\"Rec601\", \"Rec709\", \"PC.601\" or \"PC.709\")");
  }
  vi.pixel_type = rgb24 ? CS_BGR24 : CS_BGR32;
}
