...
public:
    Turn(PClip _child, int _direction, IScriptEnvironment* env)
	 : GenericVideoFilter(_child,__FUNCTION__ ), Usource(0), Vsource(0) {
		if (_direction) {
		...
		else if (vi.IsPlanar())
		{
			...
			if (direction && !vi.IsY8() && (vi.GetPlaneWidthSubsampling(PLANAR_U) != vi.GetPlaneHeightSubsampling(PLANAR_U)))
			{
				...
				Usource = FilteredResize::CreateResize(Usource, uv_width, uv_height, env);
				Vsource = FilteredResize::CreateResize(Vsource, uv_width, uv_height, env);
			}
		}
		else
		{
  ...
  static AVSValue __cdecl Create_Turn(AVSValue args, void* user_data, IScriptEnvironment* env);
