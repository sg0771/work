...
public:
  Layer( PClip _child1, PClip _child2, const char _op[], int _lev, int _x, int _y, 
         int _t, bool _chroma, IScriptEnvironment* env, int offset = 0);
  PVideoFrame __stdcall GetFrame(int n, IScriptEnvironment* env);
  ...
  static AVSValue __cdecl Create(AVSValue args, void*, IScriptEnvironment* env);
static AVSValue __cdecl TinyCreate(AVSValue args, void*, IScriptEnvironment* env);
private:
  const PClip child1, child2;
  const int levelB, T;
  const bool chroma;
...
class AutoLayer : public IClip
	/**
	  * Class for layering two clips on each other, combined by various functions
	 **/
{
public:
	AutoLayer(PClip _child1, PClip _child2, const char _op[], int _lev, int _x, int _y,
		int _t, bool _chroma, IScriptEnvironment* env, int offset = 0, int color=0xff000000);
	PVideoFrame __stdcall GetFrame(int n, IScriptEnvironment* env);
