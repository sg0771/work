...
void OL_ExclusionImage::BlendImageMask(Image444* base, Image444* overlay, Image444* mask) {
  ...
  if (opacity == 256) {
    for (int y = 0; y < h; y++) {
      for (int x = 0; x < w; x++) {
        int Y = ((int)(baseY[x]^0xff)*ovY[x] + (int)(ovY[x]^0xff)*baseY[x])>>8;
        int U = ((int)(baseU[x]^0xff)*ovY[x] + (int)(ovY[x]^0xff)*baseU[x])>>8;
