...
static int opt_abort_on(void *optctx, const char *opt, const char *arg)
{
    static const AVOption opts[] = {
        { "abort_on"        , NULL, 0, AV_OPT_TYPE_FLAGS, { .i64 = 0.0 }, INT64_MIN, INT64_MAX, .unit = "flags" },
...
#else
static void add_input_streams(OptionsContext *o, AVFormatContext *ic, int videoindex)
{
    int i, ret;
    ...
    for (i = 0; i < ic->nb_streams; i++) {
	  ...
	  /*  if (audioindex!= -1&& ist->st->index!= audioindex) {
			ist->discard = 0;
	    }*/
		...
		if (videoindex != i && st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
			ist->user_set_discard = AVDISCARD_DEFAULT;
			ist->discard = 1;
		}
		else { ist->user_set_discard = AVDISCARD_NONE; }
...
static int open_input_file(OptionsContext *o, const char *filename)
{
	...
	if (strcmp( filename,"custom") ==0)
	{
		...
		AVIOContext* pIOCtx = avio_alloc_context(pBuffer, 32*1024,  // internal Buffer and its size
    ...
    if (o->start_time != AV_NOPTS_VALUE) {
        ...
        if (!(ic->iformat->flags & AVFMT_SEEK_TO_PTS)) {
            ...
            if (dts_heuristic) {
				...
				seek_timestamp = max(seek_timestamp, 0);
            }
        }
    ...
    add_input_streams(o, ic, video_index);
...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index)
{
    ...
    ost->max_muxing_queue_size = 4000;
    ...
    if (source_index >= 0) {
	    ...
	    if (input_streams[source_index]->user_set_discard == AVDISCARD_DEFAULT) {
			input_streams[source_index]->discard = 1;
	    }
    }
...
AVRational rational1 = { 1,AV_TIME_BASE };
static int copy_chapters(InputFile *ifile, OutputFile *ofile, int copy_metadata)
{
    ...
    for (i = 0; i < is->nb_chapters; i++) {
        ...
        int64_t ts_off   = av_rescale_q(start_time - ifile->ts_offset,
			rational1, in_ch->time_base);
        int64_t rt       = (ofile->recording_time == INT64_MAX) ? INT64_MAX :
                           av_rescale_q(ofile->recording_time, rational1, in_ch->time_base);
    ...
    if (!strcmp(file_oformat->name, "ffm") &&
     ...
     if (!o->nb_stream_maps) {
        ...
        if (!o->video_disable && av_guess_codec(oc->oformat, NULL, filename, NULL, AVMEDIA_TYPE_VIDEO) != AV_CODEC_ID_NONE) {
			...
			//InputStream videostream = alloc_input_stream(AVMEDIA_TYPE_VIDEO);
            if (idx >= 0)
        ...
        if (!o->audio_disable && av_guess_codec(oc->oformat, NULL, filename, NULL, AVMEDIA_TYPE_AUDIO) != AV_CODEC_ID_NONE) {
            int best_score = -1, idx = -1;
			//idx = audio_index;
	        if (audio_index !=-1&& input_streams[audio_index]->st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
				idx = audio_index;
	        } else {
				for (i = 0; i < nb_input_streams; i++) {
					int score;
...
static int opt_target(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!strcmp(arg, "vcd")) {
        ...
        o->mux_preload = (36000 + 3 * 1200) / 90000.0f; // 0.44
...
static const OptionGroupDef groups[] = {
	...
	[GROUP_EXTERNAUDIO] = { "extern audio url",   "e",  OPT_EXTERN_AUDIO },
...
int ffmpeg_parse_options(int argc, char **argv)
{
    ...
    if (g_strVdieo)
    {
        ...
        const OptionDef* start_time = find_option(options, "ss");
        const  OptionDef* duration = find_option(options, "t");

        if (g_concatinfocount > 0) {
            for (int i = 0; i < g_concatinfocount; i++) {
                float start;
                float end;
                sscanf(g_concatinfostr[i], "%f:%f", &start, &end);
                fileduration += (end - start);
                char buffer[64];
                snprintf(buffer, sizeof buffer, "%f", start);
                add_opt(&octx, start_time, start_time->name, _strdup(buffer));
                ZeroMemory(buffer, 64);
                snprintf(buffer, sizeof buffer, "%f", end - start);
                add_opt(&octx, duration, duration->name, _strdup(buffer));
                finish_group(&octx, GROUP_INFILE, g_strVdieo);
            }
        }
        else {
            finish_group(&octx, GROUP_INFILE, g_strVdieo);
        ...
        if (g_concatinfocount == 0) {
            fileduration = input_files[0]->ctx->duration;
            fileduration /= 1000000;
        }

        OptionGroup* g = octx.groups[GROUP_OUTFILE].groups;
        OptionsContext o;
        init_options(&o);
        o.g = g;
        ret = parse_optgroup(&o, g);

        int audio_disable = o.audio_disable;
        if (audio_disable) {
            audio_index = -1;
        }
        uninit_options(&o);
        ...
        /*	for (int i = 0; i < nb_input_streams; i++) {
                if ( input_streams[i]->st->nb_frames == 0&& input_streams[i]->st->codecpar->codec_type== AVMEDIA_TYPE_VIDEO) {
        ...
        if (audio_index == -1) {
            ...
            //如果没有选择音频, 默认添加静音, 且导出的格式没有禁止音频
            if (!audio_disable) {

                char muteaudio[256];
                sprintf(muteaudio, "aevalsrc=0:duration=%f", fileduration);
                g_strAudio = _strdup(muteaudio);

                const  OptionDef* duration = find_option(options, "f");
                add_opt(&octx, duration, duration->name, "lavfi");

                finish_group(&octx, GROUP_EXTERNAUDIO, muteaudio);
                ret = open_files(&octx.groups[GROUP_EXTERNAUDIO], "input", open_input_file);
                audio_index = nb_input_streams - 1;

                if (nb_filtergraphs == 1) {
                    opt_filter_complex(NULL, "", "alimiter");
                }
            }
        }
        else {
            ...
            if (g_strAudio) {
                ...
                char buffer[64];
                snprintf(buffer, sizeof buffer, "%f", fileduration);
                add_opt(&octx, duration, duration->name, _strdup(buffer));
                finish_group(&octx, GROUP_EXTERNAUDIO, g_strAudio);
                ret = open_files(&octx.groups[GROUP_EXTERNAUDIO], "audio", open_input_file);
                ...
                for (int i = 0; i < nb_input_streams; i++) {
                    ...
                    if (input_streams[i]->dec_ctx->codec_type == AVMEDIA_TYPE_AUDIO) {
                        if (i % streamcount != audio_index) {
                            input_streams[i]->discard = 1;
                        }
                    }
                    if (input_streams[i]->dec_ctx->codec_type == AVMEDIA_TYPE_SUBTITLE) {
                        if (i % streamcount != subtitle_index) {
                            input_streams[i]->discard = 1;
                        }
                    }
                }
...
const OptionDef options[] = {
    ...
    { "copy_unknown",   OPT_BOOL | OPT_EXPERT,                       {              &copy_unknown_streams },
		...
		{ "ast",   OPT_INT | OPT_EXPERT,{ &audio_index },
    ...
    { "timelimit",      HAS_ARG | OPT_EXPERT,                        { .func_arg = show_help },
    ...
    { "hwaccel",          OPT_VIDEO | OPT_BOOL | OPT_EXPERT ,                        { &dummy },
        "use HW accelerated decoding", "hwaccel name" },
    { "hwaccel_device",    OPT_VIDEO | OPT_BOOL | OPT_EXPERT ,                        { &dummy },
        "select a device for HW acceleration", "devicename" },
    { "hwaccel_output_format",  OPT_VIDEO | OPT_BOOL | OPT_EXPERT ,                        { &dummy },
    ...
    { "hwaccel_lax_profile_check", OPT_BOOL | OPT_EXPERT,                        { &dummy},
    ...
    { "override_ffserver", OPT_BOOL | OPT_EXPERT | OPT_OUTPUT, { &dummy },
        "override the options from ffserver", "" },
    { "sdp_file", OPT_BOOL | OPT_EXPERT | OPT_OUTPUT, { &dummy },
