...
enum {
	...
	FFMPEG_ERROR_EXIT_DECODER = 24,//不支持的解码器
...
static double parse_number_or_die(const char *numstr, int type,
...
static int opt_loglevel(WXCtx *octx, void *optctx, const char *opt, const char *arg) { return 0; }
...
struct WXCtx {
 ...
 int  avffmpeg_getState(WXCtx *octx) {
	return octx->avffmpeg_state;
}

 int64_t avffmpeg_getCurrTime(WXCtx *octx) {
	return octx->avffmpeg_pts_curr;
}

 int64_t avffmpeg_getTotalTime(WXCtx *octx) {
	return octx->avffmpeg_pts_total;
}


 WXCtx * avffmpeg_create() {
 ...
 void    avffmpeg_destroy(WXCtx *octx) {
	if (octx) {
 ...
 void avffmpeg_set_video_encode_mode(WXCtx *octx, int mode) {
	if (octx) {
...
static void ffmpeg_cleanup(WXCtx *octx, int ret);
static void exit_program(WXCtx *octx, int ret);

static int parse_optgroup(WXCtx *octx, void *optctx, OptionGroup *g);

static int split_commandline(WXCtx *octx, int argc, char *argv[],
	const OptionDef *options,
	const OptionGroupDef *groups, int nb_groups);

static void uninit_parse_context(WXCtx *octx);

static int check_stream_specifier(AVFormatContext *s, AVStream *st, const char *spec);

static AVDictionary *filter_codec_opts(WXCtx *octx, AVDictionary *opts, enum AVCodecID codec_id,
	AVFormatContext *s, AVStream *st, AVCodec *codec);

static AVDictionary **setup_find_stream_info_opts(WXCtx *octx, AVFormatContext *s,
	AVDictionary *codec_opts);
...
static void init_opts(WXCtx *octx);
static void uninit_opts(WXCtx *octx);
static int opt_cpuflags(WXCtx *octx, void *optctx, const char *opt, const char *arg);
static int opt_default(WXCtx *octx, void *optctx, const char *opt, const char *arg);
static int opt_loglevel(WXCtx *octx, void *optctx, const char *opt, const char *arg);
static int opt_report(WXCtx *octx, const char *opt);
static int opt_max_alloc(WXCtx *octx, void *optctx, const char *opt, const char *arg);
static int opt_timelimit(WXCtx *octx, void *optctx, const char *opt, const char *arg);
static void parse_options(WXCtx *octx, void *optctx, int argc, char **argv, const OptionDef *options, void(*parse_arg_function)(void *, const char*));
static int parse_option(WXCtx *octx, void *optctx, const char *opt, const char *arg, const OptionDef *options);
static int ifilter_has_all_input_formats(FilterGraph *fg);
static void remove_avoptions(AVDictionary **a, AVDictionary *b);
static void assert_avoptions(WXCtx *octx, AVDictionary *m);
static int guess_input_channel_layout(InputStream *ist);
static enum AVPixelFormat choose_pixel_fmt(AVStream *st, AVCodecContext *avctx, AVCodec *codec, enum AVPixelFormat target);
static void choose_sample_fmt(AVStream *st, AVCodec *codec);
static int configure_filtergraph(FilterGraph *fg);
static int configure_output_filter(FilterGraph *fg, OutputFilter *ofilter, AVFilterInOut *out);
static void check_filter_outputs(WXCtx *octx);
static int ist_in_filtergraph(FilterGraph *fg, InputStream *ist);
static int filtergraph_is_simple(FilterGraph *fg);
static int init_simple_filtergraph(InputStream *ist, OutputStream *ost);
static int init_complex_filtergraph(FilterGraph *fg);
static void sub2video_update(InputStream *ist, AVSubtitle *sub);
static int ifilter_parameters_from_frame(InputFilter *ifilter, const AVFrame *frame);
static int ffmpeg_parse_options(WXCtx *octx, int argc, char **argv);

static HWDevice *hw_device_get_by_name(WXCtx *octx, const char *name);
static int hw_device_init_from_string(WXCtx *octx, const char *arg, HWDevice **dev);
static void hw_device_free_all(WXCtx *octx);
...
static int parse_option(WXCtx *octx, void *optctx, const char *opt, const char *arg,
	const OptionDef *options)
{
	OptionsContext *o = optctx;
	const OptionDef *po;
...
static void parse_options(WXCtx *octx, void *optctx, int argc, char **argv, const OptionDef *options,
...
static int init_input_stream(WXCtx *octx, int ist_index, char *error, int error_len)
{
	...
	if (ist->decoding_needed) {
		...
		//H264 使用多线程解码
		if (stricmp(codec->name, "h264") == 0) {
			ist->dec_ctx->thread_count = 2;// min(WXGetCpuNum(), 4);		//多线程解码;
		}
	...
	if (ost->encoding_needed) {
		...
		if (stricmp(codec->name, "libx264") == 0) {
			...
			if (octx->m_iVideoMode == 0) {
				...
				av_opt_set(ost->enc_ctx->priv_data, "preset", "ultrafast", 0);
			}
			...
			if (octx->m_iVideoMode == 2) {
				...
				av_opt_set(ost->enc_ctx->priv_data, "preset", "faster", 0);
			}
			ost->enc_ctx->qmin = 10;
			ost->enc_ctx->qmax = 51;
			...
			ost->enc_ctx->thread_count = 2;//max(WXGetCpuNum(), 4);		//多线程编码//
...
fail:
 ...
 int    avffmpeg_convert(WXCtx *octx, int argc, char **argv){
...
static int hw_device_init_from_string(WXCtx *octx, const char *arg, HWDevice **dev_out) {
