...
namespace {
FrameInfo ReadFrame(utils::ML_FileHandle&stream, FrameInfo const& prev, const FFMS_TrackType TT) {
    FrameInfo f{};
...
static void WriteFrame(utils::ML_FileHandle&stream, FrameInfo const& f, FrameInfo const& prev, const FFMS_TrackType TT) {
...
FFMS_Track::FFMS_Track(utils::ML_FileHandle&stream)
    : m_Data(std::make_shared<TrackData>()) {
    frame_vec &Frames = m_Data->Frames;
	...
	FPSDenominator = stream.Read<int32_t>();
	FPSNumerator= stream.Read<int32_t>();
	Width= stream.Read<int32_t>();
	Height= stream.Read<int32_t>();
	Pix_Fmt = stream.Read<int32_t>();
	num_audio_samples = stream.Read<int32_t>();
	audio_samples_per_second = stream.Read<int32_t>();
	TopFieldFirst = stream.Read<int32_t>();
	SARNum= stream.Read<int32_t>();
	SARDen= stream.Read<int32_t>();
    size_t FrameCount = static_cast<size_t>(stream.Read<uint64_t>());
	sample_type= stream.Read<int32_t>();
...
void FFMS_Track::Write(utils::ML_FileHandle&stream) const {
    frame_vec &Frames = m_Data->Frames;
    ...
    stream.Write<int32_t>(FPSDenominator);
    stream.Write<int32_t>(FPSNumerator);

	stream.Write<int32_t>(Width);
	stream.Write<int32_t>(Height);
	stream.Write<int32_t>(Pix_Fmt);
	stream.Write<int32_t>(num_audio_samples);
	stream.Write<int32_t>(audio_samples_per_second);


	stream.Write<int32_t>(TopFieldFirst);
	stream.Write<int32_t>(SARNum);
	stream.Write<int32_t>(SARDen);
    stream.Write<uint64_t>(size());
    stream.Write<int32_t>(sample_type);
...
void FFMS_Track::AddVideoFrame(int64_t PTS, int RepeatPict, bool KeyFrame, int FrameType, int64_t FilePos, bool Hidden) {
    m_Data->Frames.push_back({ PTS, 0, FilePos, 0, 0, 0, FrameType, RepeatPict, KeyFrame, Hidden });
}
...
void FFMS_Track::AddAudioFrame(int64_t PTS, int64_t SampleStart, uint32_t SampleCount, bool KeyFrame, int64_t FilePos, bool Hidden) {
    if (SampleCount > 0) {
        m_Data->Frames.push_back({ PTS, 0, FilePos, SampleStart, SampleCount,
...
int FFMS_Track::FrameFromPos(int64_t Pos) const {
    for (size_t i = 0; i < size(); i++)
        if (m_Data->Frames[i].FilePos == Pos)
...
int FFMS_Track::RealFrameNumber(int Frame) const {
    return m_Data->RealFrameNumbers[Frame];
}

int FFMS_Track::VisibleFrameCount() const {
    return TT == FFMS_TYPE_AUDIO ? static_cast<int>(m_Data->Frames.size()) : static_cast<int>(m_Data->RealFrameNumbers.size());
}
...
void FFMS_Track::GeneratePublicInfo() {
    frame_vec &Frames = m_Data->Frames;
    std::vector<int> &RealFrameNumbers = m_Data->RealFrameNumbers;
    std::vector<FFMS_FrameInfo> &PublicFrameInfo = m_Data->PublicFrameInfo;
