...
namespace frei0r
{
  ...
  public:
    ...
    virtual void update(double time, uint32_t* out, const uint32_t* in) = 0;
...
#define  filter_dest_cpp(val)  \
extern"C"{ \
    static int f0r_init(){return 1;}\
    ...
    static void f0r_update2(f0r_instance_t instance, double time,const uint32_t* inframe1,const uint32_t* inframe2,const uint32_t* inframe3,uint32_t* outframe){\
	    static_cast<frei0r::fx*>(instance)->update(time,outframe,inframe1,inframe2,inframe3);\
    }\
    ...
    void _##val##_init() { \
        static f0r_plugin_info_t info_cpp={ \
            f0r_construct, \
            f0r_destruct, \
            f0r_init, \
            f0r_deinit, \
            ...
            f0r_update2, \
            #val,  \
            frei0r::s_author.c_str(),\
            frei0r::s_explanation.c_str(), \
            (int)frei0r::s_effect_type,\
            (int)frei0r::s_color_model,\
            FREI0R_MAJOR_VERSION, \
            (int)frei0r::s_version.first,\
            (int)frei0r::s_version.second,\
            (int)frei0r::s_params.size(),}; \
        Feri0r_Set(&info_cpp);  \
    } \
};
