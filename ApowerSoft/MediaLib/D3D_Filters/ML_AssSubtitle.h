...
public:
	...
	ASSTrackHandler(AssContext _context) :
		ass_track_wrapper(AssEngine::InstanceDX().Read(_context.path), ass_free_track) {
		m_context = _context;
		if (this->get() == NULL)
		{
			this->reset(AssEngine::InstanceDX().Read(_context.path));
		}

	}

	ASSTrackHandler(std::string asscontent) :
		ass_track_wrapper(AssEngine::InstanceDX().Read(asscontent), ass_free_track) {}
	...
	ass_image* Render(int ts)
	{
			...
			n < (int)1000 * m_context.offset + (int)1000 * (m_context.end - m_context.start))
		{
			...
			return ass_render_frame(AssEngine::InstanceDX().ass_renderer, this->get(), subtime, NULL);
		}
		return NULL;
	}
};  // class FileHandler
