...
httpd_add_connection(httpd_t *httpd, int fd, unsigned char *local, int local_len, unsigned char *remote, int remote_len)
{
	...
	httpd->connections[i].user_data = httpd->callbacks.conn_init(fd,httpd->callbacks.opaque, local, local_len, remote, remote_len);
}
...
static void httpd_remove_connection(httpd_t *httpd, http_connection_t *connection)
{
	...
	if (httpd->is_mirror)
	{
		...
		httpd->callbacks.conn_mirror_destroy(connection);
	}
...
void httpd_remove_connection_new(httpd_t *httpd)
{
...
httpd_thread(void *arg)
{
	...
	while (1) {
		...
		for (i=0; i<httpd->max_connections; i++) {
			...
			if (httpd_get_disconnect(httpd))
			{
				httpd_remove_connection(httpd, connection);
				continue;
			}

			//ret = recvfrom(connection->socket_fd, buffer, sizeof(buffer), 0, , );
...
httpd_start(httpd_t *httpd, unsigned short *port)
{
    ...
    httpd->server_fd6 = -1;
    httpd->server_fd4 = netutils_init_socket(port, 0, 1);
    if (httpd->server_fd4 == -1) {
        int iError = SOCKET_GET_ERROR();
        //logger_log(httpd->logger, LOGGER_ERR, "Error initialising socket %d", iError);
        MUTEX_UNLOCK(httpd->run_mutex);
        if (iError == 10048 || iError == 48)
        {
            return -3;
        }
        return -1;
...
//#endif
    //logger_log(httpd->logger, LOGGER_INFO, "Initialized server socket(s)");
    	...
    	httpd->joined = 0;
    	httpd->port = *port;
    	httpd->is_mirror = (httpd->port == 6000);
    	httpd->stream_start = 0;
    	httpd->mirror_disconn = 0;
...
void httpd_disconnect(httpd_t *httpd)
{
	assert(httpd);
	MUTEX_LOCK(httpd->run_mutex);
...
int httpd_get_disconnect(httpd_t *httpd)
{
	int status;
	assert(httpd);
	MUTEX_LOCK(httpd->run_mutex);
