...
void init_queue(NewQueue *que, int size) {
	...
	InitializeCriticalSection((LPCRITICAL_SECTION)&que->locker);
...
void free_queue(NewQueue* que) {
	...
	DeleteCriticalSection((LPCRITICAL_SECTION)&que->locker);
	free(que->buf);
	que->buf = NULL;
}
...
//	fp = fopen("out.ts", "w");
	...
	memset(&s_addr2, 0, sizeof(struct sockaddr_in));
	s_addr2.sin_family = AF_INET;
	//if (argv[1])
	//	s_addr2.sin_port = htons(atoi(argv[1]));
	//else
		s_addr2.sin_port = htons(62583);


	s_addr2.sin_addr.s_addr = INADDR_ANY;

	if ( (bind(sock, (struct sockaddr*)&s_addr2, sizeof(s_addr2))) == -1 ) {
		perror("bind");
		exit(errno);
	}else
