...
{
    std::set<Ztring> Extensions=ParseOnlyKnownExtensions_GetList_Set();
    Ztring List;
    for (std::set<Ztring>::iterator Extension=Extensions.begin(); Extension!=Extensions.end(); Extension++)
    {
        List+=*Extension;
        List+=__T(',');
    }
...
void MediaInfo_Config::Log_Send (int8u Type, int8u Severity, int32u MessageCode, const Ztring &Message)
{
    ...
    std::wstring MessageU=Message.To_Unicode();
    std::string MessageA=Message.To_Local();
