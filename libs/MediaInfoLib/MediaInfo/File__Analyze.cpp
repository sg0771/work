...
bool File__Analyze::FileHeader_Begin_0x000001()
{
    ...
    size_t Buffer_Offset_ts = 0;
    while (Buffer_Offset_ts <MPEGTS_LEN && Buffer[Buffer_Offset_ts]!= MPEGTS_BYTE) //Look for first Sync word
        Buffer_Offset_ts++;
    if (Buffer_Offset_ts < MPEGTS_LEN && Buffer[Buffer_Offset_ts + MPEGTS_LEN]== MPEGTS_BYTE && Buffer[Buffer_Offset_ts + MPEGTS_LEN *2]== MPEGTS_BYTE && Buffer[Buffer_Offset_ts + MPEGTS_LEN *3]== MPEGTS_BYTE)
    {
    ...
    while (Buffer_Offset < MPEGTS_LEN && Buffer[Buffer_Offset] != XWS_BYTE) //Look for first Sync word
        Buffer_Offset++;
    if (Buffer_Offset < MPEGTS_LEN && Buffer[Buffer_Offset + MPEGTS_LEN] == XWS_BYTE && Buffer[Buffer_Offset + MPEGTS_LEN * 2] == XWS_BYTE && Buffer[Buffer_Offset + MPEGTS_LEN * 3] == XWS_BYTE)
    {
        Status[IsFinished] = true;
        return false;
    }
