...
//Patch for xws
//Tam.Xie
#define MPEGTS_BYTE 0x47
#define XWS_BYTE  0x77
#define MPEGTS_LEN 188
...
{
    //Synchronizing
    while (       Buffer_Offset+ MPEGTS_LEN *16+BDAV_Size*16+TSP_Size*16<=Buffer_Size
      && !(Buffer[Buffer_Offset+ MPEGTS_LEN * 0+BDAV_Size* 1+TSP_Size* 0]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN * 1+BDAV_Size* 2+TSP_Size* 1]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN * 2+BDAV_Size* 3+TSP_Size* 2]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN * 3+BDAV_Size* 4+TSP_Size* 3]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN * 4+BDAV_Size* 5+TSP_Size* 4]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN * 5+BDAV_Size* 6+TSP_Size* 5]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN * 6+BDAV_Size* 7+TSP_Size* 6]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN * 7+BDAV_Size* 8+TSP_Size* 7]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN * 8+BDAV_Size* 9+TSP_Size* 8]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN * 9+BDAV_Size*10+TSP_Size* 9]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN *10+BDAV_Size*11+TSP_Size*10]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN *11+BDAV_Size*12+TSP_Size*11]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN *12+BDAV_Size*13+TSP_Size*12]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN *13+BDAV_Size*14+TSP_Size*13]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN *14+BDAV_Size*15+TSP_Size*14]==MPEGTS_BYTE
        && Buffer[Buffer_Offset+ MPEGTS_LEN *15+BDAV_Size*16+TSP_Size*15]==MPEGTS_BYTE))
    {
        Buffer_Offset++;
        while (       Buffer_Offset+BDAV_Size+1<=Buffer_Size
            && Buffer[Buffer_Offset+BDAV_Size]!=MPEGTS_BYTE)
            Buffer_Offset++;
    }

    if (Buffer_Offset+ MPEGTS_LEN *16+BDAV_Size*16+TSP_Size*16>=Buffer_Size
    #ifdef MEDIAINFO_ARIBSTDB24B37_YES
     && !FromAribStdB24B37
    #endif
        )
...
bool File_MpegTs::Synched_Test()
{
    ...
    {
        //Synchro testing
        if (Buffer[Buffer_Offset+BDAV_Size]!=MPEGTS_BYTE)
        {
            Synched=false;
            #if MEDIAINFO_DUPLICATE
            ...
            if (payload_unit_start_indicator)
            {
                ...
                if (Stream->Searching_Payload_Start) //payload_unit_start_indicator
                {
                    if (Stream->Kind==complete_stream::stream::psi)
                    {
                        ...
                        if (Version_Pos>=BDAV_Size+ MPEGTS_LEN)
                        ...
                        if (*Table_ID)
                        {
                            ...
                            if (Version_Pos+5>=BDAV_Size+ MPEGTS_LEN)
...
void File_MpegTs::Header_Parse_AdaptationField()
{
    ...
    if (Trace_Activated)
    {
        ...
        if (adaptation_field_length> MPEGTS_LEN -4-1) //TS size - header - adaptation_field_length
        {
            adaptation_field_length= MPEGTS_LEN -4-1;
            Skip_XX(MPEGTS_LEN -4-1,                                        "stuffing_bytes");
        }
