...
bool File__Duplicate_MpegTs::Parsing_Begin (const int8u* ToAdd, size_t ToAdd_Size, std::map<int16u, buffer> &ToModify_)
{
    ...
    else
    {
        ...
        for (size_t Pos=0; Pos<ToModify.Size; Pos+= MPEGTS_LEN)
        {
    ...
    if (FromTS.End>FromTS.Size-4)
    {
        ...
        if (BigBuffers[PID].Buffer==NULL)
        {
            ...
            BigBuffers[PID].Buffer_Size_Max=FromTS.End+ MPEGTS_LEN;
    ...
    if (ToModify.Buffer==NULL)
        ToModify.Buffer=new int8u[FromTS.Size+(FromTS.Size/ MPEGTS_LEN)*4];
...
void File__Duplicate_MpegTs::Parsing_End (std::map<int16u, buffer> &ToModify_)
{
    ...
    //Managing big chunks
    for (size_t Pos= MPEGTS_LEN; Pos<ToModify.Size; Pos+= MPEGTS_LEN)
    {
    ...
    while (ToModify.Size-(ToModify.Offset+4)> MPEGTS_LEN)
        ToModify.Size-= MPEGTS_LEN;
