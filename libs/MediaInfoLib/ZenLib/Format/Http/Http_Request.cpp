...
bool Request::Http_Begin(std::istream &In, std::ostream &Out)
{
    ...
    if (Http->Path.find('%')!= std::string::npos)
    ...
    do
    {
        ...
        if (!Line.empty())
        {
            ...
            std::string::size_type Content_Begin=Line.find_first_not_of(' ', Separator_Pos+1);
            if (Separator_Pos!= std::string::npos && Content_Begin!= std::string::npos)
            {
                std::string Command=Line.substr(0, Separator_Pos);
                if (Command=="Cookie")
                {
                    std::string Cookie=Line.substr(Separator_Pos+1, std::string::npos);
                    while (!Cookie.empty())
                    {
                        std::string::size_type Cookie_Pos=Cookie.rfind(';');
                        if (Cookie_Pos== std::string::npos)
                            Cookie_Pos=(std::string::size_type)-1;
                        std::string Line2=Cookie.substr(Cookie_Pos+1, std::string::npos);
                        TrimLeft(Line2, ' ');
                        if (Cookie_Pos!=(std::string::size_type)-1)
                        {
                        ...
                        std::string::size_type Separator_Pos2=Line2.find('=');
                        if (Separator_Pos2!= std::string::npos)
                            Http->Request_Cookies[Line2.substr(0, Separator_Pos2)]=Format::Http::URL_Encoded_Decode(Line2.substr(Separator_Pos2+1, std::string::npos));
                    }
                }
                else
                    Http->Request_Headers[Command]=Line.substr(Content_Begin, std::string::npos);
            }
        }
    }
    while (!Line.empty());
     ...
     || (Method.size()==4 && Method[0]=='P' && Method[1]=='O' && Method[2]=='S' && Method[3]=='T'))
    {
        do
        {
            ...
            //Getting the line
            std::string::size_type Interogation_Pos=Http->Path.find('?');
            if (Interogation_Pos!= std::string::npos)
            {
                Content=Http->Path.substr(Interogation_Pos+1, std::string::npos);
            ...
            if (Method.size()==4) //Only for POST
            {
                ...
                In.read(&Content[Content_Size_Current], (std::streamsize)Content_Lengh);
            ...
            while (!Content.empty())
            {
                std::string::size_type Content_Pos=Content.rfind('&');
                if (Content_Pos== std::string::npos)
                    Content_Pos=(std::string::size_type)-1;
                std::string Line2=Content.substr(Content_Pos+1, std::string::npos);
                if (Content_Pos!=(std::string::size_type)-1)
                    Content.resize(Content_Pos);
                else
                    Content.clear();

                std::string::size_type Separator_Pos=Line2.find('=');
                if (Separator_Pos!= std::string::npos)
                    Http->Request_Queries[Line2.substr(0, Separator_Pos)]=Format::Http::URL_Encoded_Decode(Line2.substr(Separator_Pos+1, std::string::npos));
            }
        }
        while (!Line.empty());
    }
