...
Ztring& Ztring::From_Number (const float32 F, int8u Precision, ztring_t Options)
{
    ...
    #else
        toStringStream Stream;
        Stream << std::setprecision(Precision) << std::fixed << F;
        assign(Stream.str());
    ...
    if ((Options & Ztring_NoZero && size()>0) && find(__T('.'))!= std::string::npos)
    {
...
bool Ztring::Compare (const Ztring &ToCompare, const Ztring &Comparator, ztring_t Options) const
{
    ...
    if (!(Options & Ztring_CaseSensitive))
    {
        ...
        //string comparasion
        if (Comparator==__T("==")) return (Left==Right);
        if (Comparator==__T("IN")) {if (Left.find(Right)!= std::string::npos) return true; else return false;}
        if (Comparator==__T("<"))  return (Left< Right);
        if (Comparator==__T("<=")) return (Left<=Right);
        if (Comparator==__T(">=")) return (Left>=Right);
        if (Comparator==__T(">"))  return (Left> Right);
    ...
    else
    {
        ...
        if (Comparator==__T("IN")) {if (this->find(ToCompare)!=std::string::npos) return true; else return false;}
        if (Comparator==__T("<"))  return (*this< ToCompare);
        if (Comparator==__T("<=")) return (*this<=ToCompare);
        if (Comparator==__T(">=")) return (*this>=ToCompare);
        if (Comparator==__T(">"))  return (*this> ToCompare);
