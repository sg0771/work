...
RotateAny(FIBITMAP *src, double dAngle, const void *bkcolor) {
	...
	if(0 == dAngle) {
		if(image == src) {
			...
			FreeImage_CopyIccProfile(image, src);
			return image;
		}
	}
	else {
		...
		FreeImage_CopyIccProfile(dst, src);
		return dst;
	}
}

// ==========================================================
//要把ICC Profile Copy过去
//-----------------------
