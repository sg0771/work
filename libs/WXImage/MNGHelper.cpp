		...
		while(mEnd == FALSE) {
			...
			switch( mng_GetChunckType(mChunkName) ) {
				...
				case IHDR:
					...
					if(hPngMemory == NULL) {
						hPngMemory = FreeImage_OpenMemory(NULL, 0);
					}
				...
				case JDAT:
					if(hJpegMemory == NULL) {
						hJpegMemory = FreeImage_OpenMemory(NULL, 0);
					}
				...
				case IDAT:
					if(!header_only && (jng_alpha_compression_method == 0)) {
						// PNG grayscale IDAT format
						if(hIDATMemory == NULL) {
							hIDATMemory = FreeImage_OpenMemory(NULL, 0);
							mHasIDAT = TRUE;
						}
...
mng_WriteJNG(int format_id, FreeImageIO *io, FIBITMAP *dib, fi_handle handle, int flags) {
	...
	try {
		hJngMemory = FreeImage_OpenMemory(NULL, 0);
