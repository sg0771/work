...
Load(FreeImageIO* io, fi_handle handle, int page, int flags, void* data) {
	...
	if (handle) {
		...
		try {
			...
			bit_depth = png_get_bit_depth(png_ptr, info_ptr);//位深度，可能是8位或者16位
			pixel_depth = bit_depth * png_get_channels(png_ptr, info_ptr);//位深度*RGB通道
			// 
			...
			case PNG_COLOR_TYPE_PALETTE:
				...
				if (dib) {
					...
					RGBQUAD* palette = FreeImage_GetPalette(dib); //调色板
			...
			for (png_uint_32 k = 0; k < height; k++) {
				row_pointers[height - 1 - k] = FreeImage_GetScanLine(dib, k);//解码行
			}

			png_set_benign_errors(png_ptr, 1);

			bStartLoadImage = TRUE;//开始解码

			//
			png_read_image(png_ptr, row_pointers);//解码操作

			// check if the bitmap contains transparency, if so enable it in the header

			if (FreeImage_GetBPP(dib) == 32) {
				if (FreeImage_GetColorType(dib) == FIC_RGBALPHA) {
					FreeImage_SetTransparent(dib, TRUE);//透明
				}
				else {
					FreeImage_SetTransparent(dib, FALSE);//不透明
				}
			}
...
static BOOL
Save_PNG(FreeImageIO* io, FIBITMAP* dib, fi_handle handle, int page, int _flags, void* data) {
	png_structp png_ptr;
	png_infop info_ptr;
	...
	if ((dib) && (handle)) {
		try {
			...
			if ((_flags & PNG_INTERLACED) == PNG_INTERLACED) { //交错
			...
			int zlib_level = _flags & 0x0F;
			if ((zlib_level >= 1) && (zlib_level <= 9)) {
				png_set_compression_level(png_ptr, zlib_level);
			}
			else if ((_flags & PNG_Z_NO_COMPRESSION) == PNG_Z_NO_COMPRESSION) {
