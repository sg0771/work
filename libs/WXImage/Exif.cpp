...
processMakerNote(FIBITMAP *dib, const char *pval, BOOL msb_order, DWORD *subdirOffset, TagLib::MDMODEL *md_model) {
	...
	*md_model = TagLib::UNKNOWN;
	...
	if(tagMake != NULL){
		Maker = (char*)FreeImage_GetTagValue(tagMake);
	}
...
jpeg_read_exif_dir(FIBITMAP *dib, const BYTE *tiffp, DWORD dwOffsetIfd0, DWORD dwLength, DWORD dwProfileOffset, BOOL msb_order, TagLib::MDMODEL starting_md_model) {
	...
	//BYTE tmp[1000];
	//memcpy(tmp, tiffp + dwOffsetIfd0, dwLength);

	//for (int i = 0; i < 1000-1; i++)
	//{
	//	//角度
	//	if(tmp[i] == 0x01 && tmp[i+1] == 0x12 && tmp[i + 2] == 0) {
	//		int kk = 0;
	//	}
	//}
...
//解析 EXIF RAW 信息
BOOL  jpeg_read_exif_profile(FIBITMAP *dib, const BYTE *data, unsigned length) {
...
jpeg_read_exif_profile_raw(FIBITMAP *dib, const BYTE *profile, unsigned length) {
	...
	if(tag) {
		//Add By Tam
		//强制修改 EXIF RAW 中的 旋转角度为0！！！！
		//2023.03.15
		int findRotate = 0;
		BYTE* tmp = new BYTE[length];
		memcpy(tmp, profile, length);
		for (int i = 0; i < length - 5; i++){
			if (tmp[i] == 0x01 && tmp[i+1] == 0x12 && tmp[i+2] == 0x00) {
				tmp[i + 3] = 0x00;
				findRotate = 1;
				break;
			}
		...
		FreeImage_SetTagValue(tag, findRotate ? tmp : profile);
