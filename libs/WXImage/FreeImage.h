...
FI_ENUM(FREE_IMAGE_FORMAT) {
	FIF_UNKNOWN = -1,
		...
		FIF_RAW = 33,
		FIF_WEBP = 34,
		FIF_HEIC = 35,//HEIC 图像处理
};

FI_STRUCT (FIBITMAP) { 
	void *data; 
	/** space to hold encode data stream*/
	FIMEMORY *m_stream;
	FREE_IMAGE_FORMAT m_srcFormat;//原始图像格式
	int m_bSrcCMYK;//原始图像是否CMYK
};
...
FI_ENUM(FREE_IMAGE_MDMODEL) {
	FIMD_NODATA			= -1,
	FIMD_COMMENTS		= 0,	//! single comment or keywords //EXIF
	FIMD_EXIF_MAIN		= 1,	//! Exif-TIFF metadata  //EXIF
	FIMD_EXIF_EXIF		= 2,	//! Exif-specific metadata //JXR
	FIMD_EXIF_GPS		= 3,	//! Exif GPS metadata     //JXR
	FIMD_EXIF_MAKERNOTE = 4,	//! Exif maker note metadata  //Nothing
	FIMD_EXIF_INTEROP	= 5,	//! Exif interoperability metadata  //Nothing
	FIMD_IPTC			= 6,	//! IPTC/NAA metadata   //EXIF
	FIMD_XMP			= 7,	//! Abobe XMP metadata  //EXIF
	FIMD_GEOTIFF		= 8,	//! GeoTIFF metadata  //TIFF
	FIMD_ANIMATION		= 9,	//! Animation metadata  //GIF 
	FIMD_CUSTOM			= 10,	//! Used to attach other metadata types to a dib //Nothing
	FIMD_EXIF_RAW		= 11	//! Exif metadata as a raw buffer  //EXIF
};
...
FI_STRUCT(FreeImageIO) {
    ...
    FI_TellProc  tell_proc;     //! pointer to the function used to aquire the current positions
};
...
// Message output functions -------------------------------------------------
 ...
 void  FreeImage_OutputMessageProc(int fif, const char* fmt, ...); //内部log

typedef void ( *FreeImage_OutputMessageFunction)(FREE_IMAGE_FORMAT fif, const char *msg); 
 void  FreeImage_SetOutputMessage(FreeImage_OutputMessageFunction omf);  //設置log回調


// Allocate / Clone / Unload routines ---------------------------------------
//Add By Tam 
//2021.10.14
 void  FreeImage_CopyIccProfile(FIBITMAP *new_dib, FIBITMAP *dib);
...
// DIB info routines --------------------------------------------------------
 ...
 unsigned  FreeImage_GetMemorySize(FIBITMAP *dib);
...
// Smart conversion routines ------------------------------------------------
 ...
 FIBITMAP * FreeImage_ConvertFromRawBitsEx(BOOL copySource, BYTE *bits, FREE_IMAGE_TYPE type, int width, int height, int pitch, unsigned bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask, BOOL topdown FI_DEFAULT(FALSE));
...
// --------------------------------------------------------------------------
 ...
 BOOL  FreeImage_JPEGTransform(const char *src_file, const char *dst_file, FREE_IMAGE_JPEG_OPERATION operation, BOOL perfect FI_DEFAULT(TRUE));
 BOOL  FreeImage_JPEGTransformU(const wchar_t *src_file, const wchar_t *dst_file, FREE_IMAGE_JPEG_OPERATION operation, BOOL perfect FI_DEFAULT(TRUE));
 ...
 BOOL  FreeImage_JPEGTransformFromHandle(FreeImageIO* src_io, fi_handle src_handle, FreeImageIO* dst_io, fi_handle dst_handle, FREE_IMAGE_JPEG_OPERATION operation, int* left, int* top, int* right, int* bottom, BOOL perfect FI_DEFAULT(TRUE));
 BOOL  FreeImage_JPEGTransformCombined(const char *src_file, const char *dst_file, FREE_IMAGE_JPEG_OPERATION operation, int* left, int* top, int* right, int* bottom, BOOL perfect FI_DEFAULT(TRUE));
 BOOL  FreeImage_JPEGTransformCombinedU(const wchar_t *src_file, const wchar_t *dst_file, FREE_IMAGE_JPEG_OPERATION operation, int* left, int* top, int* right, int* bottom, BOOL perfect FI_DEFAULT(TRUE));
 BOOL  FreeImage_JPEGTransformCombinedFromMemory(FIMEMORY* src_stream, FIMEMORY* dst_stream, FREE_IMAGE_JPEG_OPERATION operation, int* left, int* top, int* right, int* bottom, BOOL perfect FI_DEFAULT(TRUE));
...
// upsampling / downsampling
 ...
 FIBITMAP * FreeImage_RescaleRect(FIBITMAP *dib, int dst_width, int dst_height, int left, int top, int right, int bottom, FREE_IMAGE_FILTER filter FI_DEFAULT(FILTER_CATMULLROM), unsigned flags FI_DEFAULT(0));
...
// copy / paste / composite routines
 ...
 FIBITMAP * FreeImage_CreateView(FIBITMAP *dib, unsigned left, unsigned top, unsigned right, unsigned bottom);
