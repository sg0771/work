...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data) {
	...
	try {
		...
		int DstW = FreeImage_GetWidth(dib);
		int DstH = FreeImage_GetHeight(dib);
		int DstBpp = FreeImage_GetBPP(dib);

		// try to get JPEG embedded Exif metadata
		if(dib && !((flags & RAW_PREVIEW) == RAW_PREVIEW)) {
			FIBITMAP *metadata_dib = libraw_LoadEmbeddedPreview(RawProcessor, 0/*FIF_LOAD_NOPIXELS*/);
			if(metadata_dib) {
