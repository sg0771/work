...
FreeImage_AllocateBitmap(BOOL header_only, BYTE *ext_bits, unsigned ext_pitch, FREE_IMAGE_TYPE type, int width, int height, int bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask) {
	...
	bitmap->data = NULL;
	bitmap->m_stream = NULL;
	bitmap->m_bSrcCMYK = FALSE;
	if (bitmap != NULL) {
		...
		if (bitmap->data != NULL) {
			...
			fih->m_metadata = new(std::nothrow) METADATAMAP;
...
void FreeImage_Unload(FIBITMAP *dib) {
	if (NULL != dib) {	
		if (NULL != dib->data) {
			...
			METADATAMAP *metadata = ((FREEIMAGEHEADER *)dib->data)->m_metadata;
...
FreeImage_Clone(FIBITMAP *dib) {
	...
	if (new_dib) {
		...
		// save metadata links
		METADATAMAP *src_metadata = ((FREEIMAGEHEADER *)dib->data)->m_metadata;
		METADATAMAP *dst_metadata = ((FREEIMAGEHEADER *)new_dib->data)->m_metadata;
		...
		((FREEIMAGEHEADER *)new_dib->data)->m_metadata = dst_metadata;
...
FreeImage_GetICCProfile(FIBITMAP *dib) {
	FIICCPROFILE *profile = (dib) ? (FIICCPROFILE *)&((FREEIMAGEHEADER *)dib->data)->m_iccProfile : NULL;
	return profile;
}
...
FreeImage_GetHeight(FIBITMAP *dib) {
	return (dib) ? abs(FreeImage_GetInfoHeader(dib)->biHeight) : 0;
}
...
BOOL FreeImage_CloneMetadata(FIBITMAP *dst, FIBITMAP *src) {
	...
	// get metadata links
	METADATAMAP *src_metadata = ((FREEIMAGEHEADER *)src->data)->m_metadata;
	METADATAMAP *dst_metadata = ((FREEIMAGEHEADER *)dst->data)->m_metadata;
		...
		if(src_tagmap) {
			...
			if(dst_tagmap) {
				...
				for(TAGMAP::iterator j = src_tagmap->begin(); j != src_tagmap->end(); j++) {
					...
					//旋转信息不拷贝....
					// Add By Tam
					if (FreeImage_stricmp(dst_key.c_str(), "Orientation") != 0) {
						FITAG* dst_tag = FreeImage_CloneTag((*j).second);
...
BOOL  FreeImage_SetMetadata(FREE_IMAGE_MDMODEL model, FIBITMAP *dib, const char *key, FITAG *tag) {
	...
	if (FreeImage_stricmp(key,"Orientation") == 0) {
		int debug = 0;
	}
...
FreeImage_GetMetadata(FREE_IMAGE_MDMODEL model, FIBITMAP *dib, const char *key, FITAG **tag) {
	...
	*tag = NULL;
	...
	if (*tag != NULL) {
		xFITAGHEADER* h = (xFITAGHEADER*)tag;
		WORD rx = *((WORD*)FreeImage_GetTagValue(*tag));
		int y = rx * rx;
	}
...
unsigned  FreeImage_GetMemorySize(FIBITMAP *dib) {
	...
	// add metadata size
	METADATAMAP *md = header->m_metadata;
	if (!md) {
