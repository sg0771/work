		...
		if (node != NULL) {
			if(node->m_plugin->load_proc != NULL) {
				...
				if (bitmap) {
					//保存原来的DPI信息
					//Add By Tam
					int resX = FreeImage_GetDotsPerMeterX(bitmap);;
					int resY = FreeImage_GetDotsPerMeterY(bitmap);;
					...
					bool isAlpha = FreeImage_IsTransparent(bitmap);
					if (isAlpha) { //8位，但是带有透明通道
						FIBITMAP* new_dib = (FIBITMAP*)FreeImage_ConvertTo32Bits(bitmap);
						new_dib->m_bSrcCMYK = bitmap->m_bSrcCMYK;
						FreeImage_CopyIccProfile(new_dib, bitmap);
						FreeImage_Unload(bitmap);
						bitmap = new_dib;
					}
					else if(bpp != 24){ //8位，带有颜色，但是没有透明通道
						FIBITMAP* new_dib = (FIBITMAP*)FreeImage_ConvertTo24Bits(bitmap);
						new_dib->m_bSrcCMYK = bitmap->m_bSrcCMYK;
						FreeImage_CopyIccProfile(new_dib, bitmap);
						FreeImage_Unload(bitmap);
						bitmap = new_dib;
					...
					//保存原来的DPI信息
					//Add By Tam
					FreeImage_SetDotsPerMeterX(bitmap, resX);;
					FreeImage_SetDotsPerMeterY(bitmap, resY);;
				}
				return bitmap;
			}
		}
