...
#include "../lcsm2/lcms2.h"  //Handle for icc Profile
extern int g_bUseIccParser;//
// ----------------------------------------------------------
...
jpeg_read_jfxx(FIBITMAP* dib, const BYTE* dataptr, unsigned int datalen) {
	...
	case JFXX_TYPE_JPEG:
	{
		...
		FIBITMAP* thumbnail = FreeImage_LoadFromMemory(FIF_JPEG, hmem, 0);
...
static
cmsUInt32Number GetInputPixelType(const jpeg_decompress_struct& Decompressor)
{
	int space = 0;

	cmsBool lIsITUFax = IsITUFax(Decompressor.marker_list);
	cmsBool lIsPhotoshopApp13 = HandlePhotoshopAPP13(Decompressor.marker_list);

	int ColorChannels = Decompressor.num_components;
	int extra = 0;            // Alpha = None
	int bps = 1;            // 8 bits
	int Flavor = 0;            // Vanilla
	...
	if (handle) {
		...
		try {
			...
			if ((cinfo.output_components == 4) && (cinfo.out_color_space == JCS_CMYK)) {
				...
				if (cinfo.output_components == 1) { //颜色表
			...
			if (header_only) { //只处理文件头
				// release JPEG decompression object
				jpeg_destroy_decompress(&cinfo);
				// return header data
							// DPI 信息在EXIF内，特殊处理
			//没有正确获得DPI信息，读取EXIF的DPI信息
			//Add By Tam
			//2835 72dpi
				if (cinfo.density_unit == 0) {
					...
					int DPIY = 0;
					FITAG* tagX = NULL;
					FreeImage_GetMetadata(FIMD_EXIF_MAIN, dib, "XResolution", &tagX);
					if (tagX) {
						DWORD* pvalue = (DWORD*)FreeImage_GetTagValue(tagX);
						DPIX = int(float(pvalue[0]) / float(pvalue[1]) + 0.5);
					}
					FITAG* tagY = NULL;
					FreeImage_GetMetadata(FIMD_EXIF_MAIN, dib, "YResolution", &tagY);
					if (tagY) {
						DWORD* pvalue = (DWORD*)FreeImage_GetTagValue(tagX);
						DPIY = int(float(pvalue[0]) / float(pvalue[1]) + 0.5);
					}

					if (DPIX > 0 && DPIY > 0) {
						FreeImage_SetDotsPerMeterX(dib, (unsigned)(((float)DPIX) / 0.0254000 + 0.5));
						FreeImage_SetDotsPerMeterY(dib, (unsigned)(((float)DPIY) / 0.0254000 + 0.5));
					}
				}
				return dib;
			}
			...
			if ((cinfo.out_color_space == JCS_CMYK) && ((flags & JPEG_CMYK) != JPEG_CMYK)) {
				...
				BOOL hasIcc = FALSE;
				FIICCPROFILE* icc_profile = FreeImage_GetICCProfile(dib);
				if (g_bUseIccParser && icc_profile && icc_profile->size > 0) {
					//cmyk-icc
					hasIcc = TRUE;
					void* icc_data = icc_profile->data;
					cmsUInt32Number icc_size = icc_profile->size;

					cmsHPROFILE  hInProfile = cmsOpenProfileFromMem(icc_data, icc_size);
					cmsHPROFILE  hOutProfile = cmsCreate_sRGBProfile();  //目标标准RGB
					//copy from  lcms2  sample  jpeg-icc
					cmsUInt32Number InputFormat = GetInputPixelType(cinfo); //例子用 TYPE_CMYK_8_REV;//去掉REV颜色全黑
					cmsUInt32Number OutputFormat = TYPE_BGR_8;//输出RGB888

					cmsHTRANSFORM  hTransform = cmsCreateTransform(hInProfile, InputFormat,
						hOutProfile, OutputFormat,
						INTENT_PERCEPTUAL,
					...
					if (hTransform == nullptr) {
						...
						{
							JSAMPROW in = buffer[0];
							jpeg_read_scanlines(&cinfo, buffer, 1); //解码一行CMYK数据
							JSAMPROW out = FreeImage_GetScanLine(dib, cinfo.output_height - i - 1);
							cmsDoTransform(hTransform, in, out, cinfo.output_width);  //使用lcms2转RGB
						}

						cmsDeleteTransform(hTransform);
						cmsCloseProfile(hInProfile);
						cmsCloseProfile(hOutProfile);
						FreeImage_DestroyICCProfile(dib);//去除IccProfile
					}

				}

				if (hasIcc == FALSE) { //cmyk, no icc
					while (cinfo.output_scanline < cinfo.output_height) {
						JSAMPROW in = buffer[0];
						JSAMPROW out = FreeImage_GetScanLine(dib, cinfo.output_height - cinfo.output_scanline - 1);
						jpeg_read_scanlines(&cinfo, buffer, 1);
						for (unsigned x = 0; x < cinfo.output_width; x++) {
							WORD K = (WORD)in[3];
							out[FI_RGBA_RED] = (BYTE)((K * in[0]) / 255);	// C -> R
							out[FI_RGBA_GREEN] = (BYTE)((K * in[1]) / 255);	// M -> G
							out[FI_RGBA_BLUE] = (BYTE)((K * in[2]) / 255);	// Y -> B
							in += 4;
							out += 3;
						}//可能需要做一次ICC 处理
					}
				...
				if (g_bUseIccParser && icc_profile && icc_profile->size > 0) {
					...
					//输出RGB888 //几乎不偏色
					//需要多次测试
					cmsUInt32Number InputFormat = GetInputPixelType(cinfo);//实际上是RGB888
					cmsUInt32Number OutputFormat = TYPE_BGR_8;//输出RGB888
					if (InputFormat == TYPE_RGB_8) {
						OutputFormat = TYPE_RGB_8;
					}
					else if (InputFormat == TYPE_BGR_8) {
					...
					if (hTransform == nullptr) {
						...
						for (size_t i = 0; i < cinfo.output_height; i++) {
							JSAMPROW in = buffer[0];
							jpeg_read_scanlines(&cinfo, buffer, 1); //解码RGB数据
							JSAMPROW out = FreeImage_GetScanLine(dib, cinfo.output_height - i - 1);
							cmsDoTransform(hTransform, in, out, cinfo.output_width);  //使用lcms2处理icc
						}
						cmsDeleteTransform(hTransform);
						cmsCloseProfile(hInProfile);
						cmsCloseProfile(hOutProfile);
				...
				if (!hasIcc) {
					while (cinfo.output_scanline < cinfo.output_height) {
						JSAMPROW dst = FreeImage_GetScanLine(dib, cinfo.output_height - cinfo.output_scanline - 1);
						jpeg_read_scanlines(&cinfo, &dst, 1);//BGR_8 输出
					}
				}
			...
			if (cinfo.density_unit == 0) {
				...
				FreeImage_GetMetadata(FIMD_EXIF_MAIN, dib, "YResolution", &tagY);
				if (tagY) {
					DWORD* pvalue = (DWORD*)FreeImage_GetTagValue(tagY);
					DPIY = int(float(pvalue[0]) / float(pvalue[1]) + 0.5);
				}
