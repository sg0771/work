...
mDNSlocal mDNSs32 GetNextScheduledEvent(const mDNS* const m)
{
    ...
    {
        if (e - m->SuppressResponses > 0){
            e = m->SuppressResponses;
        }
    }
    else
    {
    ...
    if (m->NextBLEServiceTime && (e - m->NextBLEServiceTime > 0)) {
        e = m->NextBLEServiceTime;
    }
...
mDNSexport void mDNS_Unlock_(mDNS *const m, const char *const functionName, const mDNSu32 lineNumber)
{
    ...
    if (m->mDNS_busy == 0)
    {
        m->NextScheduledEvent = GetNextScheduledEvent(m);//预计下次的执行时间？
