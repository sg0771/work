...
RunDirectMain(LPVOID inParam)
{
	...
	RunDirect(0, NULL);
	SetEvent(gDNSThreadQuitEvent);//结束消息
	_endthreadex(0);
	return 0;
}
...
extern BOOL DNSSD_API IsBonjourServiceDisabled();
static int s_mDNS_init = 0;
int DNSSD_API DNSServiceStart() {
	if (s_mDNS_init != 0) {
		return kmDNSRunngingErr;
	}
	s_mDNS_init = 1;
	if (!IsBonjourServiceDisabled()) { //DNS服务是否安装或者运行
		return kBonjourRunngingErr;
	}

	BOOL DNSSD_API IsBonjourServiceDisabled();
	OSStatus err = kNoErr;
...
void DNSSD_API DNSServiceStop()
{
	...
	SetEvent(gDNSThreadStopEvent); //结束服务线程

	if (WaitForSingleObject(gDNSThreadQuitEvent, 5 * 1000) == WAIT_OBJECT_0) //等待退出消息
	{
		if (gDNSThreadQuitEvent)
		{
...
static OSStatus	ServiceSpecificRun(int argc, LPTSTR argv[])
{
	...
	{
#ifdef _WIN32
		//DWORD ret = WaitForSingleObjectEx(gDNSThreadStopEvent, 0, FALSE);
		DWORD ret = WaitForSingleObjectEx(gDNSThreadStopEvent, 0, FALSE); //5s执行一次，Edit By Tam
		if (ret != WAIT_TIMEOUT)
		{
		...
		nextTimerEvent = 1;//Edit By Tam

		err = mDNSPoll(nextTimerEvent);//RecvFrom
