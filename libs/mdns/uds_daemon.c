...
static dnssd_sock_t s_listenfd = dnssd_InvalidSocket;

static request_state *s_all_requests = NULL;  //搜索对象,操作加锁
...
{
    request_state **p = &s_all_requests;
    abort_request(req);
    while (*p && *p != req) 
        p=&(*p)->next;
...
mDNSlocal void connection_termination(request_state *request)
{
    ...
    request_state **req = &s_all_requests;
...
mDNSlocal void udsserver_default_reg_domain_changed(const DNameListElem *const d, const mDNSBool add)
{
    ...
    for (request = s_all_requests; request; request = request->next)
    {
...
{
    request_state *request;
    request_state **p = &s_all_requests; //增加新的查詢
...
mDNSexport int udsserver_init(dnssd_sock_t skts[], const size_t count)
{
    ...
    {
        s_listenfd = socket(AF_DNSSD, SOCK_STREAM, 0);
        if (!dnssd_SocketValid(s_listenfd))
        {
        ...
        #if defined(_WIN32)
        {
            ...
            ret = bind(s_listenfd, (struct sockaddr *) &laddr, sizeof(laddr));
            if (ret < 0)
            {
            ...
            ret = getsockname(s_listenfd, (struct sockaddr*)&laddr, &len);
            if (ret < 0)
            {
        ...
        if (!uds_socket_setup(s_listenfd)) goto error;
    }
...
mDNSexport int udsserver_exit(void)
{
    ...
    while (s_all_requests) 
        AbortUnlinkAndFree(s_all_requests);
...
mDNSexport mDNSs32 udsserver_idle(mDNSs32 nextevent)
{
    ...
    request_state **req = &s_all_requests; //查询操作
