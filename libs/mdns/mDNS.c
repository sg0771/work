...
mDNSexport void SetNextQueryTime(mDNS *const m, const DNSQuestion *const q)
{
    ...
    if (ActiveQuestion(q))
    {
        ...
        if (mDNSOpaque16IsZero(q->TargetQID)) {
            mDNSs32* const timer = &m->NextScheduledQuery;
            if (*timer -  (q->LastQTime + q->ThisQInterval) > 0)
                *timer = (q->LastQTime + q->ThisQInterval);
        } else {
            mDNSs32* const timer = &m->NextuDNSEvent;
    ...
    while (intf)
    {
        ...
        if (m->omsg.h.numAnswers || m->omsg.h.numAdditionals)
        {
            ...
            if (intf->IPv4Available) {
                mDNSSendDNSMessage(m, &m->omsg, responseptr, intf->InterfaceID, mDNSNULL, mDNSNULL, &AllDNSLinkGroup_v4, MulticastDNSPort, mDNSNULL, mDNSfalse);
            }
            if (intf->IPv6Available) {
                mDNSSendDNSMessage(m, &m->omsg, responseptr, intf->InterfaceID, mDNSNULL, mDNSNULL, &AllDNSLinkGroup_v6, MulticastDNSPort, mDNSNULL, mDNSfalse);
            }
...
#define CacheCheckGracePeriod(CR) (                                                   \
        ((CR)->CRActiveQuestion == mDNSNULL            ) ? (5 * mDNSPlatformOneSecond) : \
        ((CR)->UnansweredQueries < MaxUnansweredQueries) ? (TicksTTL(CR)/100)            : \
        ((CR)->resrec.rroriginalttl > 10               ) ? (mDNSPlatformOneSecond/2)      : \
        ((CR)->resrec.rroriginalttl > 0                ) ? (mDNSPlatformOneSecond/20)   : 0)
...
mDNSlocal void SendQueries(mDNS *const m)
{
    ...
    m->NextScheduledQuery = m->timenow + FutureTime; //下一次发送的时间
    ...
    while (intf)
    {
        ...
        if (queryptr > m->omsg.data)
        {
            ...
            if (intf->IPv4Available) {
                mDNSSendDNSMessage(m, &m->omsg, queryptr, intf->InterfaceID, mDNSNULL, mDNSNULL, &AllDNSLinkGroup_v4, MulticastDNSPort, mDNSNULL, useBackgroundTrafficClass);
            }
            if (intf->IPv6Available) {
                mDNSSendDNSMessage(m, &m->omsg, queryptr, intf->InterfaceID, mDNSNULL, mDNSNULL, &AllDNSLinkGroup_v6, MulticastDNSPort, mDNSNULL, useBackgroundTrafficClass);
            }
...
mDNSlocal void CacheRecordRmv(mDNS *const m, CacheRecord *cr)
{
    ...
    while (m->CurrentQuestion && m->CurrentQuestion != m->NewQuestions)
    {
        ...
        if (!q->Suppressed && CacheRecordAnswersQuestion(cr, q))
        {
            ...
            mDNSBool generateRemoveEvents = (cr->resrec.rdata->MaxRDLength > 0);//rmv事件
        #if MDNSRESPONDER_SUPPORTS(APPLE, DNSSECv2)
...
mDNSexport mDNSs32 mDNS_Execute(mDNS *const m)
{
    ...
    if (m->timenow - m->NextScheduledEvent >= 0)
    {
        ...
        if (m->rrcache_size && m->timenow - m->NextCacheCheck >= 0)
        {
            ...
            for (slot = 0; slot < CACHE_HASH_SLOTS; slot++)
            {
                if (m->timenow - m->rrcache_nextcheck[slot] >= 0)  //查詢時間
                {
                    CacheGroup **cp = &m->rrcache_hash[slot];
                ...
                if (m->timenow - r2->TimeRcvd < mDNSPlatformOneSecond && RRExpireTime(r2) - m->timenow > mDNSPlatformOneSecond)
                {
                    ...
                    if (r2->resrec.rroriginalttl != r1->resrec.rroriginalttl && r1->resrec.rroriginalttl > 1)
                    {
                        if (!(r2->resrec.rroriginalttl == 240 && r1->resrec.rroriginalttl == 60 && r2->resrec.rrtype == kDNSType_TXT) &&
                            !(r2->resrec.rroriginalttl == 120 && r1->resrec.rroriginalttl == mDNSMaximumMulticastTTLSeconds && r2->resrec.rrtype == kDNSType_SRV) &&
                            ResponseIsMDNS)
                                     ...
                                     const mDNSInterfaceID InterfaceID)
{
    ...
    if (p) {
        mDNSSendDNSMessage(m, &m->omsg, p, InterfaceID, mDNSNULL, m->SPSSocket, srcaddr, srcport, mDNSNULL, mDNSfalse);
    }
    mDNS_SendKeepalives(m);
}
                                ...
                                const mDNSInterfaceID InterfaceID)
{
    ...
    if (QR_OP == StdQ) {
        ...
        mDNSCoreReceiveResponse(m, msg, end, srcaddr, srcport, dstaddr, dstport, mDNSNULL, mDNSNULL, ifid);
    }
#else
    else if (QR_OP == StdR) {
