...
// delay after restarting LLQ before nuking previous known answers (avoids flutter if we restart before we have networking up)
#define RESTART_GOODBYE_DELAY    (6 * mDNSPlatformOneSecond) 

// this interval is used after send failures on network transitions
// which typically heal quickly, so we start agressively and exponentially back off
#define INIT_UCAST_POLL_INTERVAL (3 * mDNSPlatformOneSecond) 
...
#define QuestionIntervalStep 1 // 时间间隔递增倍数，默认为3，也就是1s 3s 9s 27s 这样递增
...
#define MIN_UPDATE_REFRESH_TIME     (5 * 60 * mDNSPlatformOneSecond)  //5分钟
