...
static const PREDICTION_MODE intra_mode_list[] = { VP9_DC_PRED, VP9_V_PRED,
                                                   VP9_H_PRED,
                                                   VP9_TM_PRED };

static int mode_offset(const PREDICTION_MODE mode) {
  if (mode >= VP9_NEARESTMV) {
    return INTER_OFFSET(mode);
  } else {
    switch (mode) {
      case VP9_DC_PRED: return 0;
      case VP9_V_PRED: return 1;
      case VP9_H_PRED: return 2;
      case VP9_TM_PRED: return 3;
      default: return -1;
    }
  }
}
...
static INLINE void update_thresh_freq_fact(
  ...
  else if (cpi->sf.limit_newmv_early_exit && mode == VP9_NEWMV &&
...
void vp9_pick_intra_mode(VP9_COMP *cpi, MACROBLOCK *x, RD_COST *rd_cost,
  ...
  struct estimate_block_intra_args args = { cpi, x, VP9_DC_PRED, 1, 0 };
  ...
  // mode tests.
  for (this_mode = VP9_DC_PRED; this_mode <= VP9_H_PRED; ++this_mode) {
...
static const REF_MODE ref_mode_set[RT_INTER_MODES] = {
  { LAST_FRAME, VP9_ZEROMV },   { LAST_FRAME, VP9_NEARESTMV },
  { GOLDEN_FRAME, VP9_ZEROMV }, { LAST_FRAME, VP9_NEARMV },
  { LAST_FRAME, VP9_NEWMV },    { GOLDEN_FRAME, VP9_NEARESTMV },
  { GOLDEN_FRAME, VP9_NEARMV }, { GOLDEN_FRAME, VP9_NEWMV },
  { ALTREF_FRAME, VP9_ZEROMV }, { ALTREF_FRAME, VP9_NEARESTMV },
  { ALTREF_FRAME, VP9_NEARMV }, { ALTREF_FRAME, VP9_NEWMV }
};
static const REF_MODE ref_mode_set_svc[RT_INTER_MODES] = {
  { LAST_FRAME, VP9_ZEROMV },      { GOLDEN_FRAME, VP9_ZEROMV },
  { LAST_FRAME, VP9_NEARESTMV },   { LAST_FRAME, VP9_NEARMV },
  { GOLDEN_FRAME, VP9_NEARESTMV }, { GOLDEN_FRAME, VP9_NEARMV },
  { LAST_FRAME, VP9_NEWMV },   { GOLDEN_FRAME, VP9_NEWMV }
};
  ...
  if ((cpi->ref_frame_flags & flag_list[ref_frame]) && (yv12 != NULL)) {
    ...
    if (cm->use_prev_frame_mvs) {
      ...
      const_motion[ref_frame] =
          mv_refs_rt(cpi, cm, x, xd, tile_info, xd->mi[0], ref_frame, candidates,
          &frame_mv[VP9_NEWMV][ref_frame], mi_row, mi_col,
                     (int)(cpi->svc.use_base_mv && cpi->svc.spatial_layer_id));
    }
    vp9_find_best_ref_mvs(xd, cm->allow_high_precision_mv, candidates,
                          &frame_mv[VP9_NEARESTMV][ref_frame],
                          &frame_mv[VP9_NEARMV][ref_frame]);
...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  MODE_INFO *const mi = xd->mi[0];
  struct macroblockd_plane *const pd = &xd->plane[0];
  PREDICTION_MODE best_mode = VP9_ZEROMV;
  ...
  for (idx = 0; idx < RT_INTER_MODES; ++idx) {
    ...
    if (const_motion[ref_frame] && this_mode == VP9_NEARMV) continue;
    ...
    if (cpi->sf.short_circuit_low_temp_var >= 2 && force_skip_low_temp_var &&
        ref_frame == LAST_FRAME && this_mode == VP9_NEWMV) {
      continue;
    }
    ...
    if (this_mode == VP9_NEWMV) {
      if (ref_frame > LAST_FRAME && !cpi->use_svc &&
        ...
        frame_mv[VP9_NEWMV][ref_frame].as_int = mi->mv[0].as_int;
        rate_mv = vp9_mv_bit_cost(&frame_mv[VP9_NEWMV][ref_frame].as_mv,
                                  &x->mbmi_ext->ref_mvs[ref_frame][0].as_mv,
                                  x->nmvjointcost, x->mvcost, MV_COST_WEIGHT);
        frame_mv[VP9_NEWMV][ref_frame].as_mv.row >>= 3;
        frame_mv[VP9_NEWMV][ref_frame].as_mv.col >>= 3;

        cpi->find_fractional_mv_step(
            x, &frame_mv[VP9_NEWMV][ref_frame].as_mv,
            &x->mbmi_ext->ref_mvs[ref_frame][0].as_mv,
    ...
    if (use_golden_nonzeromv && this_mode == VP9_NEWMV &&
        ref_frame == LAST_FRAME &&
        frame_mv[VP9_NEWMV][LAST_FRAME].as_int != INVALID_MV) {
      const int pre_stride = xd->plane[0].pre[0].stride;
      const uint8_t *const pre_buf =
          xd->plane[0].pre[0].buf +
          (frame_mv[VP9_NEWMV][LAST_FRAME].as_mv.row >> 3) * pre_stride +
          (frame_mv[VP9_NEWMV][LAST_FRAME].as_mv.col >> 3);
      best_pred_sad = cpi->fn_ptr[bsize].sdf(
          x->plane[0].src.buf, x->plane[0].src.stride, pre_buf, pre_stride);
      x->pred_mv_sad[LAST_FRAME] = best_pred_sad;
    }

    if (this_mode != VP9_NEARESTMV &&
        frame_mv[this_mode][ref_frame].as_int ==
            frame_mv[VP9_NEARESTMV][ref_frame].as_int)
      continue;
    ...
    if ((this_mode == VP9_NEWMV || filter_ref == SWITCHABLE) &&
        pred_filter_search &&
        (ref_frame == LAST_FRAME ||
         (ref_frame == GOLDEN_FRAME &&
  ...
  for (ref_frame = LAST_FRAME; ref_frame <= GOLDEN_FRAME; ++ref_frame) {
    ...
    for (idy = 0; idy < 2; idy += num_4x4_blocks_high) {
      for (idx = 0; idx < 2; idx += num_4x4_blocks_wide) {
        ...
        b_mv[VP9_ZEROMV].as_int = 0;
        b_mv[VP9_NEWMV].as_int = INVALID_MV;
        vp9_append_sub8x8_mvs_for_idx(cm, xd, i, 0, mi_row, mi_col,
                                      &b_mv[VP9_NEARESTMV], &b_mv[VP9_NEARMV],
                                      mbmi_ext->mode_context);
        ...
        for (this_mode = VP9_NEARESTMV; this_mode <= VP9_NEWMV; ++this_mode) {
          ...
          if (this_mode == VP9_NEWMV) {
            ...
            if (i == 0) {
              mvp_full.row = b_mv[VP9_NEARESTMV].as_mv.row >> 3;
              mvp_full.col = b_mv[VP9_NEARESTMV].as_mv.col >> 3;
            } else {
