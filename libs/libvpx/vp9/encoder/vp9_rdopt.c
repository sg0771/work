...
static const MODE_DEFINITION vp9_mode_order[MAX_MODES] = {
  { VP9_NEARESTMV, { LAST_FRAME, NONE } },
  { VP9_NEARESTMV, { ALTREF_FRAME, NONE } },
  { VP9_NEARESTMV, { GOLDEN_FRAME, NONE } },

  { VP9_DC_PRED, { INTRA_FRAME, NONE } },

  { VP9_NEWMV, { LAST_FRAME, NONE } },
  { VP9_NEWMV, { ALTREF_FRAME, NONE } },
  { VP9_NEWMV, { GOLDEN_FRAME, NONE } },

  { VP9_NEARMV, { LAST_FRAME, NONE } },
  { VP9_NEARMV, { ALTREF_FRAME, NONE } },
  { VP9_NEARMV, { GOLDEN_FRAME, NONE } },

  { VP9_ZEROMV, { LAST_FRAME, NONE } },
  { VP9_ZEROMV, { GOLDEN_FRAME, NONE } },
  { VP9_ZEROMV, { ALTREF_FRAME, NONE } },

  { VP9_NEARESTMV, { LAST_FRAME, ALTREF_FRAME } },
  { VP9_NEARESTMV, { GOLDEN_FRAME, ALTREF_FRAME } },

  { VP9_TM_PRED, { INTRA_FRAME, NONE } },

  { VP9_NEARMV, { LAST_FRAME, ALTREF_FRAME } },
  { VP9_NEWMV, { LAST_FRAME, ALTREF_FRAME } },
  { VP9_NEARMV, { GOLDEN_FRAME, ALTREF_FRAME } },
  { VP9_NEWMV, { GOLDEN_FRAME, ALTREF_FRAME } },

  { VP9_ZEROMV, { LAST_FRAME, ALTREF_FRAME } },
  { VP9_ZEROMV, { GOLDEN_FRAME, ALTREF_FRAME } },

  { VP9_H_PRED, { INTRA_FRAME, NONE } },
  { VP9_V_PRED, { INTRA_FRAME, NONE } },
  { VP9_D135_PRED, { INTRA_FRAME, NONE } },
  { VP9_D207_PRED, { INTRA_FRAME, NONE } },
  { VP9_D153_PRED, { INTRA_FRAME, NONE } },
  { VP9_D63_PRED, { INTRA_FRAME, NONE } },
  { VP9_D117_PRED, { INTRA_FRAME, NONE } },
  { VP9_D45_PRED, { INTRA_FRAME, NONE } },
};
...
static int conditional_skipintra(PREDICTION_MODE mode,
  ...
  if (mode == VP9_D117_PRED && best_intra_mode != VP9_V_PRED &&
      best_intra_mode != VP9_D135_PRED)
    return 1;
  if (mode == VP9_D63_PRED && best_intra_mode != VP9_V_PRED &&
      best_intra_mode != VP9_D45_PRED)
    return 1;
  if (mode == VP9_D207_PRED && best_intra_mode != VP9_H_PRED &&
      best_intra_mode != VP9_D45_PRED)
    return 1;
  if (mode == VP9_D153_PRED && best_intra_mode != VP9_H_PRED &&
      best_intra_mode != VP9_D135_PRED)
    return 1;
  return 0;
}
...
#if CONFIG_VP9_HIGHBITDEPTH
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    for (mode = VP9_DC_PRED; mode <= VP9_TM_PRED; ++mode) {
      int64_t this_rd;
      int ratey = 0;
        ...
        for (idx = 0; idx < num_4x4_blocks_wide; ++idx) {
          ...
          if (xd->lossless) {
            ...
            const TX_TYPE tx_type =
                get_tx_type_4x4(VP9_PLANE_TYPE_Y, xd, block);
    ...
    for (idy = 0; idy < num_4x4_blocks_high; ++idy) {
      for (idx = 0; idx < num_4x4_blocks_wide; ++idx) {
        ...
        if (xd->lossless) {
          ...
          int64_t unused;
          const TX_TYPE tx_type = get_tx_type_4x4(VP9_PLANE_TYPE_Y, xd, block);
          const scan_order *so = &vp9_scan_orders[TX_4X4][tx_type];
          const int coeff_ctx =
              combine_entropy_contexts(tempa[idx], templ[idy]);
...
static int64_t rd_pick_intra_sub_8x8_y_mode(VP9_COMP *cpi, MACROBLOCK *mb,
  ...
  for (idy = 0; idy < 2; idy += num_4x4_blocks_high) {
    for (idx = 0; idx < 2; idx += num_4x4_blocks_wide) {
      PREDICTION_MODE best_mode = VP9_DC_PRED;
...
static int64_t rd_pick_intra_sby_mode(VP9_COMP *cpi, MACROBLOCK *x, int *rate,
                                      ...
                                      int64_t best_rd) {
  PREDICTION_MODE mode;
  PREDICTION_MODE mode_selected = VP9_DC_PRED;
  MACROBLOCKD *const xd = &x->e_mbd;
  MODE_INFO *const mic = xd->mi[0];
...
static int64_t rd_sbuv_dcpred(const VP9_COMP *cpi, MACROBLOCK *x, int *rate,
  ...
  x->e_mbd.mi[0]->uv_mode = VP9_DC_PRED;
  memset(x->skip_txfm, SKIP_TXFM_NONE, sizeof(x->skip_txfm));
  super_block_uvrd(cpi, x, rate_tokenonly, distortion, skippable, &unused,
                   bsize, INT64_MAX);
  *rate =
      *rate_tokenonly +
          cpi->intra_uv_mode_cost[cm->frame_type][x->e_mbd.mi[0]->mode]
                                 [VP9_DC_PRED];
...
static int set_and_cost_bmi_mvs(VP9_COMP *cpi, MACROBLOCK *x, MACROBLOCKD *xd,
                                ...
                                int_mv frame_mv[VP9_MB_MODE_COUNT][MAX_REF_FRAMES],
...
static int check_best_zero_mv(const VP9_COMP *cpi,
  ...
  if ((this_mode == VP9_NEARMV || this_mode == VP9_NEARESTMV ||
       this_mode == VP9_ZEROMV) &&
      frame_mv[this_mode][ref_frames[0]].as_int == 0 &&
      (ref_frames[1] == NONE ||
       frame_mv[this_mode][ref_frames[1]].as_int == 0)) {
    int rfc = mode_context[ref_frames[0]];
    int c1 = cost_mv_ref(cpi, VP9_NEARMV, rfc);
    int c2 = cost_mv_ref(cpi, VP9_NEARESTMV, rfc);
    int c3 = cost_mv_ref(cpi, VP9_ZEROMV, rfc);

    if (this_mode == VP9_NEARMV) {
      if (c1 > c3) return 0;
    } else if (this_mode == VP9_NEARESTMV) {
      if (c2 > c3) return 0;
    } else {
      assert(this_mode == VP9_ZEROMV);
      if (ref_frames[1] == NONE) {
        if ((c3 >= c2 && frame_mv[VP9_NEARESTMV][ref_frames[0]].as_int == 0) ||
            (c3 >= c1 && frame_mv[VP9_NEARMV][ref_frames[0]].as_int == 0))
          return 0;
      } else {
        if ((c3 >= c2 && frame_mv[VP9_NEARESTMV][ref_frames[0]].as_int == 0 &&
             frame_mv[VP9_NEARESTMV][ref_frames[1]].as_int == 0) ||
            (c3 >= c1 && frame_mv[VP9_NEARMV][ref_frames[0]].as_int == 0 &&
             frame_mv[VP9_NEARMV][ref_frames[1]].as_int == 0))
          return 0;
      }
    }
  }
...
static int64_t rd_pick_best_sub8x8_mode(
  ...
  for (i = 0; i < 4; i++) bsi->modes[i] = VP9_ZEROMV;
  ...
  for (idy = 0; idy < 2; idy += num_4x4_blocks_high) {
    for (idx = 0; idx < 2; idx += num_4x4_blocks_wide) {
      ...
      int_mv mode_mv[VP9_MB_MODE_COUNT][2];
      int_mv frame_mv[VP9_MB_MODE_COUNT][MAX_REF_FRAMES];
      PREDICTION_MODE mode_selected = VP9_ZEROMV;
      int64_t best_rd = INT64_MAX;
      ...
      for (ref = 0; ref < 1 + has_second_rf; ++ref) {
        ...
        frame_mv[VP9_ZEROMV][frame].as_int = 0;
        vp9_append_sub8x8_mvs_for_idx(
            cm, xd, i, ref, mi_row, mi_col, &frame_mv[VP9_NEARESTMV][frame],
            &frame_mv[VP9_NEARMV][frame], mbmi_ext->mode_context);
      }

      // search for the best motion vector on this segment
      for (this_mode = VP9_NEARESTMV; this_mode <= VP9_NEWMV; ++this_mode) {
        ...
        if (!has_second_rf && this_mode == VP9_NEWMV &&
            seg_mvs[i][mi->ref_frame[0]].as_int == INVALID_MV) {
          MV *const new_mv = &mode_mv[VP9_NEWMV][0].as_mv;
          int step_param = 0;
          uint32_t bestsme = UINT_MAX;
...
static int discount_newmv_test(const VP9_COMP *cpi, int this_mode,
                               ...
                               int ref_frame) {
  return (!cpi->rc.is_src_frame_alt_ref && (this_mode == VP9_NEWMV) &&
          (this_mv.as_int != 0) &&
          ((mode_mv[VP9_NEARESTMV][ref_frame].as_int == 0) ||
           (mode_mv[VP9_NEARESTMV][ref_frame].as_int == INVALID_MV)) &&
          ((mode_mv[VP9_NEARMV][ref_frame].as_int == 0) ||
           (mode_mv[VP9_NEARMV][ref_frame].as_int == INVALID_MV)));
}
...
static int64_t handle_inter_mode(
  ...
  if (pred_filter_search) {
    ...
    if ((this_mode != VP9_NEWMV) || (af == lf)) best_filter = af;
  }
  ...
  for (i = 0; i < is_comp_pred + 1; ++i) {
    ...
    if (this_mode != VP9_NEWMV) clamp_mv2(&cur_mv[i].as_mv, xd);
  ...
  if (discount_newmv_test(cpi, this_mode, frame_mv[refs[0]], mode_mv,
    ...
    *rate2 +=
        ...
        cost_mv_ref(cpi, VP9_NEARESTMV, mbmi_ext->mode_context[refs[0]]));
  } else {
    *rate2 += cost_mv_ref(cpi, this_mode, mbmi_ext->mode_context[refs[0]]);
  }

  if (RDCOST(x->rdmult, x->rddiv, *rate2, 0) > ref_best_rd &&
      mi->mode != VP9_NEARESTMV)
    return INT64_MAX;
...
void vp9_rd_pick_inter_mode_sb(VP9_COMP *cpi, TileDataEnc *tile_data,
  ...
  INTERP_FILTER single_inter_filter[VP9_MB_MODE_COUNT][MAX_REF_FRAMES];
  int single_skippable[VP9_MB_MODE_COUNT][MAX_REF_FRAMES];
  ...
  PREDICTION_MODE best_intra_mode = VP9_DC_PRED;
  ...
  for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
    ...
    if (cpi->ref_frame_flags & flag_list[ref_frame]) {
      ...
      setup_buffer_inter(cpi, x, ref_frame, bsize, mi_row, mi_col,
                         frame_mv[VP9_NEARESTMV], frame_mv[VP9_NEARMV],
                         yv12_mb);
    }
    frame_mv[VP9_NEWMV][ref_frame].as_int = INVALID_MV;
    frame_mv[VP9_ZEROMV][ref_frame].as_int = 0;
  }
  ...
  if (!segfeature_active(seg, segment_id, SEG_LVL_REF_FRAME)) {
    ...
    if (cpi->rc.is_src_frame_alt_ref && (cpi->oxcf.arnr_max_frames == 0)) {
      ...
      if (frame_mv[VP9_NEARMV][ALTREF_FRAME].as_int != 0)
        mode_skip_mask[ALTREF_FRAME] |= (1 << VP9_NEARMV);
      if (frame_mv[VP9_NEARESTMV][ALTREF_FRAME].as_int != 0)
        mode_skip_mask[ALTREF_FRAME] |= (1 << VP9_NEARESTMV);
    }
  }
  ...
  for (midx = 0; midx < MAX_MODES; ++midx) {
    ...
    if (sf->motion_field_mode_search) {
      ...
      if (skip_ref_frame && this_mode != VP9_NEARESTMV &&
          this_mode != VP9_NEWMV)
        if (rf > INTRA_FRAME)
          if (ref_frame != rf) continue;

      if (const_motion)
        if (this_mode == VP9_NEARMV || this_mode == VP9_ZEROMV) continue;
    }
    ...
    if (ref_frame == INTRA_FRAME) {
      ...
      if (this_mode != VP9_DC_PRED) {
        ...
        if ((mode_search_skip_flags & FLAG_SKIP_INTRA_BESTINTER) &&
            (this_mode >= VP9_D45_PRED && this_mode <= VP9_TM_PRED)) {
    ...
    if (ref_frame == INTRA_FRAME) {
      ...
      if (this_mode != VP9_DC_PRED && this_mode != VP9_TM_PRED)
  ...
  // are corrected.
  if (best_mbmode.mode == VP9_NEWMV) {
    const MV_REFERENCE_FRAME refs[2] = { best_mbmode.ref_frame[0],
                                         best_mbmode.ref_frame[1] };
    int comp_pred_mode = refs[1] > INTRA_FRAME;

    if (frame_mv[VP9_NEARESTMV][refs[0]].as_int == best_mbmode.mv[0].as_int &&
        ((comp_pred_mode && frame_mv[VP9_NEARESTMV][refs[1]].as_int ==
                                best_mbmode.mv[1].as_int) ||
         !comp_pred_mode))
      best_mbmode.mode = VP9_NEARESTMV;
    else if (frame_mv[VP9_NEARMV][refs[0]].as_int == best_mbmode.mv[0].as_int &&
             ((comp_pred_mode && frame_mv[VP9_NEARMV][refs[1]].as_int ==
                                     best_mbmode.mv[1].as_int) ||
              !comp_pred_mode))
      best_mbmode.mode = VP9_NEARMV;
    else if (best_mbmode.mv[0].as_int == 0 &&
             ((comp_pred_mode && best_mbmode.mv[1].as_int == 0) ||
              !comp_pred_mode))
      best_mbmode.mode = VP9_ZEROMV;
  }
...
void vp9_rd_pick_inter_mode_sub8x8(VP9_COMP *cpi, TileDataEnc *tile_data,
  ...
  int64_t dist_uv;
  int skip_uv;
  PREDICTION_MODE mode_uv = VP9_DC_PRED;
  const int intra_cost_penalty = vp9_get_intra_cost_penalty(
      cm->base_qindex, cm->y_dc_delta_q, cm->bit_depth);
