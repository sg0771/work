...
JOCTET *jsimd_huff_encode_one_block_neon(void *state, JOCTET *buffer,
                                         ...
                                         c_derived_tbl *actbl)
{
    uint8_t* ppSrc = (uint8_t*)jsimd_huff_encode_one_block_consts;
  ...
  const uint8x16x4_t idx_rows_0123 =
    vld1q_u8_x4(ppSrc + 0 * DCTSIZE);
  const uint8x16x4_t idx_rows_4567 =
    vld1q_u8_x4(ppSrc + 8 * DCTSIZE);
#else
  /* GCC does not currently support intrinsics vl1dq_<type>_x4(). */
  const uint8x16x4_t idx_rows_0123 = { {
    vld1q_u8(ppSrc + 0 * DCTSIZE),
    vld1q_u8(ppSrc + 2 * DCTSIZE),
    vld1q_u8(ppSrc + 4 * DCTSIZE),
    vld1q_u8(ppSrc + 6 * DCTSIZE)
  } };
  const uint8x16x4_t idx_rows_4567 = { {
    vld1q_u8(ppSrc + 8 * DCTSIZE),
    vld1q_u8(ppSrc + 10 * DCTSIZE),
    vld1q_u8(ppSrc + 12 * DCTSIZE),
    vld1q_u8(ppSrc + 14 * DCTSIZE)
  } };
#endif
