...
{
#if HAVE_THREAD
#if defined(__GNUC__) && (__GNUC__ > 4 || __GNUC__ == 4 && __GNUC_MINOR__ > 0) && (ARCH_X86|| ARCH_X64)
...
uint32_t x264_endian_fix32(uint32_t x); //Add In common/x86/

#elif defined(__GNUC__) && HAVE_ARMV6
static ALWAYS_INLINE uint32_t x264_endian_fix32( uint32_t x )
{
    asm("rev %0, %0":"+r"(x));
    return x;
}
...
#if ARCH_X64
uint64_t x264_endian_fix64(uint64_t x); //Add In common/x86/
#else
static ALWAYS_INLINE uint64_t x264_endian_fix64( uint64_t x )
{
    return x264_endian_fix32(x>>32) + ((uint64_t)x264_endian_fix32(x)<<32);
}
#endif

static ALWAYS_INLINE intptr_t endian_fix( intptr_t x )
{
    return WORD_SIZE == 8 ? x264_endian_fix64(x) : x264_endian_fix32(x);
}
...
#if HAVE_MMX
void x264_prefetch(void *p);
#elif defined(__GNUC__) && (__GNUC__ > 3 || __GNUC__ == 3 && __GNUC_MINOR__ > 1) &&\
      (ARCH_X86|| ARCH_X64 || ARCH_ARM)
