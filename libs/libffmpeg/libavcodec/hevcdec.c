...
static int hevc_decode_extradata(HEVCContext *s, uint8_t *buf, int length, int first)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(s->ps.sps_list); i++) {
        if (first && s->ps.sps_list[i]) {
            ...
            export_stream_params(s, sps);
			set_sps(s, sps, sps->pix_fmt);//fix  for HWDecoder
            break;
        }
    }
...
static int hls_slice_header(HEVCContext* s);
static enum AVPixelFormat get_hevc_pixel_format(HEVCContext* h, int force_callback)
{
    if (h->ps.sps == NULL) {
        return AV_PIX_FMT_NONE;//Not Extradata, sps pps vps
    }
#define HWACCEL_HEVC_MAX (CONFIG_HEVC_DXVA2_HWACCEL + \
                     (CONFIG_HEVC_D3D11VA_HWACCEL * 2) + \
                     CONFIG_HEVC_NVDEC_HWACCEL + \
                     CONFIG_HEVC_VAAPI_HWACCEL + \
                     CONFIG_HEVC_VIDEOTOOLBOX_HWACCEL + \
                     CONFIG_HEVC_VDPAU_HWACCEL)
    enum AVPixelFormat pix_fmts[HWACCEL_HEVC_MAX + 2], * fmt = pix_fmts;
    const enum AVPixelFormat* choices = pix_fmts;
    int i;

    switch (h->ps.sps->bit_depth_chroma) {
    case 9:
        if (CHROMA444(h)) {
    ...
    *fmt = AV_PIX_FMT_NONE;
    ...
    for (i = 0; choices[i] != AV_PIX_FMT_NONE; i++)
        if (choices[i] == h->avctx->pix_fmt && !force_callback)
            return choices[i];
    enum AVPixelFormat hevc_fmt = ff_thread_get_format(h->avctx, choices);
    return hevc_fmt;
}
//Add By TamXie
static enum AVPixelFormat ff_hevc_get_pixel_format(HEVCContext* h)
{
    return get_hevc_pixel_format(h, 1);
}
...
static av_cold int hevc_decode_init(AVCodecContext *avctx)
{
    ...
    if((avctx->active_thread_type & FF_THREAD_FRAME) && avctx->thread_count > 1)
      ...
      //Add By TamXie
        //hw_device_ctx 由使用者配置
        //使用硬解码时，像素格式由硬解码器决定
        //需要设置extradata
    if (avctx->extradata_size) {
        //通过extradata设置sps pps
        //如果extradata无效，继续
        avctx->pix_fmt = ff_hevc_get_pixel_format(s);
        if (avctx->pix_fmt == AV_PIX_FMT_NONE) // ff_hevc_get_pixel_format will log an error
            avctx->pix_fmt = avctx->sw_pix_fmt;// fallback to sw_pix_fmt
    }
    if (avctx->pix_fmt == AV_PIX_FMT_NONE) // ff_hevc_get_pixel_format will log an error
        avctx->pix_fmt = avctx->sw_pix_fmt;// fallback to sw_pix_fmt
    return 0;
