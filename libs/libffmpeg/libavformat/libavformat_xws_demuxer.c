...
typedef struct WTSFilter WTSFilter;

typedef int XPESCallback (WTSFilter *f, const uint8_t *buf, int len,
                         int is_start, int64_t pos);

typedef struct WTSPESFilter {
    XPESCallback *pes_cb;
    void *opaque;
} WTSPESFilter;

typedef void SectionCallback (WTSFilter *f, const uint8_t *buf, int len);

typedef void SetServiceCallback (void *opaque, int ret);

typedef struct WTSSectionFilter {
    int section_index;
    int section_h_size;
    int last_ver;
    unsigned crc;
...
struct WTSFilter {
    ...
    enum WTSFilterType type;
    union {
        WTSPESFilter pes_filter;
        WTSSectionFilter section_filter;
    } u;
};
...
typedef struct WTSContext WTSContext;

#define MPEGTS_OPTIONS \
    { "resync_size",   "set size limit for looking up a new synchronization", offsetof(WTSContext, resync_size), AV_OPT_TYPE_INT,  { .i64 =  MAX_RESYNC_SIZE}, 0, INT_MAX,  AV_OPT_FLAG_DECODING_PARAM }

static const AVOption xoptions[] = {
    MPEGTS_OPTIONS,
    {"fix_teletext_pts", "try to fix pts values of dvb teletext streams", offsetof(WTSContext, fix_teletext_pts), AV_OPT_TYPE_BOOL,
     {.i64 = 1}, 0, 1, AV_OPT_FLAG_DECODING_PARAM },
    {"ts_packetsize", "output option carrying the raw packet size", offsetof(WTSContext, raw_packet_size), AV_OPT_TYPE_INT,
     {.i64 = 0}, 0, 0, AV_OPT_FLAG_DECODING_PARAM | AV_OPT_FLAG_EXPORT | AV_OPT_FLAG_READONLY },
    {"scan_all_pmts", "scan and combine all PMTs", offsetof(WTSContext, scan_all_pmts), AV_OPT_TYPE_BOOL,
     {.i64 = -1}, -1, 1, AV_OPT_FLAG_DECODING_PARAM },
    {"skip_unknown_pmt", "skip PMTs for programs not advertised in the PAT", offsetof(WTSContext, skip_unknown_pmt), AV_OPT_TYPE_BOOL,
     {.i64 = 0}, 0, 1, AV_OPT_FLAG_DECODING_PARAM },
    {"merge_pmt_versions", "re-use streams when PMT's version/pids change", offsetof(WTSContext, merge_pmt_versions), AV_OPT_TYPE_BOOL,
     {.i64 = 0}, 0, 1,  AV_OPT_FLAG_DECODING_PARAM },
    {"skip_changes", "skip changing / adding streams / programs", offsetof(WTSContext, skip_changes), AV_OPT_TYPE_BOOL,
     {.i64 = 0}, 0, 1, 0 },
    {"skip_clear", "skip clearing programs", offsetof(WTSContext, skip_clear), AV_OPT_TYPE_BOOL,
     {.i64 = 0}, 0, 1, 0 },
    { NULL },
};

static const AVClass xws_class = {
    .class_name = "wxs demuxer",
...
static struct XProgram * get_program(WTSContext *ts, unsigned int programid)
{
    int i;
...
static void clear_program(WTSContext *ts, unsigned int programid)
{
    int i;
...
static void clear_programs(WTSContext *ts)
{
    av_freep(&ts->prg);
    ts->nb_prg = 0;
}

static void add_pat_entry(WTSContext *ts, unsigned int programid)
{
    struct XProgram *p;
...
static void add_pid_to_pmt(WTSContext *ts, unsigned int programid,
                           unsigned int pid)
{
    struct XProgram *p = get_program(ts, programid);
    int i;
    if (!p)
        return;
...
static void set_pmt_found(WTSContext *ts, unsigned int programid)
{
    struct XProgram *p = get_program(ts, programid);
    if (!p)
        return;
 ...
 *         0 otherwise
 */
static int discard_pid(WTSContext *ts, unsigned int pid)
{
    int i, j, k;
 ...
 */
static void write_section_data(WTSContext *ts, WTSFilter *tss1,
                               const uint8_t *buf, int buf_size, int is_start)
{
    WTSSectionFilter *tss = &tss1->u.section_filter;
...
static WTSFilter *xws_open_filter(WTSContext *ts, unsigned int pid,
                                        enum WTSFilterType type)
{
    WTSFilter *filter;
    ...
    if (pid >= NB_PID_MAX || ts->pids[pid])
        return NULL;
    filter = av_mallocz(sizeof(WTSFilter));
    if (!filter)
        return NULL;
    ts->pids[pid] = filter;
...
static WTSFilter *xws_open_section_filter(WTSContext *ts,
                                                unsigned int pid,
                                                SectionCallback *section_cb,
                                                void *opaque,
                                                int check_crc)
{
    WTSFilter *filter;
    WTSSectionFilter *sec;

    if (!(filter = xws_open_filter(ts, pid, MPEGTS_SECTION)))
        return NULL;
...
static WTSFilter *ws_open_pes_filter(WTSContext *ts, unsigned int pid,
                                            XPESCallback *pes_cb,
                                            void *opaque)
{
    WTSFilter *filter;
    ...
    if (!(filter = xws_open_filter(ts, pid, MPEGTS_PES)))
        return NULL;
...
static WTSFilter *xws_open_pcr_filter(WTSContext *ts, unsigned int pid)
{
    return xws_open_filter(ts, pid, MPEGTS_PCR);
}

static void xws_close_filter(WTSContext *ts, WTSFilter *filter)
{
    int pid;
    ...
    if (filter->type == MPEGTS_SECTION)
        ...
        XPESContext *pes = filter->u.pes_filter.opaque;
...
static int xws_analyze(const uint8_t *buf, int size, int packet_size,
                   int probe)
{
...
static int get_packet_size(AVFormatContext* s)
{
    ...
    while (buf_size < PROBE_PACKET_MAX_BUF) {
        ...
        score      = xws_analyze(buf, buf_size, TS_PACKET_SIZE,      0);
        dvhs_score = xws_analyze(buf, buf_size, TS_DVHS_PACKET_SIZE, 0);
        fec_score  = xws_analyze(buf, buf_size, TS_FEC_PACKET_SIZE,  0);
...
static int skip_identical(const SectionHeader *h, WTSSectionFilter *tssf)
{
...
static void xws_find_stream_type(AVStream *st,
                                    uint32_t stream_type,
...
static int xws_set_stream_info(AVStream *st, XPESContext *pes,
                                  uint32_t stream_type, uint32_t prog_reg_desc)
{
    ...
    xws_find_stream_type(st, pes->stream_type, ISO_types);
    if (pes->stream_type == 4 || pes->stream_type == 0x0f)
        st->request_probe = 50;
    if ((prog_reg_desc == AV_RL32("HDMV") ||
         prog_reg_desc == AV_RL32("HDPR")) &&
        st->codecpar->codec_id == AV_CODEC_ID_NONE) {
        xws_find_stream_type(st, pes->stream_type, HDMV_types);
        if (pes->stream_type == 0x83) {
            // HDMV TrueHD streams also contain an AC3 coded version of the
            // audio track - add a second stream for this
            AVStream *sub_st;
            // priv_data cannot be shared between streams
            XPESContext *sub_pes = av_malloc(sizeof(*sub_pes));
            if (!sub_pes)
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_NONE)
        xws_find_stream_type(st, pes->stream_type, MISC_types);
    if (st->codecpar->codec_id == AV_CODEC_ID_NONE) {
...
static void reset_pes_packet_state(XPESContext *pes)
{
...
static int new_pes_packet(XPESContext *pes, AVPacket *pkt)
{
    uint8_t *sd;
...
static int read_sl_header(XPESContext *pes, SLConfigDescr *sl,
...
static AVBufferRef *ws_buffer_pool_get(WTSContext *ts, int size)
{
...
static int ws_push_data(WTSFilter *filter,
                            const uint8_t *buf, int buf_size, int is_start,
                            int64_t pos)
{
    XPESContext *pes   = filter->u.pes_filter.opaque;
    WTSContext *ts = pes->ts;
    const uint8_t *p;
    int ret, len, code;
                ...
                if (pes->header[0] == 0x00 && pes->header[1] == 0x00 &&
                    ...
                    if (!pes->st) {
                        ...
                        if (!pes->st)
                            return AVERROR(ENOMEM);
                        pes->st->id = pes->pid;
                        xws_set_stream_info(pes->st, pes, 0, 0);
                    }
                    ...
                    /* allocate pes buffer */
                    pes->buffer = ws_buffer_pool_get(ts, pes->total_size);
                    if (!pes->buffer)
                        return AVERROR(ENOMEM);
                ...
                if (   pes->ts->fix_teletext_pts
                    ...
                    while ((p = av_find_program_from_stream(pes->stream, p, pes->st->index))) {
                        if (p->pcr_pid != -1 && p->discard != AVDISCARD_ALL) {
                            WTSFilter *f = pes->ts->pids[p->pcr_pid];
                            if (f) {
                                AVStream *st = NULL;
                                if (f->type == MPEGTS_PES) {
                                    XPESContext *pcrpes = f->u.pes_filter.opaque;
                                    if (pcrpes)
                                        st = pcrpes->st;
...
static XPESContext *add_pes_stream(WTSContext *ts, int pid, int pcr_pid)
{
    WTSFilter *tss;
    XPESContext *pes;

    /* if no pid found, then add a pid context */
    pes = av_mallocz(sizeof(XPESContext));
    if (!pes)
        return 0;
    pes->ts      = ts;
    ...
    pes->pts     = AV_NOPTS_VALUE;
    pes->dts     = AV_NOPTS_VALUE;
    tss          = ws_open_pes_filter(ts, pid, ws_push_data, pes);
    if (!tss) {
        av_free(pes);
        return 0;
    }
...
static void m4sl_cb(WTSFilter *filter, const uint8_t *section,
                    int section_len)
{
    WTSContext *ts = filter->u.section_filter.opaque;
    WTSSectionFilter *tssf = &filter->u.section_filter;
    SectionHeader h;
...
static void scte_data_cb(WTSFilter *filter, const uint8_t *section,
                    int section_len)
{
    AVProgram *prg = NULL;
    WTSContext *ts = filter->u.section_filter.opaque;
    ...
    if (prg && prg->pcr_pid != -1 && prg->discard != AVDISCARD_ALL) {
        WTSFilter *f = ts->pids[prg->pcr_pid];
        if (f && f->last_pcr != -1)
...
static int ws_parse_mpeg2_descriptor(AVFormatContext *fc, AVStream *st, int stream_type,
                              ...
                              WTSContext *ts)
{
    ...
    if ((st->codecpar->codec_id == AV_CODEC_ID_NONE || st->request_probe > 0) &&
        stream_type == STREAM_TYPE_PRIVATE_DATA)
        xws_find_stream_type(st, desc_tag, DESC_types);
        ...
        for (i = 0; i < mp4_descr_count; i++)
            if (mp4_descr[i].dec_config_descr_len &&
                ...
                if (st->codecpar->codec_id == AV_CODEC_ID_MPEG4SYSTEMS)
                    xws_open_section_filter(ts, pid, m4sl_cb, ts, 0);
            }
        break;
    ...
    case 0x05: /* registration descriptor */
        ...
        if (st->codecpar->codec_id == AV_CODEC_ID_NONE || st->request_probe > 0) {
            xws_find_stream_type(st, st->codecpar->codec_tag, REGD_types);
    ...
    case 0x26: /* metadata descriptor */
        ...
        if (get8(pp, desc_end) == 0xFF) {
            st->codecpar->codec_tag = bytestream_get_le32(pp);
            if (st->codecpar->codec_id == AV_CODEC_ID_NONE)
                xws_find_stream_type(st, st->codecpar->codec_tag, METADATA_types);
        }
        break;
...
static AVStream *find_matching_stream(WTSContext *ts, int pid, unsigned int programid,
...
static void pmt_cb(WTSFilter *filter, const uint8_t *section, int section_len)
{
    WTSContext *ts = filter->u.section_filter.opaque;
    WTSSectionFilter *tssf = &filter->u.section_filter;
    ...
    for (i = 0; ; i++) {
        ...
        if (ts->pids[pid] && ts->pids[pid]->type == MPEGTS_PES) {
            ...
            if (ts->pids[pid])
                xws_close_filter(ts, ts->pids[pid]); // wrongly added sdt filter probably
            ...
            if (!st) {
                ...
                if (stream_type == 0x86 && prog_reg_desc == AV_RL32("CUEI")) {
                    xws_find_stream_type(st, stream_type, SCTE_types);
                    xws_open_section_filter(ts, pid, scte_data_cb, ts, 1);
                }
            }
        }
        ...
        if (pes && !pes->stream_type)
            xws_set_stream_info(st, pes, stream_type, prog_reg_desc);
        ...
        if (desc_list_end > p_end)
            goto out;
        for (;;) {
            if (ws_parse_mpeg2_descriptor(ts->stream, st, stream_type, &p,
    ...
    if (!ts->pids[pcr_pid])
        xws_open_pcr_filter(ts, pcr_pid);
...
static void pat_cb(WTSFilter *filter, const uint8_t *section, int section_len)
{
    WTSContext *ts = filter->u.section_filter.opaque;
    WTSSectionFilter *tssf = &filter->u.section_filter;
    SectionHeader h1, *h = &h1;
        ...
        if (sid == 0x0000) {
            ...
            if (fil)
                if (   fil->type != MPEGTS_SECTION
                    ...
                    xws_close_filter(ts, ts->pids[pmt_pid]);

            if (!ts->pids[pmt_pid])
                xws_open_section_filter(ts, pmt_pid, pmt_cb, ts, 0);
...
static void eit_cb(WTSFilter *filter, const uint8_t *section, int section_len)
{
    WTSContext *ts = filter->u.section_filter.opaque;
    const uint8_t *p, *p_end;
    SectionHeader h1, *h = &h1;
...
static void sdt_cb(WTSFilter *filter, const uint8_t *section, int section_len)
{
    WTSContext *ts = filter->u.section_filter.opaque;
    WTSSectionFilter *tssf = &filter->u.section_filter;
    SectionHeader h1, *h = &h1;
...
static int handle_packet(WTSContext *ts, const uint8_t *packet, int64_t pos)
{
    WTSFilter *tss;
    ...
    if (!cc_ok) {
        ...
        if (tss->type == MPEGTS_PES) {
            XPESContext *pc = tss->u.pes_filter.opaque;
...
static int xws_resync(AVFormatContext *s, int seekback, const uint8_t *current_packet)
{
    WTSContext *ts = s->priv_data;
    AVIOContext *pb = s->pb;
    int c, i;
    ...
    if (current_packet[0] == 0x80 && current_packet[12] == WX_BYTE) {
                       ...
                       const uint8_t **data)
{
    ...
    for (;;) {
        ...
        if ((*data)[0] != WX_BYTE) {
            ...
            if (xws_resync(s, raw_packet_size, *data) < 0)
...
static int handle_packets(WTSContext *ts, int64_t nb_packets)
{
    AVFormatContext *s = ts->stream;
        ...
        for (i = 0; i < NB_PID_MAX; i++) {
            if (ts->pids[i]) {
                if (ts->pids[i]->type == MPEGTS_PES) {
                    XPESContext *pes = ts->pids[i]->u.pes_filter.opaque;
                    av_buffer_unref(&pes->buffer);
                    pes->data_index = 0;
...
static int xws_probe(const AVProbeData *p)
{
...
#define CHECK_BLOCK 100
    ...
    for (i = 0; i<check_count; i+=CHECK_BLOCK) {
        ...
        int score      = xws_analyze(p->buf + TS_PACKET_SIZE     *i, TS_PACKET_SIZE     *left, TS_PACKET_SIZE     , 1);
        int dvhs_score = xws_analyze(p->buf + TS_DVHS_PACKET_SIZE*i, TS_DVHS_PACKET_SIZE*left, TS_DVHS_PACKET_SIZE, 1);
        int fec_score  = xws_analyze(p->buf + TS_FEC_PACKET_SIZE *i, TS_FEC_PACKET_SIZE *left, TS_FEC_PACKET_SIZE , 1);
...
extern AVInputFormat ff_xws_demuxer;
static int xws_read_header(AVFormatContext *s)
{
    WTSContext *ts = s->priv_data;
    AVIOContext *pb   = s->pb;
    ...
    if (s->iformat == &ff_xws_demuxer) {
        /* normal demux */

        /* first do a scan to get all the services */
        seek_back(s, pb, pos);

        xws_open_section_filter(ts, SDT_PID, sdt_cb, ts, 0);
        xws_open_section_filter(ts, PAT_PID, pat_cb, ts, 0);
        xws_open_section_filter(ts, EIT_PID, eit_cb, ts, 0);
...
static int xws_raw_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    WTSContext *ts = s->priv_data;
    int ret, i;
...
static int xws_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    WTSContext *ts = s->priv_data;
    int ret, i;
...
static void xws_free(WTSContext *ts)
{
    int i;
    ...
    for (i = 0; i < NB_PID_MAX; i++)
        if (ts->pids[i])
            xws_close_filter(ts, ts->pids[i]);
}

static int xws_read_close(AVFormatContext *s)
{
    WTSContext *ts = s->priv_data;
    xws_free(ts);
    return 0;
...
static av_unused int64_t xws_get_pcr(AVFormatContext *s, int stream_index,
                              int64_t *ppos, int64_t pos_limit)
{
    ...
    int pid = ((XPESContext*)s->streams[stream_index]->priv_data)->pid;
    ...
    while(pos < pos_limit) {
        ...
        if (avio_read(s->pb, buf, TS_PACKET_SIZE) != TS_PACKET_SIZE)
            return AV_NOPTS_VALUE;
        if (buf[0] != WX_BYTE) {
            if (xws_resync(s, TS_PACKET_SIZE, buf) < 0)
                return AV_NOPTS_VALUE;
            pos = avio_tell(s->pb);
            continue;
        }
...
static int64_t xws_get_dts(AVFormatContext *s, int stream_index,
                              int64_t *ppos, int64_t pos_limit)
{
    WTSContext *ts = s->priv_data;
    int64_t pos;
...
static int avpriv_xws_add_stream(AVFormatContext *s, int pid, uint32_t stream_type, int program)
{
    WTSContext *ts = s->priv_data;
    WTSFilter *tss = ts->pids[pid];
    XPESContext *pes = NULL;
    AVStream *st = NULL;
    ...
    xws_set_stream_info(st, pes, stream_type, AV_RL32("HDMV"));
    if (program >= 0)
...
static WTSContext *avpriv_xws_parse_open(AVFormatContext *s)
{
    WTSContext *ts;

    ts = av_mallocz(sizeof(WTSContext));
    if (!ts)
        return NULL;
 ...
 * packet is output */
static int avpriv_xws_parse_packet(WTSContext *ts, AVPacket *pkt,
...
static void avpriv_ws_parse_close(WTSContext *ts)
{
    xws_free(ts);
    av_free(ts);
}

AVInputFormat ff_xws_demuxer = {
    .name           = "xws",
    .long_name      = NULL_IF_CONFIG_SMALL("WXMedia Media Stream"),
    .priv_data_size = sizeof(WTSContext),
    .read_probe     = xws_probe,
    .read_header    = xws_read_header,
    .read_packet    = xws_read_packet,
    .read_close     = xws_read_close,
    .read_timestamp = xws_get_pcr,
    .flags          = AVFMT_SHOW_IDS | AVFMT_TS_DISCONT,
    .priv_class     = &xws_class,
};
