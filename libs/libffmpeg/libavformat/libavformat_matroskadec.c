...
static EbmlSyntax ebml_syntax_wxmv[3];
...
//add by Tam
static EbmlSyntax ebml_syntax_wxmv[] = {
    { WXMV_ID_HEADER,      EBML_NEST, 0, 0, { .n = ebml_header } },
    { MATROSKA_ID_SEGMENT, EBML_STOP },
    { 0 }
};
...
static const char *const matroska_doctypes[] = { "matroska", "webm" };// Add By Tam.xie
...
static int matroska_probe(const AVProbeData *p)
{
   ...
   // if (AV_RB32(p->buf) != EBML_ID_HEADER)
    ...
    //add by Tam.Xie for wxmv
    if (AV_RB32(p->buf) == WXMV_ID_HEADER){
        bMKV = 1;
    }
    if(!bMKV){
        return 0;
...
static int matroska_read_header(AVFormatContext *s)
{
    ...
    if (ebml_parse(matroska, ebml_syntax, &ebml) || !ebml.doctype) {
        av_log(matroska->ctx, AV_LOG_ERROR, "+++++ EBML header ebml_syntax parsing failed\n");
    ...
    if(!bCheckMKV){
        ...
        if (ebml_parse(matroska, ebml_syntax_wxmv, &ebml) || !ebml.doctype) {
            av_log(matroska->ctx, AV_LOG_ERROR, "------ EBML header ebml_syntax_wxmv parsing failed\n");
            ebml_free(ebml_syntax_wxmv, &ebml);
            return AVERROR_INVALIDDATA;
        }else{
            bMKV = 0;
        }
    ...
    if (ebml.version         > EBML_VERSION      ||
        ...
        ebml_free(bMKV?ebml_syntax:ebml_syntax_wxmv, &ebml);
