...
typedef struct WTSSection {
    int pid;
    int cc;
    int discontinuity;
    void (*write_packet)(struct WTSSection *s, const uint8_t *packet);
    void *opaque;
} WTSSection;

typedef struct WTSService {
    WTSSection pmt; /* MPEG-2 PMT table context */
    int sid;           /* service ID */
    uint8_t name[256];
...
enum {
    WTS_SERVICE_TYPE_DIGITAL_TV                   = 0x01,
    WTS_SERVICE_TYPE_DIGITAL_RADIO                = 0x02,
    WTS_SERVICE_TYPE_TELETEXT                     = 0x03,
    WTS_SERVICE_TYPE_ADVANCED_CODEC_DIGITAL_RADIO = 0x0A,
    WTS_SERVICE_TYPE_MPEG2_DIGITAL_HDTV           = 0x11,
    WTS_SERVICE_TYPE_ADVANCED_CODEC_DIGITAL_SDTV  = 0x16,
    WTS_SERVICE_TYPE_ADVANCED_CODEC_DIGITAL_HDTV  = 0x19,
    WTS_SERVICE_TYPE_HEVC_DIGITAL_HDTV            = 0x1F,
};
typedef struct WTSWrite {
    const AVClass *av_class;
    WTSSection pat; /* MPEG-2 PAT table */
    WTSSection sdt; /* MPEG-2 SDT table context */
    WTSService **services;
    ...
    int pcr_period_ms;
#define WTS_FLAG_REEMIT_PAT_PMT  0x01
#define WTS_FLAG_AAC_LATM        0x02
#define WTS_FLAG_PAT_PMT_AT_FRAMES           0x04
#define WTS_FLAG_SYSTEM_B        0x08
#define WTS_FLAG_DISCONT         0x10
    int flags;
    int copyts;
    int tables_version;
...
static void xws_write_section(WTSSection *s, uint8_t *buf, int len)
{
    unsigned int crc;
...
static int xws_write_section1(WTSSection *s, int tid, int id,
                                 ...
                                 uint8_t *buf, int len)
{
    ...
    *q++ = last_sec_num;
    ...
    xws_write_section(s, section, tot_len);
    return 0;
}
...
static void xws_write_pat(AVFormatContext *s)
{
    WTSWrite *ts = s->priv_data;
    WTSService *service;
    ...
    xws_write_section1(&ts->pat, PAT_TID, ts->transport_stream_id, ts->tables_version, 0, 0,
                          data, q - data);
}
...
{
    WTSWrite *ts = s->priv_data;
    WTSWriteStream *ts_st = st->priv_data;
    int stream_type;
    ...
    case AV_CODEC_ID_AAC:
        stream_type = (ts->flags & WTS_FLAG_AAC_LATM)
    ...
    case AV_CODEC_ID_AC3:
        stream_type = (ts->flags & WTS_FLAG_SYSTEM_B)
...
static int xws_write_pmt(AVFormatContext *s, WTSService *service)
{
    WTSWrite *ts = s->priv_data;
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        *q++ = stream_type;
        ...
        case AVMEDIA_TYPE_AUDIO:
            if (st->codecpar->codec_id==AV_CODEC_ID_AC3 && (ts->flags & WTS_FLAG_SYSTEM_B)) {
                *q++=0x6a; // AC3 descriptor see A038 DVB SI
                *q++=1; // 1 byte, all flags sets to 0
                *q++=0; // omit all fields...
            }
            if (st->codecpar->codec_id==AV_CODEC_ID_EAC3 && (ts->flags & WTS_FLAG_SYSTEM_B)) {
    ...
    xws_write_section1(&service->pmt, PMT_TID, service->sid, ts->tables_version, 0, 0,
                          data, q - data);
    return 0;
}

static void xws_write_sdt(AVFormatContext *s)
{
    WTSWrite *ts = s->priv_data;
    WTSService *service;
    ...
    *q++ = 0xff;
    ...
    xws_write_section1(&ts->sdt, SDT_TID, ts->transport_stream_id, ts->tables_version, 0, 0,
                          data, q - data);
}
...
invalid:
    ...
    if (str_len > 255)
        return AVERROR(EINVAL);
    buf[0] = (uint8_t)str_len;
    memcpy(&buf[1], str, str_len);
    return 0;
}

static int64_t get_pcr(const WTSWrite *ts)
{
    return av_rescale(ts->totol_size + 11, 8 * PCR_TIME_BASE, ts->mux_rate) +
           ts->first_pcr;
}
...
static void write_packet(AVFormatContext *s, const uint8_t *packet)
{
    ...
    ts->totol_size += TS_PACKET_SIZE;
}

static void section_write_packet(WTSSection *s, const uint8_t *packet)
{
    AVFormatContext *ctx = s->opaque;
    write_packet(ctx, packet);
}

static WTSService *xws_add_service(AVFormatContext *s, int sid,
                                         ...
                                         AVProgram *program)
{
    ...
    service = av_mallocz(sizeof(WTSService));
    if (!service)
        return NULL;
    ...
    service->pmt.cc           = 15;
    service->pmt.discontinuity= ts->flags & WTS_FLAG_DISCONT;
...
{
    WTSWrite *ts = s->priv_data;
    WTSWriteStream *ts_st = pcr_st->priv_data;
...
static void select_pcr_streams(AVFormatContext *s)
{
    ...
    for (int i = 0; i < ts->nb_services; i++) {
        WTSService *service = ts->services[i];
...
static int xws_init(AVFormatContext *s)
{
    WTSWrite *ts = s->priv_data;
    int i, j;
    int ret;
    ...
    if (!s->nb_programs) {
        ...
        if (!xws_add_service(s, ts->service_id, s->metadata, NULL))
            return AVERROR(ENOMEM);
    } else {
        for (i = 0; i < s->nb_programs; i++) {
            AVProgram *program = s->programs[i];
            if (!xws_add_service(s, program->id, program->metadata, program))
                return AVERROR(ENOMEM);
        }
    }
    ...
    ts->pat.cc           = 15;
    ts->pat.discontinuity= ts->flags & WTS_FLAG_DISCONT;
    ts->pat.write_packet = section_write_packet;
    ts->pat.opaque       = s;

    ts->sdt.pid          = SDT_PID;
    ts->sdt.cc           = 15;
    ts->sdt.discontinuity= ts->flags & WTS_FLAG_DISCONT;
    ...
    for (i = 0; i < s->nb_streams; i++) {
        AVStream *st = s->streams[i];
        WTSWriteStream *ts_st;

        ts_st = av_mallocz(sizeof(WTSWriteStream));
        if (!ts_st) {
            return AVERROR(ENOMEM);
        }
        st->priv_data = ts_st;
        ...
        for (j = 0; j < i; j++) {
            WTSWriteStream *ts_st_prev = s->streams[j]->priv_data;
        ...
        ts_st->cc              = 15;
        ts_st->discontinuity   = ts->flags & WTS_FLAG_DISCONT;
        if (st->codecpar->codec_id == AV_CODEC_ID_AAC &&
            ...
            ts_st->amux->oformat =
                av_guess_format((ts->flags & WTS_FLAG_AAC_LATM) ? "latm" : "adts",
                                NULL, NULL);
...
static void xws_insert_null_packet(AVFormatContext *s)
{
    uint8_t *q;
...
static void xws_insert_pcr_only(AVFormatContext *s, AVStream *st)
{
    WTSWrite *ts = s->priv_data;
    WTSWriteStream *ts_st = st->priv_data;
    uint8_t *q;
...
static void xws_write_pts(uint8_t *q, int fourbits, int64_t pts)
{
    int val;
...
static void xws_write_pes(AVFormatContext *s, AVStream *st,
                             ...
                             int64_t pts, int64_t dts, int key, int stream_id)
{
    ...
    if (ts->flags & WTS_FLAG_PAT_PMT_AT_FRAMES && st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
        force_pat = 1;
    }

    if (ts->flags & WTS_FLAG_REEMIT_PAT_PMT) {
        force_pat = 1;
        force_sdt = 1;
        ts->flags &= ~WTS_FLAG_REEMIT_PAT_PMT;
    }
    ...
    while (payload_size > 0) {
        ...
        if (ts->mux_rate > 1) {
            ...
            if (pcr >= ts->next_pcr) {
                ...
                for (int i = 0; i < s->nb_streams; i++) {
                    ...
                    WTSWriteStream *ts_st2 = st2->priv_data;
            ...
            if (dts != AV_NOPTS_VALUE && (dts - pcr / 300) > delay) {
                ...
                if (write_pcr)
                    xws_insert_pcr_only(s, st);
                else
                    xws_insert_null_packet(s);
        ...
        *q++      = 0x10 | ts_st->cc; // payload indicator + CC
        ...
        if (is_start) {
            ...
            *q++ = header_len;
            if (pts != AV_NOPTS_VALUE) {
                xws_write_pts(q, flags >> 6, pts);
                q += 5;
            }
            if (dts != AV_NOPTS_VALUE && pts != AV_NOPTS_VALUE && dts != pts) {
                xws_write_pts(q, 1, dts);
                q += 5;
            }
...
static int xws_ff_check_h264_startcode(AVFormatContext *s, const AVStream *st, const AVPacket *pkt)
{
...
static int xws_write_packet_internal(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_H264) {
        ...
        int ret = xws_ff_check_h264_startcode(s, st, pkt);
        if (ret < 0)
            return ret;
    ...
    if (ts_st->payload_size && (ts_st->payload_size + size > ts->pes_payload_size ||
        ...
        xws_write_pes(s, st, ts_st->payload, ts_st->payload_size,
    ...
    if (st->codecpar->codec_type != AVMEDIA_TYPE_AUDIO || size > ts->pes_payload_size) {
        ...
        xws_write_pes(s, st, buf, size, pts, dts,
...
static void xws_write_flush(AVFormatContext *s)
{
    WTSWrite *ts = s->priv_data;
    int i;
...
static int xws_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    if (!pkt) {
        xws_write_flush(s);
        return 1;
    } else {
        return xws_write_packet_internal(s, pkt);
    }
}

static int xws_write_end(AVFormatContext *s)
{
    if (s->pb)
        xws_write_flush(s);
    return 0;
...
static void xws_deinit(AVFormatContext *s)
{
    WTSWrite *ts = s->priv_data;
    WTSService *service;
    int i;
...
static int xws_check_bitstream(struct AVFormatContext *s, const AVPacket *pkt)
{
    int ret = 1;
    AVStream *st = s->streams[pkt->stream_index];
...
#define OFFSET(x) offsetof(WTSWrite, x)
#define ENC AV_OPT_FLAG_ENCODING_PARAM
static const AVOption xws_options[] = {
    { "ws_transport_stream_id", "Set transport_stream_id field.",
      OFFSET(transport_stream_id), AV_OPT_TYPE_INT, { .i64 = 0x0001 }, 0x0001, 0xffff, ENC },
    { "ws_original_network_id", "Set original_network_id field.",
      OFFSET(original_network_id), AV_OPT_TYPE_INT, { .i64 = DVB_PRIVATE_NETWORK_START }, 0x0001, 0xffff, ENC },
    { "ws_service_id", "Set service_id field.",
      OFFSET(service_id), AV_OPT_TYPE_INT, { .i64 = 0x0001 }, 0x0001, 0xffff, ENC },
    { "ws_service_type", "Set service_type field.",
      OFFSET(service_type), AV_OPT_TYPE_INT, { .i64 = 0x01 }, 0x01, 0xff, ENC, "ws_service_type" },
    { "digital_tv", "Digital Television.",
      0, AV_OPT_TYPE_CONST, { .i64 = WTS_SERVICE_TYPE_DIGITAL_TV }, 0x01, 0xff, ENC, "ws_service_type" },
    { "digital_radio", "Digital Radio.",
      0, AV_OPT_TYPE_CONST, { .i64 = WTS_SERVICE_TYPE_DIGITAL_RADIO }, 0x01, 0xff, ENC, "ws_service_type" },
    { "teletext", "Teletext.",
      0, AV_OPT_TYPE_CONST, { .i64 = WTS_SERVICE_TYPE_TELETEXT }, 0x01, 0xff, ENC, "ws_service_type" },
    { "advanced_codec_digital_radio", "Advanced Codec Digital Radio.",
      0, AV_OPT_TYPE_CONST, { .i64 = WTS_SERVICE_TYPE_ADVANCED_CODEC_DIGITAL_RADIO }, 0x01, 0xff, ENC, "ws_service_type" },
    { "mpeg2_digital_hdtv", "MPEG2 Digital HDTV.",
      0, AV_OPT_TYPE_CONST, { .i64 = WTS_SERVICE_TYPE_MPEG2_DIGITAL_HDTV }, 0x01, 0xff, ENC, "ws_service_type" },
    { "advanced_codec_digital_sdtv", "Advanced Codec Digital SDTV.",
      0, AV_OPT_TYPE_CONST, { .i64 = WTS_SERVICE_TYPE_ADVANCED_CODEC_DIGITAL_SDTV }, 0x01, 0xff, ENC, "ws_service_type" },
    { "advanced_codec_digital_hdtv", "Advanced Codec Digital HDTV.",
      0, AV_OPT_TYPE_CONST, { .i64 = WTS_SERVICE_TYPE_ADVANCED_CODEC_DIGITAL_HDTV }, 0x01, 0xff, ENC, "ws_service_type" },
    { "hevc_digital_hdtv", "HEVC Digital Television Service.",
      0, AV_OPT_TYPE_CONST, { .i64 = WTS_SERVICE_TYPE_HEVC_DIGITAL_HDTV }, 0x01, 0xff, ENC, "ws_service_type" },
    { "ws_pmt_start_pid", "Set the first pid of the PMT.",
      OFFSET(pmt_start_pid), AV_OPT_TYPE_INT, { .i64 = 0x1000 }, FIRST_OTHER_PID, LAST_OTHER_PID, ENC },
    { "ws_start_pid", "Set the first pid.",
    ...
    { "ws_flags", "MPEG-TS muxing flags", OFFSET(flags), AV_OPT_TYPE_FLAGS, { .i64 = 0 }, 0, INT_MAX, ENC, "ws_flags" },
    { "resend_headers", "Reemit PAT/PMT before writing the next packet",
      0, AV_OPT_TYPE_CONST, { .i64 = WTS_FLAG_REEMIT_PAT_PMT }, 0, INT_MAX, ENC, "ws_flags" },
    { "latm", "Use LATM packetization for AAC",
      0, AV_OPT_TYPE_CONST, { .i64 = WTS_FLAG_AAC_LATM }, 0, INT_MAX, ENC, "ws_flags" },
    { "pat_pmt_at_frames", "Reemit PAT and PMT at each video frame",
      0, AV_OPT_TYPE_CONST, { .i64 = WTS_FLAG_PAT_PMT_AT_FRAMES}, 0, INT_MAX, ENC, "ws_flags" },
    { "system_b", "Conform to System B (DVB) instead of System A (ATSC)",
      0, AV_OPT_TYPE_CONST, { .i64 = WTS_FLAG_SYSTEM_B }, 0, INT_MAX, ENC, "ws_flags" },
    { "initial_discontinuity", "Mark initial packets as discontinuous",
      0, AV_OPT_TYPE_CONST, { .i64 = WTS_FLAG_DISCONT }, 0, INT_MAX, ENC, "ws_flags" },
    { "ws_copyts", "don't offset dts/pts", OFFSET(copyts), AV_OPT_TYPE_BOOL, { .i64 = -1 }, -1, 1, ENC },
...
static const AVClass xws_muxer_class = {
    .class_name = "xws muxer",
    .item_name  = av_default_item_name,
    .option     = xws_options,
    .version    = LIBAVUTIL_VERSION_INT,
};

AVOutputFormat ff_xws_muxer = {
    .name            = "xws",
    .long_name       = "WXMedia Media Stream",
    .mime_type       = "video/ws",
    .extensions      = "ws",
    .priv_data_size  = sizeof(WTSWrite),
    .audio_codec     = AV_CODEC_ID_AAC,
    .video_codec     = AV_CODEC_ID_H264,
    .init            = xws_init,
    .write_packet    = xws_write_packet,
    .write_trailer   = xws_write_end,
    .deinit          = xws_deinit,
    .check_bitstream = xws_check_bitstream,
    .flags           = AVFMT_ALLOW_FLUSH | AVFMT_VARIABLE_FPS | AVFMT_NODIMENSIONS,
    .priv_class      = &xws_muxer_class,
};
