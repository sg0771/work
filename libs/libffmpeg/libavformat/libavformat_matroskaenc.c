...
static int mkv_write_header(AVFormatContext *s)
{
    ...
    //Edit by Tam.Xie for wxmv
    if(!strcmp(s->oformat->name, "wxmv")){
        ebml_header = start_ebml_master(pb, WXMV_ID_HEADER, MAX_EBML_HEADER_SIZE);
    }else{
    ...
    put_ebml_string(pb, EBML_ID_DOCTYPE           , s->oformat->name);//写入容器名字，包括自定义
...
//Add By Tam.Xie
AVOutputFormat ff_wxmv_muxer = {
    .name              = "wxmv",
    .long_name         = NULL_IF_CONFIG_SMALL("wxmv"),
    .mime_type         = "video/x-wxmv",
    .extensions        = "wxmv",
    .priv_data_size    = sizeof(MatroskaMuxContext),
