...
// Set Key And IV For HLS
//Add By Tam
//2022.07.29
static int s_bSetKey = 0;
static char s_key[33];
static char s_iv[33];
void av_set_hls_key_and_iv(char *key, char* iv){
    if(key == NULL && iv == NULL){
	    s_bSetKey  =0;
    }else {
        s_bSetKey = 1;
                          ...
                          struct playlist *pls, AVIOContext *in)
{
    ...
    while (!avio_feof(in)) {
        ...
        if (av_strstart(line, "#EXT-X-STREAM-INF:", &ptr)) {
            ...
            if (has_iv) {
                memcpy(cur_init_section->m_iv, iv, sizeof(iv));
            } else {
                int seq = pls->start_seq_no + pls->n_segments;
                memset(cur_init_section->m_iv, 0, sizeof(cur_init_section->m_iv));
                AV_WB32(cur_init_section->m_iv + 12, seq);
            }
            ...
            if (is_segment) {
                ...
                if (has_iv) {
                    memcpy(seg->m_iv, iv, sizeof(iv));
                } else {
                    int seq = pls->start_seq_no + pls->n_segments;
                    memset(seg->m_iv, 0, sizeof(seg->m_iv));
                    AV_WB32(seg->m_iv + 12, seq);
                }
...
static int open_input(HLSContext *c, struct playlist *pls, struct segment *seg, AVIOContext **in)
{
    ...
    if (seg->key_type == KEY_NONE) {
        ...
        if (s_bSetKey) { //using key an iv by Set
            //Add By Tam 2022.07.29
            //av_log(NULL, AV_LOG_ERROR, "AAAA Using Custom AES-128 Key[%s] IV[%s]\n", s_key, s_iv);
            //ff_data_to_hex(iv, s_iv, sizeof(s_iv), 0);
            //ff_data_to_hex(key, s_key, sizeof(s_key), 0);
            strcpy(key,s_key);
            strcpy(iv, s_iv);
            av_log(NULL, AV_LOG_ERROR, "BBBB Using Custom AES-128 Key[%s] IV[%s]\n", s_key, s_iv);
        }
        else {
            if (strcmp(seg->key, pls->key_url)) {
           	     AVIOContext *pb = NULL;
            	    if (open_url(pls->parent, &pb, seg->key, c->avio_opts, opts, NULL) == 0) {
                	    ret = avio_read(pb, pls->m_key, sizeof(pls->m_key));
               	 	    if (ret != sizeof(pls->m_key)) {
            ...
            ff_data_to_hex(iv, seg->m_iv, sizeof(seg->m_iv), 0);
            ff_data_to_hex(key, pls->m_key, sizeof(pls->m_key), 0);
        }
        ...
        {
            av_log(NULL, AV_LOG_ERROR, "open_url [%s] = %d\n", url, ret);
        }
        if (ret < 0) {
            goto cleanup;
        }
