...
//add by Tam
static int s_nBufferSize  = 0;
static void* s_DisplayWindow = NULL;
//Set D3D Device TargetWindow(for rendering) and Buffer Size

void av_set_display_window(void* hwnd, int screen_x, int screen_y, int cx, int cy){
    s_DisplayWindow = hwnd;
    int s1 = FFMAX(screen_x,screen_y);
    int s2 = FFMAX(cx,cy);
    s_nBufferSize = FFMAX(s1, s2);
}
...
static int dxva2_device_create9(AVHWDeviceContext *ctx, UINT adapter)
{
    ...
    if(s_nBufferSize != 0){
        d3dpp.BackBufferWidth    = s_nBufferSize;
        d3dpp.BackBufferHeight   = s_nBufferSize;
    }
    ...
    HWND hwnd = (HWND)s_DisplayWindow;
    if(!IsWindow(hwnd)){
...
static int dxva2_device_create9ex(AVHWDeviceContext *ctx, UINT adapter)
{
    ...
    hr = IDirect3D9Ex_CreateDeviceEx(d3d9ex, adapter, D3DDEVTYPE_HAL, hwnd,
                                     FF_D3DCREATE_FLAGS,
                                     &d3dpp, NULL, &exdev);
    s_DisplayWindow = NULL;
    if (FAILED(hr)) {
