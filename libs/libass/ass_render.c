...
static ASS_Image *render_text(ASS_Renderer *render_priv)
{
    ...
    //uint32_t shadow = 0x00000001;// 0xe58acf01;  //c[3]
    //uint32_t outline2 = 0x00000001;// 0xf6b2ff01;//
    //uint32_t outline1 = 0xff478101;// 0xffffff01; //c[2]
    //uint32_t main = 0xffb2ce01;// 0xf8ccff01;  //c[0]

    //uint32_t shadow = 0xe58acf01;  //c[3]
    //uint32_t outline2 = 0xf6b2ff01;//
    //uint32_t outline1 = 0xffffff01; //c[2]
    //uint32_t main = 0xf8ccff01;  //c[0]
    for (unsigned i = 0; i < n_bitmaps; i++) {
        CombinedBitmapInfo* info = &bitmaps[i];
    //info->c[5] = 0xf8ccff01;
        if (!info->bm_bg || render_priv->state.border_style == 4)
            continue;
        if (i==1)
        {
         
            tail =
                render_glyph(render_priv, info->bm_bg, info->x, info->y, info->c[5], 0,
                    1000000, tail, IMAGE_TYPE_BACKGROUND, info->image);
        }
        else
        {
            tail =
    ...
    for (unsigned i = 0; i < n_bitmaps; i++) {
        ...
        if ((info->effect_type == EF_KARAOKE_KO)
            ...
            tail =
                render_glyph(render_priv, info->bm_oo, info->x, info->y, info->c[4],
                                                   ...
                                                   ASS_Style *rstyle)
{
    ...
    *new = *rstyle;
    ...
    if (requested & ASS_OVERRIDE_BIT_COLORS) {
        ...
        new->OutlineColour2 = user->OutlineColour2;

        new->BackColour = user->BackColour;
        new->BackColour2 = user->BackColour2;
    }
...
void reset_render_context(ASS_Renderer *render_priv, ASS_Style *style)
{
    ...
    render_priv->state.c[4] = style->OutlineColour2;
    render_priv->state.c[5] = style->BackColour2;
    ...
    render_priv->state.border_x2 = style->Outline;
    render_priv->state.border_y2 = style->Outline;
    render_priv->state.border_oo = style->Outline;
...
get_bitmap_glyph(ASS_Renderer *render_priv, GlyphInfo *info,
                 int32_t *leftmost_x,
                 ASS_Vector *pos, ASS_Vector *pos_o, ASS_Vector* pos_oo, ASS_Vector* pos_bg,
                 ASS_DVector *offset, bool first, int flags)
{
    ...
    *pos_oo = *pos;
    ...
    if (flags & FILTER_BORDER_STYLE_3) {
        if (!(flags & (FILTER_NONZERO_BORDER | FILTER_NONZERO_SHADOW| FILTER_NONZERO_BACK)))
            return;
 ...
 /*       ASS_DVector offset = { -bord.x, -bord.y - info->asc };
        ...
        ASS_DVector offset = { -bord.x , -bord.y - info->asc };
        //ASS_DVector offset = { -bord.x, -bord.y  };
        for (int i = 0; i < 3; i++) {
            m[i][0] = m1[i][0] * scale.x;
            m[i][1] = m1[i][1] * scale.y;
            m[i][2] = m1[i][0] * offset.x + m1[i][1] * offset.y + m1[i][2];
    ...
    } else if (flags & FILTER_NONZERO_BORDER) {
...
key.outline = ass_cache_get(render_priv->cache.outline_cache, &ol_key, render_priv);
    ...
    if (render_priv->state.c[4] !=0 && render_priv->state.c[4] != 255)
    {
        OutlineHashKey ool_key;
        ool_key.type = -1;
        if (flags & FILTER_BORDER_STYLE_3) {

        }
        else if (flags & FILTER_NONZERO_BORDER) {
            ool_key.type = OUTLINE_BORDER;
            BorderHashKey* k = &ool_key.u.border;
            k->outline = info->outline;

            double w = 64 * render_priv->border_scale;
            double bord_x = w * (info->border_oo+info->border_x)  / tr->scale.x;
            double bord_y = w * (info->border_oo+info->border_y) / tr->scale.y;
        ...
        if (ool_key.type!=-1)
        {
            key.outline = ass_cache_get(render_priv->cache.outline_cache, &ool_key, render_priv);
            if (!key.outline || !key.outline->valid ||
                !quantize_transform(m, pos_o, offset, false, &key)) {
                ass_cache_dec_ref(key.outline);
                return;
            ...
            info->bm_oo = ass_cache_get(render_priv->cache.bitmap_cache, &key, render_priv);
            if (!info->bm_oo || !info->bm_oo->buffer) {
                ass_cache_dec_ref(info->bm_oo);
                info->bm_oo = NULL;
                *pos_oo = *pos;
            }
            else if (!info->bm)
    ...
    //render_priv->state.c[5] = 0xf8ccff01;
    // 
    // 
    // 
    //���Ʊ���  
    OutlineHashKey bg_key;
    if (render_priv->state.c[5] != 0&& render_priv->state.c[5] != 255) {

        {

            bg_key.type = OUTLINE_BOX;

            double w = 64 * render_priv->border_scale;
            ASS_DVector bord = { info->border_x2 * w, info->border_y2 * w };
            double width = info->hspacing_scaled + info->advance.x;
            double height = info->asc + info->desc;
           ...
           ASS_DVector offset = { -bord.x, -bord.y - info->asc };
            ...
            for (int i = 0; i < 3; i++) {
                m[i][0] = m1[i][0] * scale.x;
                m[i][1] = m1[i][1] *scale.y;
                //m[i][2] = m1[i][0] * offset.x* orig_scale.x + orig_scale.y *m1[i][1] * offset.y+ m1[i][2];
                m[i][2] = m1[i][0] * offset.x + m1[i][1] * offset.y + m1[i][2];
           }
            ...
            key.outline = ass_cache_get(render_priv->cache.outline_cache, &bg_key, render_priv);
            if (!key.outline || !key.outline->valid ||
                !quantize_transform(m, pos_bg, offset, false, &key)) {
                ass_cache_dec_ref(key.outline);
                return;
            }
            info->bm_bg = ass_cache_get(render_priv->cache.bitmap_cache, &key, render_priv);
            

            if (!info->bm_bg || !info->bm_bg->buffer) {
                ass_cache_dec_ref(info->bm_bg);
                info->bm_bg = NULL;
                *pos_bg = *pos;
            }
            else if (!info->bm)
                                ...
                                double max_border_x, double max_border_y)
{
    ...
    //justin ��Ҫ��֮ǰ���õ� border Ӱ�����Ŀ���
    render_priv->text_info.border_x = (int)(render_priv->border_scale * max_border_x + 0.5);
...
static void measure_text(ASS_Renderer *render_priv)
{
    ...
    for (int i = 0; i < text_info->length; i++) {
        ...
        max_border_y = FFMAX(max_border_y,   FFMAX(cur->border_y2, FFMAX(cur->border_oo, cur->border_y)));
        max_border_x = FFMAX(max_border_x,  FFMAX(cur->border_x2, FFMAX(cur->border_oo, cur->border_x)));
        if (cur->symbol != '\n')
            scale = 1.0 / 64;
    }
    ...
    measure_text_on_eol(render_priv, scale, cur_line,
            max_asc, max_desc, max_border_x, max_border_y);
    text_info->height += cur_line * render_priv->state.line_spacing;
...
#undef DIFF
    ...
    for (i = 0; i < text_info->length; ++i) {
        ...
        if (cur->linebreak) {
            ...
            cur_line++;
            pen_shift_x = d6_to_double(-cur->pos.x);
            pen_shift_y += height + render_priv->state.line_spacing;
        }
...
static void split_style_runs(ASS_Renderer *render_priv)
{
    ...
    for (int i = 1; i < render_priv->text_info.length; i++) {
        ...
        info->starts_new_run =
            ...
            last->border_x2 != info->border_x2 ||
            last->border_y2 != info->border_y2 ||
            last->border_oo != info->border_oo ||
...
static bool parse_events(ASS_Renderer *render_priv, ASS_Event *event)
{
    ...
    while (true) {
        ...
        info->border_x2 = render_priv->state.border_x2;
        info->border_y2 = render_priv->state.border_y2;
        info->border_oo = render_priv->state.border_oo;
...
static void reorder_text(ASS_Renderer *render_priv)
{
    ...
    for (int i = 0; i < text_info->length; i++) {
        ...
        if (text_info->glyphs[i].linebreak) {
            ...
            pen.y += double_to_d6(render_priv->state.line_spacing);
            lineno++;
        }
                                      ...
                                      double device_x, double device_y)
{
    ...
    for (int i = 0; i < text_info->length; i++) {
        ...
        for (; info; info = info->next) {
            ...
            if (info->border_x || info->border_y || info->border_oo)
                flags |= FILTER_NONZERO_BORDER;

            if (info->c[5] )
                flags |= FILTER_NONZERO_BACK;
        ...
        /*    if (!(flags & FILTER_NONZERO_BORDER) &&
            ...
            if (new_run) {
                ...
                current_info->bm = current_info->bm_o = current_info->bm_oo = current_info->bm_s = current_info->bm_bg = NULL;
            ...
            ASS_Vector pos, pos_o, pos_oo, pos_bg;
            ...
            get_bitmap_glyph(render_priv, info, &current_info->leftmost_x, &pos, &pos_o, &pos_oo,&pos_bg,
                             &offset, !current_info->bitmap_count, flags);

            if (!info->bm && !info->bm_o && !info->bm_oo && !info->bm_bg) {
                ass_cache_dec_ref(info->bm);
                ass_cache_dec_ref(info->bm_o);
                ass_cache_dec_ref(info->bm_oo);
                ass_cache_dec_ref(info->bm_bg);
            ...
            current_info->bitmaps[current_info->bitmap_count].bm_oo = info->bm_oo;
            current_info->bitmaps[current_info->bitmap_count].bm_bg = info->bm_bg;
            current_info->bitmaps[current_info->bitmap_count].pos   = pos;
            current_info->bitmaps[current_info->bitmap_count].pos_o = pos_o;
            current_info->bitmaps[current_info->bitmap_count].pos_oo = pos_oo;
            current_info->bitmaps[current_info->bitmap_count].pos_bg = pos_bg;
    ...
    for (int i = 0; i < nb_bitmaps; i++) {
        ...
        for (int j = 0; j < info->bitmap_count; j++) {
            ...
            info->bitmaps[j].pos_oo.x -= info->x;
            info->bitmaps[j].pos_oo.y -= info->y;
            info->bitmaps[j].pos_bg.x -= info->x;
            info->bitmaps[j].pos_bg.y -= info->y;
        }
        ...
        if (val->bm_o.buffer)
            info->bm_o = &val->bm_o;
        if (val->bm_oo.buffer)
            info->bm_oo = &val->bm_oo;
        if (val->bm_s.buffer)
            info->bm_s = &val->bm_s;
        if (val->bm_bg.buffer)
            info->bm_bg = &val->bm_bg;
        info->image = val;
        continue;
    }
...
size_t ass_composite_construct(void *key, void *value, void *priv)
{
    ...
    ASS_Rect rect, rect_o, rect_oo, rect_bg;
    rectangle_reset(&rect);
    rectangle_reset(&rect_o);
    rectangle_reset(&rect_oo);
    rectangle_reset(&rect_bg);
    size_t n_bm = 0, n_bm_o =0 , n_bm_oo= 0, n_bm_bg=0;
    BitmapRef *last = NULL, *last_o = NULL, *last_oo = NULL, *last_bg= NULL;
    for (int i = 0; i < k->bitmap_count; i++) {
        ...
        if (ref->bm_oo) {
            rectangle_combine(&rect_oo, ref->bm_oo, ref->pos_oo);
            last_oo = ref;
            n_bm_oo++;
        }
        if (ref->bm_bg) {
            rectangle_combine(&rect_bg, ref->bm_bg, ref->pos_bg);
            last_bg = ref;
            n_bm_bg++;
        }
    }
    ...
    if (!bord && n_bm_oo == 1) {
        copy_bitmap(render_priv->engine, &v->bm_oo, last_oo->bm_oo);
        v->bm_oo.left += last_oo->pos_oo.x;
        v->bm_oo.top += last_oo->pos_oo.y;
    }
    else if (n_bm_oo && alloc_bitmap(render_priv->engine, &v->bm_oo,
        rect_oo.x_max - rect_oo.x_min + 2 * bord,
        rect_oo.y_max - rect_oo.y_min + 2 * bord,
        true)) {
        Bitmap* dst = &v->bm_oo;
        dst->left = rect_oo.x_min - bord;
        dst->top = rect_oo.y_min - bord;
        for (int i = 0; i < k->bitmap_count; i++) {
            Bitmap* src = k->bitmaps[i].bm_oo;
            if (!src)
                continue;
            int x = k->bitmaps[i].pos_oo.x + src->left - dst->left;
            int y = k->bitmaps[i].pos_oo.y + src->top - dst->top;
            assert(x >= 0 && x + src->w <= dst->w);
            assert(y >= 0 && y + src->h <= dst->h);
            unsigned char* buf = dst->buffer + y * dst->stride + x;
            render_priv->engine->add_bitmaps(buf, dst->stride,
    ...
    if (!bord && n_bm_bg == 1) {
        copy_bitmap(render_priv->engine, &v->bm_bg, last_bg->bm_bg);
        v->bm_bg.left += last_bg->pos_bg.x;
        v->bm_bg.top += last_bg->pos_bg.y;
    }
    else if (n_bm_bg && alloc_bitmap(render_priv->engine, &v->bm_bg,
        rect_bg.x_max - rect_bg.x_min + 2 * bord,
        rect_bg.y_max - rect_bg.y_min + 2 * bord,
        true)) {

        Bitmap* dst = &v->bm_bg;
        dst->left = rect_bg.x_min - bord;
        dst->top = rect_bg.y_min - bord;
        for (int i = 0; i < k->bitmap_count; i++) {
            Bitmap* src = k->bitmaps[i].bm_bg;
            if (!src)
                continue;
            int x = k->bitmaps[i].pos_bg.x + src->left - dst->left;
            int y = k->bitmaps[i].pos_bg.y + src->top - dst->top;
            assert(x >= 0 && x + src->w <= dst->w);
            assert(y >= 0 && y + src->h <= dst->h);
            unsigned char* buf = dst->buffer + y * dst->stride + x;
            render_priv->engine->add_bitmaps(buf, dst->stride,
    ...
    //else
    //    ass_synth_blur(render_priv->engine, &v->bm_oo, k->filter.be, r2);
    ...
    if (flags & FILTER_NONZERO_SHADOW) {
        if (n_bm_oo) {
            copy_bitmap(render_priv->engine, &v->bm_s, &v->bm_oo);
        ...
        ass_synth_blur(render_priv->engine, &v->bm_s, k->filter.be, r2);
        if (render_priv->state.c[0])
        {
            fix_outline(&v->bm, &v->bm_s);
        }
    ...
    {
        fix_outline(&v->bm_o, &v->bm_oo);
        fix_outline(&v->bm, &v->bm_o);
    }
        

    return sizeof(CompositeHashKey) + sizeof(CompositeHashValue) +
        bitmap_size(&v->bm) + bitmap_size(&v->bm_o)+ bitmap_size(&v->bm_s)+ bitmap_size(&v->bm_oo)+ bitmap_size(&v->bm_bg);
}
...
static GlyphInfo*
ass_calc_event_pos(ASS_Renderer* render_priv, ASS_Event* event, int index, int* lineindex)
{
    Rect pos = {0};
    if (event->Style >= render_priv->track->n_styles) {
        ass_msg(render_priv->library, MSGL_WARN, "No style found");
    ...
    *lineindex = 0;
    ...
    {
        if (render_priv->text_info.glyphs[i].linebreak>0)
        {
            *lineindex= *lineindex+1;
        }
    }
    return render_priv->text_info.glyphs+index;
}

static TextInfo*
ass_calc_textinfo(ASS_Renderer* render_priv, ASS_Event* event, EventImages* event_images, int index)
{
    Rect pos = { 0 };
    if (event->Style >= render_priv->track->n_styles) {
        ass_msg(render_priv->library, MSGL_WARN, "No style found");
...
#if _M_X64
    if (priv->images_root->bitmap == 0xdddddddddddddddd) {
        int sss = 0;
    }
#else
    if (priv->images_root->bitmap == 0xdddddddd) {
        int sss = 0;
    }
#endif
...
Rect ass_cursor_pos(ASS_Renderer* priv, ASS_Track* track,
    long long now, int index)
{
    Rect pos = { 0 };
    // init frame
    if (!ass_start_frame(priv, track, now)) {
    ...
    for (int i = 0; i < track->n_events; i++) {
        ...
        if ((event->Start <= now)
            && (now < (event->Start + event->Duration))) {
            int lineindex = 0;
            GlyphInfo* glyphinfo = ass_calc_event_pos(priv, event, index, &lineindex);
            if (glyphinfo != NULL)
            {
                
                
                pos.x0 = d6_to_int(glyphinfo->pos.x);
                pos.y0 = d6_to_int(glyphinfo->pos.y);// +priv->text_info.lines[0].desc;
                pos.x1 = pos.x0 + d6_to_int(glyphinfo->advance.x);
                pos.y1 = d6_to_int(glyphinfo->pos.y)+priv->text_info.lines[0].asc+ priv->state.line_spacing;            }
        }
    }
...
Rect ass_bound_rect(ASS_Renderer* priv, ASS_Track* track,
    long long now, const char* name)
{
    Rect rect = { 10000,10000,0,0 };
    //
    EventImages event_images;
    // init frame
    if (!ass_start_frame(priv, track, now)) {
    ...
    for (int i = 0; i < track->n_events; i++) {
        ...
        if ((event->Start <= now)
            ...
            if (strcmp(track->events[i].Name, name) == 0)
            {
                TextInfo* textinfo = ass_calc_textinfo(priv, event, &event_images, 0);
                rect.x0 = event_images.left;// +textinfo
                rect.y0 = event_images.top;
                rect.x1 = event_images.left + event_images.width;
                rect.y1 = event_images.top + event_images.height;
            }
