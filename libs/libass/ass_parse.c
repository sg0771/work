...
char *parse_tags(ASS_Renderer *render_priv, char *p, char *end, double pwr,
                 bool nested)
{
    render_priv->state.line_spacing = 0;
...
#define complex_tag(name) mystrcmp(&p, (name))
        ...
        if (tag("linespace")) {
            ...
            if (nargs)
                ...
                val = render_priv->settings.line_spacing;
            render_priv->state.line_spacing = val;
        }
        else if (tag("oo_bord")) {
            double val;
            if (nargs) {
                val = argtod(*args);
                val = render_priv->state.border_oo * (1 - pwr) + val * pwr;
                val = (val < 0) ? 0 : val;
            }
            else
                val = render_priv->state.style->Outline;
            render_priv->state.border_oo = val;
        }
        if (tag("x_bord")) {
            double val;
            if (nargs) {
                val = argtod(*args);
                val = render_priv->state.border_x2 * (1 - pwr) + val * pwr;
                val = (val < 0) ? 0 : val;
            }
            else
                val = render_priv->state.style->Outline;
            render_priv->state.border_x2 = val;
        }
        else if (tag("y_bord")) {
            double val;
            if (nargs) {
                val = argtod(*args);
                val = render_priv->state.border_y2 * (1 - pwr) + val * pwr;
                val = (val < 0) ? 0 : val;
            }
            else
                val = render_priv->state.style->Outline;
            render_priv->state.border_y2 = val;
        }
        if (tag("xbord")) {
            double val;
            if (nargs) {
            ...
            if (nargs) {
                ...
                change_alpha(&render_priv->state.c[4],
                    _a(render_priv->state.style->OutlineColour2), 1);
                change_alpha(&render_priv->state.c[5],
                    _a(render_priv->state.style->BackColour2), 1);
            }
            // FIXME: simplify
        } else if (tag("an")) {
            int val = argtoi(*args);
        ...
        if (nargs) {
            ...
            change_color(&render_priv->state.c[4],
                render_priv->state.style->OutlineColour2, 1);
        }
        else if (tag("6c")) {
        if (nargs) {
            uint32_t val = parse_color_tag(args->start);
            change_color(&render_priv->state.c[5], val, pwr);
        }
        else
            change_color(&render_priv->state.c[5],
                render_priv->state.style->BackColour2, 1);
        }
        else if (tag("1a")) {
            if (nargs) {
        ...
        if (nargs) {
            uint32_t val = parse_alpha_tag(args->start);
            change_alpha(&render_priv->state.c[5], val, pwr);
        }
        else
            change_alpha(&render_priv->state.c[5],
                _a(render_priv->state.style->BackColour2), 1);
    ...
    //render_priv->state.border_y2 = 40;
    //render_priv->state.border_x2 = 40;
    return p;
}
