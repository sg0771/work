...
#define BITMAP_CACHE_MAX_SIZE (16 * MEGABYTE)
...
typedef struct {
    ...
    int x, y;
    Bitmap *bm, *bm_o, *bm_s, *bm_oo, *bm_bg;   // glyphs, outline, shadow bitmaps
...
typedef struct glyph_info {
    ...
    bool starts_new_run;
    uint32_t c[6];              // colors
    uint8_t a_pre_fade[6];      // alpha values before applying fades
...
typedef struct {
    ...
    double border_y;
    double border_x2;            // outline width
    double border_y2;
    double line_spacing;
    double border_oo;
    enum {
    ...
    int border_style;
    uint32_t c[6];              // colors(Primary, Secondary, so on) in RGBA
