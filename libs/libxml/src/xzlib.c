...
xz_avail_zstrm(xz_statep state)
{
    ...
    ret = xz_avail(state);
    state->zstrm.avail_in = (uint32_t) state->strm.avail_in;
    state->zstrm.next_in = (uint8_t *) state->strm.next_in;
    return ret;
}
#endif
...
xz_decomp(xz_statep state)
{
    ...
    do {
        ...
        if (state->how == GZIP) {
            state->zstrm.avail_in = (uint32_t) state->strm.avail_in;
            state->zstrm.next_in = (uint8_t*) state->strm.next_in;
            state->zstrm.avail_out = (uint32_t) state->strm.avail_out;
            state->zstrm.next_out = (uint8_t*) state->strm.next_out;
