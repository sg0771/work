...
static int wxm_probe(const AVProbeData* p)
{
...
static int wxm_same_audio_codec(AVCodecParameters* apar, int flags)
{
    int bits_per_coded_sample = (flags & FLV_AUDIO_SAMPLESIZE_MASK) ? 16 : 8;
    int wxm_codecid = flags & FLV_AUDIO_CODECID_MASK;
    int codec_id;
    ...
    default:
        return apar->codec_tag == (wxm_codecid >> FLV_AUDIO_CODECID_OFFSET);
    }
}

static void wxm_set_audio_codec(AVFormatContext* s, AVStream* astream,
    AVCodecParameters* apar, int wxm_codecid)
{
    switch (wxm_codecid) {
        // no distinction between S16 and S8 PCM codec flags
    case FLV_CODECID_PCM:
    ...
    default:
        avpriv_request_sample(s, "Audio codec (%x)",
            wxm_codecid >> FLV_AUDIO_CODECID_OFFSET);
        apar->codec_tag = wxm_codecid >> FLV_AUDIO_CODECID_OFFSET;
    }
}

static int wxm_same_video_codec(AVCodecParameters* vpar, int flags)
{
    int wxm_codecid = flags & FLV_VIDEO_CODECID_MASK;
    ...
    default:
        return vpar->codec_tag == wxm_codecid;
    }
}

static int wxm_set_video_codec(AVFormatContext* s, AVStream* vstream,
    int wxm_codecid, int read)
{
    int ret = 0;
    ...
    case FLV_CODECID_VP6A:
        if (wxm_codecid == FLV_CODECID_VP6A)
        ...
        ret = 1;     // 1 byte body size adjustment for wxm_read_packet()
        break;
    case FLV_CODECID_H264:
    ...
    case FLV_CODECID_HEVC:
        ...
        break;
    default:
        avpriv_request_sample(s, "Video codec (%x)", wxm_codecid);
        par->codec_tag = wxm_codecid;
    }
            ...
            if (amf_type == AMF_DATA_TYPE_NUMBER ||
                ...
                if (!strcmp(key, "duration"))
                    ...
                    if (!strcmp(key, "videocodecid") && vpar) {
                        int ret = wxm_set_video_codec(s, vstream, num_val, 0);
                        if (ret < 0)
                            return ret;
                    }
                    else if (!strcmp(key, "audiocodecid") && apar) {
                        int id = ((int)num_val) << FLV_AUDIO_CODECID_OFFSET;
                        wxm_set_audio_codec(s, astream, apar, id);
                    }
...
static int wxm_read_metabody(AVFormatContext* s, int64_t next_pos)
{
    WXMContext* flv = s->priv_data;
    AMFDataType type;
...
static int wxm_read_header(AVFormatContext* s)
{
    WXMContext* flv = s->priv_data;
    avio_skip(s->pb, 4);
    int flags = avio_r8(s->pb);
    av_log(s, AV_LOG_ERROR, "FLV flags = %d", flags);
    flv->missing_streams = flags & (FLV_HEADER_FLAG_HASVIDEO | FLV_HEADER_FLAG_HASAUDIO);
    s->ctx_flags |= AVFMTCTX_NOHEADER;
    avio_skip(s->pb, 4);
    int offset = 13; ///avio_rb32(s->pb);//Must be 9, Like FLV...
    avio_seek(s->pb, offset, SEEK_SET);
    int pre_tag_size = avio_rb32(s->pb);
    if (pre_tag_size) {
        av_log(s, AV_LOG_WARNING, "Read FLV header error, input file is not a standard flv format, first PreviousTagSize0 always is 0\n");
    }

    s->start_time = 0;
    flv->sum_wxm_tag_size = 0;
    flv->last_keyframe_stream_index = -1;

    return 0;
}

static int wxm_read_close(AVFormatContext* s)
{
    int i;
    WXMContext* flv = s->priv_data;
...
static int wxm_get_extradata(AVFormatContext* s, AVStream* st, int size)
{
    int ret;
    if (!size)
        return 0;
...
static int wxm_queue_extradata(WXMContext* flv, AVIOContext* pb, int stream,
    int size)
{
    if (!size)
        return 0;
...
static int wxm_data_packet(AVFormatContext* s, AVPacket* pkt,
...
static int wxm_read_packet(AVFormatContext* s, AVPacket* pkt)
{
    WXMContext* flv = s->priv_data;
...
retry:
    ...
    if (type == FLV_TAG_TYPE_AUDIO) {
        ...
        if (size > 13 + 1 + 4) { // Header-type metadata stuff
            ...
            if (type == 0 && dts == 0 || type < 0) {
                ...
                return wxm_data_packet(s, pkt, dts, next);
            }
    ...
    skip:
        if (avio_seek(s->pb, next, SEEK_SET) != next) {
            // This can happen if wxm_read_metabody above read past
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (stream_type == FLV_STREAM_TYPE_AUDIO) {
            if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO &&
                (s->audio_codec_id || wxm_same_audio_codec(st->codecpar, flags)))
                break;
        }
        else if (stream_type == FLV_STREAM_TYPE_VIDEO) {
            if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO &&
                (s->video_codec_id || wxm_same_video_codec(st->codecpar, flags)))
                break;
        }
    ...
    if (stream_type == FLV_STREAM_TYPE_AUDIO) {
        ...
        if (!st->codecpar->codec_id) {
            wxm_set_audio_codec(s, st, st->codecpar,
            ...
            wxm_set_audio_codec(s, st, par, flags & FLV_AUDIO_CODECID_MASK);
            sample_rate = par->sample_rate;
            avcodec_parameters_free(&par);
        }
    }
    else if (stream_type == FLV_STREAM_TYPE_VIDEO) {
        int ret = wxm_set_video_codec(s, st, flags & FLV_VIDEO_CODECID_MASK, 1);
        if (ret < 0)
            return ret;
        size -= ret;
    }
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_AAC ||
        ...
        st->codecpar->codec_id == AV_CODEC_ID_HEVC || //Add for RTMP-H265
        ...
        if (type == 0 && (!st->codecpar->extradata || st->codecpar->codec_id == AV_CODEC_ID_AAC ||
            st->codecpar->codec_id == AV_CODEC_ID_H264
            || st->codecpar->codec_id == AV_CODEC_ID_HEVC //Add For RTMP_H265
            )) {
            AVDictionaryEntry* t;

            if (st->codecpar->extradata) {
                if ((ret = wxm_queue_extradata(flv, s->pb, stream_type, size)) < 0)
                    return ret;
                ret = FFERROR_REDO;
                goto leave;
            }
            if ((ret = wxm_get_extradata(s, st, size)) < 0)
                return ret;
...
leave:
    ...
    if (!flv->trust_datasize) {
        if (last != orig_size + 11 && last != orig_size + 10 &&
            !avio_feof(s->pb) &&
            (last != orig_size || !last) && last != flv->sum_wxm_tag_size &&
            !flv->broken_sizes) {
            av_log(s, AV_LOG_ERROR, "Packet mismatch %d %d %d\n", last, orig_size + 11, flv->sum_wxm_tag_size);
...
static int wxm_read_seek(AVFormatContext* s, int stream_index,
...
#define OFFSET(x) offsetof(WXMContext, x)
#define VD AV_OPT_FLAG_VIDEO_PARAM | AV_OPT_FLAG_DECODING_PARAM
static const AVOption options[] = {
    { "wxm_metadata", "Allocate streams according to the onMetaData array", OFFSET(trust_metadata), AV_OPT_TYPE_BOOL, {.i64 = 0 }, 0, 1, VD },
    { "wxm_full_metadata", "Dump full metadata of the onMetadata", OFFSET(dump_full_metadata), AV_OPT_TYPE_BOOL, {.i64 = 0 }, 0, 1, VD },
    { "wxm_ignore_prevtag", "Ignore the Size of previous tag", OFFSET(trust_datasize), AV_OPT_TYPE_BOOL, {.i64 = 0 }, 0, 1, VD },
    { "missing_streams", "", OFFSET(missing_streams), AV_OPT_TYPE_INT, {.i64 = 0 }, 0, 0xFF, VD | AV_OPT_FLAG_EXPORT | AV_OPT_FLAG_READONLY },
    { NULL }
};
static const AVClass wxm_class = {
    .class_name = "wxmdec",
...
AVInputFormat ff_wxm_demuxer = {
    .name = "wxm",
    .long_name = "ApowerSoft (WXMedia)",
    .priv_data_size = sizeof(WXMContext),
    .read_probe = wxm_probe,
    .read_header = wxm_read_header,
    .read_packet = wxm_read_packet,
    .read_seek = wxm_read_seek,
    .read_close = wxm_read_close,
    .extensions = "wxm",
    .priv_class = &wxm_class,
};
