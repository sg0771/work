...
bool CVobSubFile::ReadIdx(CString fn, int& ver)
{
	...
	for (ptrdiff_t line = 0; !bError && f.ReadString(str); line++) {
		...
		if (entry == L"size") {
			...
			RGBQUAD pal[4];
			if (swscanf_s(str, L"%x,%x,%x,%x",
				(unsigned int*)&pal[0], (unsigned int*)&pal[1], (unsigned int*)&pal[2], (unsigned int*)&pal[3]) != 4) {
				bError = true;
				continue;
			}
...
bool CVobSubFile::WriteIdx(CString fn)
{
	...
	str.Format(L"custom colors: %ws, tridx: %04x, colors: %06x, %06x, %06x, %06x\n\n",
	...
	for (size_t i = 0; i < std::size(m_langs); i++) {
		...
		for (size_t j = 0; j < sp.size(); j++) {
			...
			str.Format(L"timestamp: %ws%02d:%02d:%02d:%03d, filepos: %09I64x\n",
					   sp[j].start < 0 ? L"-" : L"",
...
bool CVobSubFile::SaveWinSubMux(CString fn)
{
	...
	std::unique_ptr<BYTE[]> p4bpp(new BYTE[720*576/2]);
	if (!p4bpp) {
		return false;
	}
	...
	for (size_t i = 0; i < sp.size(); i++) {
		...
		CString bmpfn;
		bmpfn.Format(L"%ws_%06u.bmp", fn.GetBuffer(), i+1);

		CString str;
		str.Format(L"%ws\t%02d:%02d:%02d:%02d %02d:%02d:%02d:%02d\t%03d %03d %03d %03d %d %d %d %d\n",
				   bmpfn.GetBuffer(),
...
bool CVobSubFile::SaveScenarist(CString fn)
{
	...
	std::unique_ptr<BYTE[]> p4bpp(new BYTE[(m_size.cy - 2) * 360]);
	if (!p4bpp) {
		return false;
	}
	...
	for (size_t i = 0, k = 0; i < sp.size(); i++) {
		...
		CString bmpfn;
		bmpfn.Format(L"%ws_%04u.bmp", fn.GetBuffer(), i+1);
		...
		str.Format(L"%04u\t%02d:%02d:%02d:%02d\t%02d:%02d:%02d:%02d\t%ws\n",
				   ++k,
				   h1, m1, s1, f1,
				   h2, m2, s2, f2,
				   title.GetBuffer());
...
bool CVobSubFile::SaveMaestro(CString fn)
{
	...
	for (size_t i = 0, k = 0; i < sp.size(); i++) {
		...
		CString bmpfn;
		bmpfn.Format(L"%ws_%04d.bmp", fn.GetBuffer(), i+1);
		title = bmpfn.Mid(bmpfn.ReverseFind('/')+1);
...
{
	DSAutoLock cAutoLock(&m_csSubPics);
		...
		if (key == L"size") {
			...
			if (sl.GetCount() == 3) {
				...
				if (tridx.RemoveHead() == L"tridx") {
					WCHAR tr[4];
					swscanf_s(tridx.RemoveHead(), L"%c%c%c%c", &tr[0], (unsigned int)sizeof(WCHAR),
							   &tr[1], (unsigned int)sizeof(WCHAR), &tr[2], (unsigned int)sizeof(WCHAR), &tr[3], (unsigned int)sizeof(WCHAR));
