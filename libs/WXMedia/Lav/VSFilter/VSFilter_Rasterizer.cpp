	...
	short* kernel, int kernel_size, int divisor)
{
	...
	int* tmp = (int*)av_malloc(stride * sizeof(int));
...
{
	Edge* pNewEdgeBuffer = (Edge*)av_realloc(mpEdgeBuffer, sizeof(Edge) * edges);
	if (pNewEdgeBuffer) {
...
bool Rasterizer::EndPath(HDC hdc)
{
	...
	if (::EndPath(hdc)) {
		...
		mpPathTypes = (BYTE*)av_malloc(sizeof(BYTE) * mPathPoints);
		mpPathPoints = (POINT*)av_malloc(sizeof(POINT) * mPathPoints);
...
bool Rasterizer::PartialEndPath(HDC hdc, long dx, long dy)
{
	...
	if (::EndPath(hdc)) {
		...
		pNewTypes = (BYTE*)av_realloc(mpPathTypes, (mPathPoints + nPoints) * sizeof(BYTE));
		pNewPoints = (POINT*)av_realloc(mpPathPoints, (mPathPoints + nPoints) * sizeof(POINT));
...
bool Rasterizer::ScanConvert()
{
	try {
		...
		mEdgeNext = 1;
		mEdgeHeapSize = 2048;
		mpEdgeBuffer = (Edge*)av_malloc(sizeof(Edge)*mEdgeHeapSize);
		if (!mpEdgeBuffer) {
		...
		mpScanBuffer = new unsigned int[m_pOutlineData->mHeight];
...
void Rasterizer::CreateWidenedRegionFast(const int ry)
{
	...
	int yPrec = UINT(m_pOutlineData->mOutline.front().first >> 32);
	POSITION pos = centerGroups.GetHeadPosition();
...
bool Rasterizer::Rasterize(int xsub, int ysub, int fBlur, double fGaussianBlur)
{
	...
	m_pOverlayData->mpOverlayBufferBody	= (byte*)av_malloc(m_pOverlayData->mOverlayPitch * m_pOverlayData->mOverlayHeight);
	m_pOverlayData->mpOverlayBufferBorder = (byte*)av_malloc(m_pOverlayData->mOverlayPitch * m_pOverlayData->mOverlayHeight);
	...
	if (fGaussianBlur > 0) {
		...
		if (m_pOverlayData->mOverlayWidth >= filter.width && m_pOverlayData->mOverlayHeight >= filter.width) {
			...
			byte *tmp = (byte*)av_malloc(pitch * m_pOverlayData->mOverlayHeight * sizeof(byte));
			if (!tmp) {
				return false;
			}
	...
	for (int pass = 0; pass < fBlur; pass++) {
		if (m_pOverlayData->mOverlayWidth >= 3 && m_pOverlayData->mOverlayHeight >= 3) {
			...
			byte* tmp = new byte[pitch * m_pOverlayData->mOverlayHeight];
			if (!tmp) {
				return false;
			}
