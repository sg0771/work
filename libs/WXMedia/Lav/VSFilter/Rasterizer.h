...
struct COverlayData {
	...
	COverlayData(const COverlayData& overlayData)
		...
		if (mOverlayPitch > 0 && mOverlayHeight > 0) {
			mpOverlayBufferBody = (byte*)av_malloc(mOverlayPitch * mOverlayHeight);
			mpOverlayBufferBorder = (byte*)av_malloc(mOverlayPitch * mOverlayHeight);
	...
	void DeleteOverlay() {
		if (mpOverlayBufferBody) {
			av_free(mpOverlayBufferBody);
			mpOverlayBufferBody = nullptr;
		}
		if (mpOverlayBufferBorder) {
			av_free(mpOverlayBufferBorder);
			mpOverlayBufferBorder = nullptr;
		}
	}
};
