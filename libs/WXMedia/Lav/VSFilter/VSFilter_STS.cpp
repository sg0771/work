...
static bool OpenSubRipper(CTextFile* file, CSimpleTextSubtitle& ret, int CharSet)
{
	...
	while (file->ReadString(buff)) {
		...
		int c = swscanf_s(buff, L"%d%c%d%c%d%4[^-] --> %d%c%d%c%d%4s\n",
			&hh1, &sep, 1, &mm1, &sep, 1, &ss1, msStr1, (unsigned int)std::size(msStr1),
			&hh2, &sep, 1, &mm2, &sep, 1, &ss2, msStr2, (unsigned int)std::size(msStr2));
	...
	while (file->ReadString(buff)) {
		...
		if (buff[0] == L'[') {
			for (int i = 0; i < buff.GetLength() && buff[i] == L'['; ) {
				...
				if (tag == L"font") {
					_wcsicmp(def.fontNameW.c_str(), param) == 0 ? font.SetString(param) : font.Empty();
				}
				else if (tag == L"colf") {
		...
		int c = swscanf_s(buff, L"%d:%d:%d%c%d,%d:%d:%d%c%d\n",
			&hh1, &mm1, &ss1, &sep, (unsigned int)sizeof(WCHAR),
			&hs1, &hh2, &mm2, &ss2, &sep, (unsigned int)sizeof(WCHAR), &hs2);
		...
		for (int i = 0, j, k, len = line.GetLength(); i < len; i++) {
...

			{
				...
				if (!_wcsnicmp(code, L"{c:$", 4)) {
					...
					code.Format(L"{\\fn%ws}", code.Mid(3).GetBuffer());
					ret += code;
				}
				else if (!_wcsnicmp(code, L"{s:", 3)) {
...
static bool LoadFont(const CString& font)
{
	...
	if (hFont == INVALID_HANDLE_VALUE) {
		...
		CString fn;
		fn.Format(L"%wsfont%08lx.ttf", path, chksum);
		...
		if (entry == L"dialogue") {
			...
			if (styles) {
				...
				try {
					...
					style->fontNameW = GetStrW(pszBuff, nBuffLength);
...
void CSimpleTextSubtitle::ChangeUnknownStylesToDefault()
{
	...
	for (size_t i = 0; i < GetCount(); i++) {
		...
		STSStyle* val;
		if (!m_styles.Lookup(stse.style.c_str(), val)) {
			if (!unknown.Lookup(stse.style.c_str(), val)) {
				if (fReport) {
					CString msg;
					msg.Format(L"Unknown style found: \"%ws\", changed to \"Default\"!\n\nPress Cancel to ignore further warnings.", stse.style.c_str());
					if (MessageBoxW(NULL, msg, L"Warning", MB_OKCANCEL | MB_ICONWARNING | MB_SYSTEMMODAL) != IDOK) {
						fReport = false;
					}
				}

				unknown[stse.style.c_str()] = NULL;
			}
...
void CSimpleTextSubtitle::AddStyle(CString name, STSStyle* style)
{
	...
	if (m_styles.Lookup(name, val)) {
		...
		CString name3;
		do {
			name3.Format(L"%ws%d", name2.GetBuffer(), idx);
			idx++;
...
{
	STSStyle* style = NULL;
	m_styles.Lookup(GetAt(i).style.c_str(), style);

	if (!style) {
		m_styles.Lookup(L"Default", style);

		style->SetDefault();//Add By Tam
	}
...
bool CSimpleTextSubtitle::GetStyle(int i, STSStyle& stss)
{
	...
	defstyle->SetDefault();//Add By Tam
...
void CSimpleTextSubtitle::ConvertUnicode(int i, bool fUnicode)
{
	...
	if (stse.fUnicode ^ fUnicode) {
		...
		stse.str = fUnicode
			? MBCSSSAToUnicode(stse.str.c_str(), CharSet)
			: UnicodeSSAToMBCS(stse.str.c_str(), CharSet);
...
bool CSimpleTextSubtitle::SaveAs(CString fn, Subtitle::SubType type, double fps, int delay, CTextFile::enc e, bool bCreateExternalStyleFile)
{
	...
	if (type == Subtitle::SMI) {
		...
		str = (type == Subtitle::SSA)
			? L"Style: %ws,%ws,%d,&H%06x,&H%06x,&H%06x,&H%06x,%d,%d,%d,%.2f,%.2f,%d,%d,%d,%d,%d,%d\n"
			: L"Style: %ws,%ws,%d,&H%08x,&H%08x,&H%08x,&H%08x,%d,%d,%d,%d,%.2f,%.2f,%.2f,%.2f,%d,%.2f,%.2f,%d,%d,%d,%d,%d\n";
		...
		while (pos) {
			...
			if (type == Subtitle::SSA) {
				str2.Format(str, key.GetBuffer(),
					s->fontNameW.c_str(), (int)s->fontSize,
					s->colors[0] & 0xffffff,
					s->colors[1] & 0xffffff,
					s->colors[2] & 0xffffff,
					s->colors[3] & 0xffffff,
	...
	CStringW fmt =
		...
		type == Subtitle::SUB ? L"{%d}{%d}%ws\n" :
		type == Subtitle::SMI ? L"<SYNC Start=%d><P Class=UNKNOWNCC>\n%ws\n<SYNC Start=%d><P Class=UNKNOWNCC>&nbsp;\n" :
		type == Subtitle::PSB ? L"{%d:%02d:%02d}{%d:%02d:%02d}%ws\n" :
		type == Subtitle::SSA ? L"Dialogue: Marked=0,%d:%02d:%02d.%02d,%d:%02d:%02d.%02d,%ws,%ws,%04d,%04d,%04d,%ws,%ws\n" :
		type == Subtitle::ASS ? L"Dialogue: %d,%d:%02d:%02d.%02d,%d:%02d:%02d.%02d,%ws,%ws,%04d,%04d,%04d,%ws,%ws\n" :
		L"";
	//	Sort(true);
	...
	for (int i = 0, j = (int)GetCount(), k = 0; i < j; i++) {
		...
		if (type == Subtitle::SRT) {
			str2.Format(fmt, i - k + 1, hh1, mm1, ss1, ms1, hh2, mm2, ss2, ms2, str.GetBuffer());
		}
		else if (type == Subtitle::SUB) {
			str.Replace(L'\n', L'|');
			str2.Format(fmt, int(t1 * fps / 1000), int(t2 * fps / 1000), str.GetBuffer());
		}
		else if (type == Subtitle::SMI) {
			str.Replace(L"\n", L"<br>");
			str2.Format(fmt, t1, str.GetBuffer(), t2);
		}
		else if (type == Subtitle::PSB) {
			str.Replace(L'\n', L'|');
			str2.Format(fmt, hh1, mm1, ss1, hh2, mm2, ss2, str.GetBuffer());
		}
	...
	if (bCreateExternalStyleFile && !m_fUsingAutoGeneratedDefaultStyle && m_styles.Lookup(L"Default", s) && type != Subtitle::SSA && type != Subtitle::ASS) {
		...
		str = L"Style: Default,%ws,%d,&H%08x,&H%08x,&H%08x,&H%08x,%d,%d,%d,%d,%.2f,%.2f,%.2f,%.2f,%d,%.2f,%.2f,%d,%d,%d,%d,%d\n";
		str2.Format(str,
			s->fontNameW.c_str(), (int)s->fontSize,
			(s->colors[0] & 0xffffff) | (s->alpha[0] << 24),
			(s->colors[1] & 0xffffff) | (s->alpha[1] << 24),
...
void STSStyle::SetDefault()
{
	...
	int nFontPos = WXGetSubtitleFontPos();
	if (nFontPos <= 50) {
		int targetBottom = SUB_HEIGHT * nFontPos / 100;
		if (targetBottom < 2)
			targetBottom = 2;
		this->marginRect.bottom = targetBottom;
		this->scrAlignment = 2;
	}
	else {
		int targetTop = 288 * nFontPos / 100;
		if (targetTop < 2)
			targetTop = 2;
		this->marginRect.top = 288 - targetTop;
		this->scrAlignment = 8;
	}
	...
	shadowDepthX =
		shadowDepthY = 3;
	colors[0] = WXGetSubtitleFontColor();
	colors[1] = 0x0000FFFF;
	colors[2] = 0x00000000;
	colors[3] = 0x00000000;
	alpha[0] = 0x00;
	...
	fontNameW = WXGetSubtitleFontName();
	fontSize = WXGetSubtitleFontSize();
	fontScaleX =
		fontScaleY = 100;
	fontSpacing = 0;
	fontWeight = FW_BOLD;

	fItalic = WXGetSubtitleFontItalic(); // 斜体
	fUnderline = WXGetSubtitleFontUnderLine();//下划线
	fStrikeOut = WXGetSubtitleFontStrikeOut();//删除线
...
{
	return (marginRect.left == s.marginRect.left &&
		marginRect.right == s.marginRect.right &&
		marginRect.bottom == s.marginRect.bottom &&
		marginRect.top == s.marginRect.top
...
LOGFONTA& operator <<= (LOGFONTA& lfa, STSStyle& s)
{
	...
	CStringA sa = s.fontNameW.c_str();//wchar_t to char 
	strncpy_s(lfa.lfFaceName, LF_FACESIZE, sa, _TRUNCATE);
	HDC hDC = GetDC(0);
...
LOGFONTW& operator <<= (LOGFONTW& lfw, STSStyle& s)
{
	...
	wcsncpy_s(lfw.lfFaceName, LF_FACESIZE, s.fontNameW.c_str(), _TRUNCATE);
	HDC hDC = GetDC(0);
...
std::wstring& operator <<= (std::wstring& stylec, const STSStyle& s)
{
	CString style;
	style.Format(L"%d;%d;%d;%d;%d;%d;%f;%f;%f;%f;0x%06lx;0x%06lx;0x%06lx;0x%06lx;0x%02x;0x%02x;0x%02x;0x%02x;%d;%ws;%f;%f;%f;%f;%ld;%d;%d;%d;%d;%f;%f;%f;%f;%d",
...
STSStyle& operator <<= (STSStyle& s, const std::wstring& style)
{
	s.SetDefault();

	try {
		CStringW str = style.c_str();
		LPCWSTR pszBuff = str;
		...
		if (str.Find(';') >= 0) {
			...
			s.fontNameW = GetStrW(pszBuff, nBuffLength, L';');
