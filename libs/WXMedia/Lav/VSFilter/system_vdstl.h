...
public:
	...
	pointer allocate(size_type n, void *p_close = 0) {
		...
		pointer p = (pointer)av_malloc(n*sizeof(T));
...
public:
	...
	vdstructex(const T *pStruct, size_t len) : mSize(len), mpMemory((T*)av_malloc(len)) {
		memcpy(mpMemory, pStruct, len);
	}

	vdstructex(const vdstructex<T>& src) : mSize(src.mSize), mpMemory((T*)av_malloc(src.mSize)) {
	...
	void resize(size_type len) {
		if (mSize != len)
			mpMemory = (T *)av_realloc(mpMemory, mSize = len);
	}
