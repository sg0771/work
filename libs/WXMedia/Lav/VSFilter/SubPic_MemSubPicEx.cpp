...
STDMETHODIMP CMemSubPicEx::Unlock(RECT* pDirtyRect)
{
	...
	int w = m_rcDirty.right - m_rcDirty.left;
	int h = m_rcDirty.bottom - m_rcDirty.top;
...
STDMETHODIMP CMemSubPicEx::AlphaBlt(RECT* pSrc, RECT* pDst, SubPicDesc* pTarget)
{
	ASSERT(pTarget);
...
bool CMemSubPicExAllocator::Alloc(bool fStatic, ISubPic** ppSubPic)
{
	if (!ppSubPic) {
		return false;
	}
