...
HRESULT CSubtitleInputPin::CompleteConnect(IPin* pReceivePin)
{
	...
	if (m_mt.majortype == MEDIATYPE_Text) {
		if (!(m_pSubStream = new CRenderedTextSubtitle(m_pSubLock))) {
			return E_FAIL;
		}
		CRenderedTextSubtitle* pRTS = (CRenderedTextSubtitle*)m_pSubStream.p;
		pRTS->SetName(CString(GetPinName(pReceivePin).c_str()) + L" (embeded)");
		...
		if (m_mt.subtype == MEDIASUBTYPE_XSUB) {
			name = CString(GetPinName(pReceivePin).c_str());
		} else {
			if (psi != NULL) {
				dwOffset = psi->dwOffset;

				name = ISO6392ToLanguage(psi->IsoLang).c_str();
				CStringW lng(psi->IsoLang);
				if (lng.CompareNoCase(name.Left(lng.GetLength())) != 0) {
					name.AppendFormat(L" (%ws)", lng.GetBuffer());
				}
				lcid = VS_ISO6392ToLcid(psi->IsoLang);
		...
		if (m_mt.subtype == MEDIASUBTYPE_UTF8
			...
			if (!(m_pSubStream = new CVobSubStream(m_pSubLock))) {
				return E_FAIL;
			}
			CVobSubStream* pVSS = (CVobSubStream*)m_pSubStream.p;
			pVSS->Open(name, m_mt.pbFormat + dwOffset, m_mt.cbFormat - dwOffset);
		} else if (IsHdmvSub(&m_mt)) {
			if (!(m_pSubStream = new CRenderedHdmvSubtitle(m_pSubLock, (m_mt.subtype == MEDIASUBTYPE_DVB_SUBTITLES) ? ST_DVB : ST_HDMV, name, lcid))) {
				return E_FAIL;
			}
			...
			CStringA hdr = VobSubDefHeader(x ? x : 720, y ? y : 576).c_str();
			...
			if (!(m_pSubStream = new CXSUBSubtitle(m_pSubLock, name, lcid, size))) {
				return E_FAIL;
			}
		}
	}
...
STDMETHODIMP CSubtitleInputPin::NewSegment(REFERENCE_TIME tStart, REFERENCE_TIME tStop, double dRate)
{
	...
	if (m_mt.majortype == MEDIATYPE_Text
			...
			&& (m_mt.subtype == MEDIASUBTYPE_UTF8
				...
				|| m_mt.subtype == MEDIASUBTYPE_ASS2
				|| m_mt.subtype == MEDIASUBTYPE_WEBVTT))) {
		DSAutoLock cAutoLock2(m_pSubLock);
		CRenderedTextSubtitle* pRTS = (CRenderedTextSubtitle*)m_pSubStream.p;
		pRTS->RemoveAll();
		pRTS->CreateSegments();
...
STDMETHODIMP CSubtitleInputPin::Receive(IMediaSample* pSample)
{
	...
	if ((tStart == INVALID_TIME || tStop == INVALID_TIME) && !IsHdmvSub(&m_mt)) {
		...
		{
			std::unique_lock<std::mutex> lock(m_mutexQueue);
			m_sampleQueue.emplace_back(new SubtitleSample(tStart, tStop, pData, (size_t)len));
			lock.unlock();
			m_condQueueReady.notify_one();
		}
	}
...
{
	VS_SetThreadName(DWORD_MAX, "Subtitle Input Pin Thread");
	...
	while(!m_bExitDecodingThread) {
		...
		if (!needStopProcessing()) {
			DSAutoLock cAutoLock(m_pSubLock);
	...
	if (m_mt.majortype == MEDIATYPE_Text) {
		...
		if (m_mt.subtype == MEDIASUBTYPE_UTF8 || m_mt.subtype == MEDIASUBTYPE_WEBVTT) {
			...
			if (!str.IsEmpty()) {
				...
				if (!stse.str.empty()) {
					pRTS->Add(stse.str.c_str(), true, (int)(tStart / 10000), (int)(tStop / 10000),
							  stse.style.c_str(), stse.actor.c_str(), stse.effect.c_str(), stse.marginRect, stse.layer, stse.readorder);
					bInvalidate = true;
				}
			}
		} else if (m_mt.subtype == MEDIASUBTYPE_VOBSUB
