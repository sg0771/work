...
bool CWord::CreateOpaqueBox()
{
	...
	try {
		m_pOpaqueBox = new CPolygon(style, str, 0, 0, 0, 1.0, 1.0, 0, m_renderingCaches);
...
void CWord::Transform(const CPoint &org ) // Transform_SSE2
{
	...
	for (ptrdiff_t i = 0; i < mPathPointsD4 + 1; i++) {
		...
		int x_vals[4], y_vals[4];

		// 将 __m128i 向量存储到数组（内存）中
		_mm_store_si128((__m128i*)x_vals, __pointxRounded);
		_mm_store_si128((__m128i*)y_vals, __pointyRounded);

		if (i == mPathPointsD4) { // last cycle
			for (int k = 0; k < mPathPointsM4; k++) {
				mpPathPoints[i * 4 + k].x = x_vals[3 - k];
				mpPathPoints[i * 4 + k].y = y_vals[3 - k];
			}
		} else {
			for (int k = 0; k < 4; k++) {
				mpPathPoints[i * 4 + k].x = x_vals[3 - k];
...
bool CPolygon::CreatePath()
{
	...
	if (mPathPoints != len) {
		BYTE* pNewPathTypes = (BYTE*)av_realloc(mpPathTypes, len * sizeof(BYTE));
		if (!pNewPathTypes) {
			return false;
		}
		mpPathTypes = pNewPathTypes;
		POINT* pNewPathPoints = (POINT*)av_realloc(mpPathPoints, len * sizeof(POINT));
		if (!pNewPathPoints) {
			return false;
		}
...
CWord* CClipper::Copy()
{
	return new CClipper(m_str, m_size, m_scalex, m_scaley, m_inverse, m_cpOffset, m_renderingCaches);
}
...
void CRenderedTextSubtitle::ParseString(CSubtitle* sub, CStringW str, STSStyle& style)
{
	...
	for (int i = 0, j = 0, len = str.GetLength(); j <= len; j++) {
		...
		if (i < j) {
			if (CWord* w = new CText(style, str.Mid(i, j - i), m_ktype, m_kstart, m_kend, sub->m_scalex, sub->m_scaley, m_renderingCaches)) {
		...
		if (c == L'\n') {
			if (CWord* w = new CText(style, CStringW(), m_ktype, m_kstart, m_kend, sub->m_scalex, sub->m_scaley, m_renderingCaches)) {
				sub->m_words.AddTail(w);
				m_kstart = m_kend;
			}
		} else if (c == L' ') {
			if (CWord* w = new CText(style, CStringW(c), m_ktype, m_kstart, m_kend, sub->m_scalex, sub->m_scaley, m_renderingCaches)) {
				sub->m_words.AddTail(w);
				m_kstart = m_kend;
			}
		}
...
void CRenderedTextSubtitle::ParsePolygon(CSubtitle* sub, CStringW str, STSStyle& style)
{
	...
	if (CWord* w = new CPolygon(style, str, m_ktype, m_kstart, m_kend,
...
bool CRenderedTextSubtitle::ParseSSATag(SSATagsList& tagsList, const CStringW& str)
{
	...
	tagsList.reset(new CAtlList<SSATag>());
	...
	while (pos) {
		...
		switch (tag.cmd) {
			...
			case SSA_fe:
				...
				break;
			case SSA_fn:
				style.fontNameW = (!tag.params.IsEmpty() && !tag.params[0].IsEmpty() && tag.params[0] != L"0" && !bUseOriginal)
								 ? (LPCWSTR)CString(tag.params[0]).Trim()
								 : org.fontNameW;
				break;
			case SSA_frx:
...
CSubtitle* CRenderedTextSubtitle::GetSubtitle(int entry)
{
	...
	try {
		sub = new CSubtitle(m_renderingCaches);
	} catch (CMemoryException* e) {
		e->Delete();
		return NULL;
	}
	...
	m_nPolygon = 0;
	m_polygonBaselineOffset = 0;
	ParseEffect(sub, GetAt(entry).effect.c_str());
...
STDMETHODIMP CRenderedTextSubtitle::GetStreamInfo(int iStream, WCHAR** ppName, LCID* pLCID)
{
	...
	if (ppName) {
		*ppName = (WCHAR*)CoTaskMemAlloc((m_name.GetLength() + 1) * sizeof(WCHAR));
		...
		if (pLCID) {
			*pLCID = VS_ISO6391ToLcid(CW2A(*ppName));
			if (*pLCID == 0) {
				*pLCID = VS_ISO6392ToLcid(CW2A(*ppName));
			}
		}
	}
