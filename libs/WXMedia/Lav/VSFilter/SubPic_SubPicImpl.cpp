...
STDMETHODIMP CSubPicImpl::GetSourceAndDest(
	...
	BOOL bPositionRelative, POINT ShiftPos,
	int xOffsetInPixels, const BOOL bUseSpecialCase) const
{
	...
	if (m_size.cx > 0 && m_size.cy > 0) {
		...
		const SIZE szTarget = rcTarget.Size();
		...
		*pRcSource = rcTemp;
		CPoint pt = m_virtualTextureTopLeft;
		rcTemp.OffsetRect(pt + CPoint(xOffsetInPixels, 0));
...
STDMETHODIMP CSubPicAllocatorImpl::GetStatic(ISubPic** ppSubPic)
{
	...
	{
		DSAutoLock cAutoLock(&m_staticLock);
