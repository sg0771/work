...
)
{
    if (0 != cbProperties) {
        ...
        Props.cbData = std::min(cbProperties, (DWORD)sizeof(Props));
...
HRESULT CDynamicOutputPin::AsynchronousBlockOutputPin(HANDLE hNotifyCallerPinBlockedEvent)
{
    ...
    DSAutoLock alBlockStateLock(&m_BlockStateLock);
...
CMemAllocator::Alloc(void)
{
    ...
    */
    ...
    //                MEM_COMMIT,
    //                PAGE_READWRITE);
    //m_pBuffer = (PBYTE)av_malloc(lToAllocate); //Add By Tam
    ...
    for (; m_lAllocated < m_lCount; m_lAllocated++/*, pNext += lAlignedSize*/) {
        ...
        LPBYTE buffer = (LPBYTE)av_malloc(lAlignedSize);
        if (buffer == NULL) {
               return E_OUTOFMEMORY;
        }
        m_arrBuffer.push_back(buffer);
        pSample = new CMediaSample(
            NAME("Default memory media sample"),
            this,
            &hr,
            buffer/*pNext + m_lPrefix*/,      // GetPointer() value
...
CMemAllocator::ReallyFree(void)
{
    ...
    //    av_free(m_pBuffer); //Add By Tam
    //    m_pBuffer = NULL;
    //}
    if (m_arrBuffer.size() > 0) {
        for (size_t i = 0; i < m_arrBuffer.size(); i++)
        {
            LPBYTE buffer = m_arrBuffer[i];
            av_free(buffer);
        }
        m_arrBuffer.clear();
    }
