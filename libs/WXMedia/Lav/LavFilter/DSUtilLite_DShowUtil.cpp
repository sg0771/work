...
#include "DSUtilLite_DSUtilLiteImpl.h"
...
//在 IFilterGraph 上查找 IBaseFilter
...
//在 IFilterGraph 上查找指定GUID的IBaseFilter
...
//在 IFilterGraph 上查找指定GUID和clsidSubtype 的IBaseFilter
...
void getExtraData(const BYTE *format, const GUID *formattype, const size_t formatlen, BYTE *extra, size_t *extralen)
{
  ...
  if (*formattype == FORMAT_WaveFormatEx) {
    ...
    extralength = std::min(extralength, formatlen - sizeof(VORBISFORMAT2));
    ...
    extralength   = std::min(mp1vi->cbSequenceHeader, (DWORD)(formatlen - FIELD_OFFSET(MPEG1VIDEOINFO, bSequenceHeader[0])));
  } else if (*formattype == FORMAT_MPEG2Video) {
    MPEG2VIDEOINFO *mp2vi = (MPEG2VIDEOINFO *)format;
    extraposition = (BYTE *)mp2vi->dwSequenceHeader;
    extralength   = std::min(mp2vi->cbSequenceHeader, (DWORD)(formatlen - FIELD_OFFSET(MPEG2VIDEOINFO, dwSequenceHeader[0])));
