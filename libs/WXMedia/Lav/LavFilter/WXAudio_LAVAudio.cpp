...
#include "DSUtilLite_DeCSSInputPin.h"
  ...
  : CTransformFilter(NAME("lavc audio decoder"), 0, __uuidof(CLAVAudio))
{
  ...
  m_pInput = new CDeCSSTransformInputPin((WCHAR*)TEXT("CDeCSSTransformInputPin"), this, phr,(LPWSTR) L"Input");
  if(!m_pInput) {
    *phr = E_OUTOFMEMORY;
  }
  if (FAILED(*phr)) {
...
HRESULT CLAVAudio::ReadSettings(HKEY rootKey)
{
  ...
  //  if (SUCCEEDED(hr)) {
  //    memcpy(&m_settings.bFormats, pBuf, std::min(dwVal, (DWORD)sizeof(m_settings.bFormats)));
  //    SAFE_CO_FREE(pBuf);
  //  }
  ...
  //  if (SUCCEEDED(hr)) {
  //    memcpy(&m_settings.bBitstream, pBuf, std::min(dwVal, (DWORD)sizeof(m_settings.bBitstream)));
  //    SAFE_CO_FREE(pBuf);
  //  }
  ...
  //  if (SUCCEEDED(hr)) {
  //    memcpy(&m_settings.bSampleFormats, pBuf, std::min(dwVal, (DWORD)sizeof(m_settings.bSampleFormats)));
  //    SAFE_CO_FREE(pBuf);
  //  }
  ...
  //if (SUCCEEDED(hr)) {
  //  for (int i = 0; i < Codec_AudioNB; ++i) {
  //    const audio_codec_config_t *info = get_codec_config((LAVAudioCodec)i);
...
HRESULT CLAVAudio::ffmpeg_init(AVCodecID codec, const void *format, const GUID format_type, DWORD formatlen)
{
  ...
  for(int i = 0; i < Codec_AudioNB; ++i) {
    const audio_codec_config_t *config = get_codec_config((LAVAudioCodec)i);
    bool bMatched = false;
...
HRESULT CLAVAudio::ProcessBuffer(IMediaSample *pMediaSample, BOOL bEOF)
{
  ...
  consumed = consumed_header + std::min(consumed, buffer_size);
