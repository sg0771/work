...
#include "WXSplitterImpl.h"
#include "WXSplitter.h"
#include "WXSplitter_OutputPin.h"
#include "WXSplitter_InputPin.h"
...
{
  DSAutoLock DSAutoLock(this);

  if (m_pDemuxer)
      m_pDemuxer->AbortOpening();
...
STDMETHODIMP CLAVSplitter::NonDelegatingQueryInterface(REFIID riid, void** ppv)
{
  ...
  *ppv = nullptr;
  ...
  if (m_pDemuxer && (riid == IID_IAMExtendedSeeking || riid == IID_IAMMediaContent || riid == IID_IPropertyBag || riid == __uuidof(IDSMResourceBag))) {
...
// IFileSourceFilter
STDMETHODIMP CLAVSplitter::Load(LPCOLESTR pszFileName, const AM_MEDIA_TYPE * _pmt)
{
  ...
  if(FAILED(hr = m_pDemuxer->Open(pszFileName))) {
      m_strError = m_pDemuxer->GetError();
    SAFE_DELETE(m_pDemuxer);
    return hr;
  }
  m_pDemuxer->AddRef();
...
HRESULT CLAVSplitter::DemuxNextPacket()
{
  Packet *pPacket;
  HRESULT hr = S_OK;
  hr = m_pDemuxer->GetNextPacket(&pPacket);//读取一帧数据
...
STDMETHODIMP CLAVSplitter::RenameOutputPin(DWORD TrackNumSrc, DWORD TrackNumDst, std::deque<CMediaType> pmts)
{
  ...
  if (pPin && pPin->IsConnected()) {
    ...
    if (FAILED(hr)) {
     ...
     mtIdx = QueryAcceptMediaTypes(pPin->GetConnected(), pmts);
    bMediaTypeFound = (mtIdx >= 0);
  ...
  for (const std::string& token : tokenList) {
    ...
    if (found) {
      ...
      if (flags.length() > 0) {
        ...
        std::string _not = res[3];
        if (_not.length() == 1 && _not == "!") {
