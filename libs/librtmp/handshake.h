...
#define HMAC_setup(ctx, key, len)	 {HMAC_CTX_reset(&ctx); HMAC_Init_ex(&ctx, key, (int)len, EVP_sha256(), NULL);}
#define HMAC_close(ctx)	{HMAC_CTX_reset(&ctx);HMAC_CTX_free(&ctx);}

#define HMAC_finish(ctx, dig, dlen)	HMAC_Final(&ctx, dig, &dlen);HMAC_CTX_reset(&ctx);HMAC_CTX_free(&ctx);
#else
