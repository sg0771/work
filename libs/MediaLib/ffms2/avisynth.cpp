...
struct FFMS_IndexItem {
	...
	static std::list<FFMS_IndexItem>& GetList(){
		static std::list<FFMS_IndexItem> s_list_FFMS_Indexs;
...
FFMS_API(FFMS_Index*)FFMS_ProcessIndex(const char* Source, int* Track){
	...
	for(FFMS_IndexItem  item : FFMS_IndexItem::GetList()){
		if (_wcsicmp(item.m_path.c_str(), wstr.c_str()) == 0) {
			*Track = item.Track;
	...
	if (pIndex == nullptr) {
		...
		FFMS_TrackTypeIndexSettings(Indexer, FFMS_TYPE_AUDIO, 1);
		pIndex = FFMS_DoIndexing(Indexer, FFMS_IEH_CLEAR_TRACK, &E);
		...
		if (FFMS_WriteIndex(CacheFile.c_str(), pIndex, &E)) {
			FFMS_DestroyIndex(pIndex);
