...
class AudioEffect : public GenericVideoFilter
{
public:
    AudioEffect(PClip _child, int _effectType)
        : GenericVideoFilter(ConvertAudio::Create(_child, SAMPLE_INT16 | SAMPLE_FLOAT | SAMPLE_INT32, SAMPLE_FLOAT), __FUNCTION__)
        , m_iEffectType(_effectType) {
    }
    virtual ~AudioEffect() {}
    ...
    void __stdcall GetAudio(void* buf, __int64 start, __int64 count, IScriptEnvironment* env) {
        ...
        short* s02data = new short[count];
        short* s04data = new short[count];

        memset(s02data, 0x00, sizeof(short) * count);
        memset(s04data, 0x00, sizeof(short) * count);

        int start02 = vi.SamplesPerSecond() * 0.2;
        int start04 = vi.SamplesPerSecond() * 0.4;
        if (start > start02) {

            child->GetAudio(s02data, start - start02, count, env);

        }
        if (start > start04) {
            child->GetAudio(s04data, start - start04, count, env);
        }
    ...
    static AVSValue __cdecl Create(AVSValue args, void*, IScriptEnvironment* env) {
        if (!args[0].AsClip()->GetVideoInfo().AudioChannels())
            return args[0];
        return new AudioEffect(args[0].AsClip(), args[1].AsInt());
    }
private:
    int m_iEffectType;
};
...
extern const AVSFunction Audio_filters[] = {
    { "Amplify", "cf+", Amplify::Create },//ML_Timeline.cpp 2193
    { "Gips", "cbb", Gips::Create },//ML_Timeline.cpp 357
    { "AudioEffect", "ci", AudioEffect::Create },//ML_Timeline.cpp 352
    { "AssumeSampleRate", "ci", AssumeRate::Create },//unused
    { "Normalize", "c[volume]f[show]b", Normalize::Create },
    { "MixAudio", "cc[clip1_factor]f[clip2_factor]f", MixAudio::Create },//ML_Timeline.cpp 2189
    { "ResampleAudio", "ci[]i", ResampleAudio::Create },//ML_Timeline.cpp 184
    { "ConvertToMono", "c", ConvertToMono::Create },//ML_Timeline.cpp 191
    { "MergeChannels", "c+", MergeChannels::Create },
    { "MonoToStereo", "cc", MergeChannels::Create },//ML_Timeline.cpp 202
    { "GetLeftChannel", "c", GetChannel::Create_left },
    { "GetRightChannel", "c", GetChannel::Create_right },
    { "GetChannel", "ci+", GetChannel::Create_n },
    { "GetChannels", "ci+", GetChannel::Create_n },
    { "KillVideo", "c", KillVideo::Create },
    { "KillAudio", "c", KillAudio::Create },
    { "killaudio", "c", KillAudio::Create },
    { "ConvertAudioTo16bit", "c", ConvertAudio::Create_16bit },//ML_Timeline.cpp 343
...
AssumeRate::AssumeRate(PClip _clip, int _rate)
    : GenericVideoFilter(_clip, __FUNCTION__) {
    if (_rate < 0)
        _rate = 0;
...
ConvertToMono::ConvertToMono(PClip _clip)
    : GenericVideoFilter(ConvertAudio::Create(_clip,
        SAMPLE_INT16 | SAMPLE_FLOAT, SAMPLE_FLOAT), __FUNCTION__) {
...
MergeChannels::MergeChannels(PClip _clip, int _num_children, PClip* _child_array, IScriptEnvironment* env)
    : GenericVideoFilter(_clip, __FUNCTION__), num_children(_num_children), child_array(_child_array) {
...
GetChannel::GetChannel(PClip _clip, int* _channel, int _numchannels)
    : GenericVideoFilter(_clip, __FUNCTION__), channel(_channel), numchannels(_numchannels)
{
...
Normalize::Normalize(PClip _child, float _max_factor, bool _showvalues)
    : GenericVideoFilter(ConvertAudio::Create(_child, SAMPLE_INT16 | SAMPLE_FLOAT, SAMPLE_FLOAT), __FUNCTION__),
...
void __stdcall Normalize::GetAudio(void* buf, __int64 start, __int64 count, IScriptEnvironment* env) {
    ...
    if (vi.SampleType() == SAMPLE_INT16) {
        ...
        for (int i = 0; i < chanXcount; ++i) {
            samples[i] = Saturate_int16((Int32x32To64(samples[i], factor) + 32768) >> 17);
        }
...
void __stdcall MixAudio::GetAudio(void* buf, __int64 start, __int64 count, IScriptEnvironment* env) {
    ...
    if (vi.SampleType() & SAMPLE_INT16) {
        ...
        for (unsigned i = 0; i < unsigned(count) * channels; ++i) {
            samples[i] = Saturate_int16((Int32x32To64(samples[i], track1_factor) + Int32x32To64(clip_samples[i], track2_factor) + 32768) >> 17);
        }
#else
...
void __stdcall ResampleAudio::GetAudio(void* buf, __int64 start, __int64 count, IScriptEnvironment* env) {
    ...
    if (vi.IsSampleType(SAMPLE_INT16)) {
        ...
        last_samples = std::max((long long)overlap, source_samples);                     // Samples for next time
        ...
        last_samples = std::max((long long)overlap, source_samples);
