 ...
 *** Lanczos3 filter ***
 ***********************/
LanczosFilter::LanczosFilter(int t) {
   taps = (double)(std::max( 1, std::min(100,t)));
}
 ...
 *** Blackman filter ***
 ***********************/
BlackmanFilter::BlackmanFilter(int t) {
   taps = (double)(std::max( 1, std::min(100,t)));
   rtaps = 1.0/taps;
}
...
GaussianFilter::GaussianFilter(double p ) {
  param = std::min(100.0, std::max(0.1,p));
}
 ...
 *** Sinc filter ***
 ***********************/
SincFilter::SincFilter(int t) {
   taps = (double)(std::max( 1, std::min(20,t)));
}
 ...
 **/
{
  ...
  double filter_step = std::min(scale, 1.0);
  double filter_support = support() / filter_step;
  int fir_filter_size = int(ceil(filter_support*2));
  int* result = (int*)av_malloc((1 + target_width*(1+fir_filter_size)) * 4);
  ...
  *cur++ = fir_filter_size;
  ...
  for (int i=0; i<target_width; ++i) {
    ...
    *cur++ = start_pos;
    ...
    double ok_pos = std::max(0.0, std::min(original_width-1.0,pos));
 ...
 **/
{
  ...
  int* result = luma ?
                (int*)av_malloc(2*4 + target_width_a*(1+fir_fs_mmx)*8) :
                (int*)av_malloc(2*4 + target_width_a*(1+fir_filter_size)*8);
  ...
  *result = luma ? fir_fs_mmx : fir_filter_size;
  ...
  for (int i=0; i<target_width_a; ++i) {
    ...
    *(cur[ii]) = luma ?   // Write offset of first pixel.
                 (intptr_t)(temp + (start_pos & -2) * 2) :
                 (intptr_t)(temp + start_pos * 8);
