...
public:
  ...
  void __stdcall GetAudio(void* buf, __int64 start, __int64 count, IScriptEnvironment* env);
  bool __stdcall GetParity(int n);
  intptr_t __stdcall SetCacheHints(int cachehints,int frame_range);
 ...
 **/
public:
  AudioDub(PClip child1, PClip child2, PClip child3, PClip child4, PClip child5, int mode, IScriptEnvironment* env);
  virtual ~AudioDub() {
	  if (m_child3)
		  m_child3.~PClip();
	  if (m_child4)
...
private:
  ...
  PClip m_child3, m_child4, m_child5;
  VideoInfo vi;
};
