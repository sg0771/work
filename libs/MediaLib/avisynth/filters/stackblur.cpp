	...
	if (step == 1)
	{
		...
		for (y = minY; y < maxY; y++)
		{
			...
			for (x = 0; x < w; x++)
			{
				...
				dst_ptr[3] = 255;//(sum_a * mul_sum) >> shr_sum;
				dst_ptr += 4;
...
)
{
	...
	else
	{
		...
		for (int i = 0; i < cores; i++)
		{
			...
			//workers[i]->ThreadStart();
		}

		//for (int i = 0; i < cores; i++)
			//workers[i]->ThreadStop();
...
extern const AVSFunction blur_filters[] = {
	{ "StackBlur", "c",StackBlur::Create},  { 0 }
};
 AVSValue __cdecl StackBlur::Create(AVSValue args, void* mode, IScriptEnvironment* env)
{
	 return new StackBlur(args[0].AsClip(),env);
}

StackBlur::StackBlur(PClip _child, IScriptEnvironment* env) : GenericVideoFilter(_child,__FUNCTION__ )
{
	Radius = 100;
}
...
PVideoFrame StackBlur::GetFrame(int n, IScriptEnvironment* env)
{
	...
	BYTE* buf = dst->GetWritePtr();
	env->BitBlt(buf, dst->GetPitch(), src->GetReadPtr(), src->GetPitch(), src->GetRowSize(), src->GetHeight());
	...
	stackblur(buf, vi.width, vi.height, this->Radius);
