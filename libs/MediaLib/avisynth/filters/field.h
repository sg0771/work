...
{
public:
  ComplementParity(PClip _child) : GenericVideoFilter(_child,__FUNCTION__ ) {
...
{
public:
  AssumeParity(PClip _child, bool _parity) : GenericVideoFilter(_child,__FUNCTION__ ), parity(_parity) { 
    if (parity) {
	  vi.Clear(IT_BFF);
	  vi.Set(IT_TFF);
	} else {
...
{
public:
  AssumeFieldBased(PClip _child) : GenericVideoFilter(_child,__FUNCTION__ ) 
  { vi.SetFieldBased(true); vi.Clear(IT_BFF); vi.Clear(IT_TFF); }
  inline bool __stdcall GetParity(int n) 
    { return n&1; }
...
{
public:
  AssumeFrameBased(PClip _child) : GenericVideoFilter(_child,__FUNCTION__ ) 
  { vi.SetFieldBased(false); vi.Clear(IT_BFF); vi.Clear(IT_TFF); }
  inline bool __stdcall GetParity(int n) 
    { return false; }
