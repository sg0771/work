...
Levels::Levels( PClip _child, int in_min, double gamma, int in_max, int out_min, int out_max, bool coring, bool _dither,
  ...
  : GenericVideoFilter(_child,__FUNCTION__ ), map(0), mapchroma(0), dither(_dither)
{
  ...
  if (vi.IsYUV()) {
    ...
    for (int i=0; i<256*scale; ++i) {
      ...
      if (coring)
        map[i] = std::min(std::max(int(p*(219.0/255.0)+16.5),  16), 235);
      else
        map[i] = std::min(std::max(int(p+0.5), 0), 255);

      int q = ((bias + i - 128*scale) * (out_max-out_min)) / divisor + 128.5;

      if (coring)
        mapchroma[i] = std::min(std::max(q, 16), 240);
      else
        mapchroma[i] = std::min(std::max(q, 0), 255);
    }
  }
  else if (vi.IsRGB()) {
    for (int i=0; i<256*scale; ++i) {
...
RGBAdjust::RGBAdjust(PClip _child, double r,  double g,  double b,  double a,
  ...
  : GenericVideoFilter(_child,__FUNCTION__ ), analyze(_analyze), dither(_dither), mapR(0), mapG(0), mapB(0), mapA(0)
{
	try {	// HIDE DAMN SEH COMPILER BUG!!!
  if (!vi.IsRGB())
  ...
  if (dither) {
    ...
    for (int i=0; i<256*256; ++i) {
      mapR[i] = int(pow(std::min(std::max((rb*256 + i * r -127.5)/(255.0*256), 0.0), 1.0), rg) * 255.0 + 0.5);
      mapG[i] = int(pow(std::min(std::max((gb*256 + i * g -127.5)/(255.0*256), 0.0), 1.0), gg) * 255.0 + 0.5);
      mapB[i] = int(pow(std::min(std::max((bb*256 + i * b -127.5)/(255.0*256), 0.0), 1.0), bg) * 255.0 + 0.5);
      mapA[i] = int(pow(std::min(std::max((ab*256 + i * a -127.5)/(255.0*256), 0.0), 1.0), ag) * 255.0 + 0.5);
    }
  }
  else {
    mapR = new BYTE[256];
...
Tweak::Tweak( PClip _child, double _hue, double _sat, double _bright, double _cont, bool _coring, bool _sse,
  ...
  : GenericVideoFilter(_child,__FUNCTION__ ), coring(_coring), sse(_sse), dither(_dither), map(0), mapUV(0)
{
  try {  // HIDE DAMN SEH COMPILER BUG!!!
    if (vi.IsRGB())
    ...
    if (dither) {
      ...
      if (coring) {
        for (int i = 0; i < 256*256; i++) {
          ...
          map[i] = std::min(std::max(y, 16), 235);
        }
      }
      else {
        for (int i = 0; i < 256*256; i++) {
        ...
        for (int u = 0; u < 256; u++) {
          ...
          for (int v = 0; v < 256; v++) {
            ...
            if (allPixels || ProcessPixel(destv, destu, startHue, endHue, maxSat, minSat, p, iSat)) {
              ...
              mapUV[(u<<12)|(v<<4)|d]  = (unsigned short)(std::min(std::max(u,minUV),maxUV) | ((std::min(std::max(v,minUV),maxUV))<<8));
            }
          }
        }
      }
