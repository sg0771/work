...
extern const AVSFunction Fps_filters[] = {
  ...
  { "ChangeFPS1", "ci[]i[linear]b", ChangeFPS::Create },     // dst framerate, fetch all frames
...
void FloatToFPS(const char *name, float n, unsigned &num, unsigned &den, IScriptEnvironment* env)
{
  ...
  if (reduce_float(n, num, den))
    env->ThrowError("%s: FPS value is out of range.��n : %f ,num: %d, den: %d \n", name, n, num,den);
...
{
	if (0) { ; }
	else if (lstrcmpiA(p, "ntsc_film"        ) == 0) { num = 24000; den = 1001; }
	else if (lstrcmpiA(p, "ntsc_video"       ) == 0) { num = 30000; den = 1001; }
	else if (lstrcmpiA(p, "ntsc_double"      ) == 0) { num = 60000; den = 1001; }
	else if (lstrcmpiA(p, "ntsc_quad"        ) == 0) { num =120000; den = 1001; }

	else if (lstrcmpiA(p, "ntsc_round_film"  ) == 0) { num =  2997; den =  125; }
	else if (lstrcmpiA(p, "ntsc_round_video" ) == 0) { num =  2997; den =  100; }
	else if (lstrcmpiA(p, "ntsc_round_double") == 0) { num =  2997; den =   50; }
	else if (lstrcmpiA(p, "ntsc_round_quad"  ) == 0) { num =  2997; den =   25; }

	else if (lstrcmpiA(p, "film"             ) == 0) { num =    24; den =    1; }

	else if (lstrcmpiA(p, "pal_film"         ) == 0) { num =    25; den =    1; }
	else if (lstrcmpiA(p, "pal_video"        ) == 0) { num =    25; den =    1; }
	else if (lstrcmpiA(p, "pal_double"       ) == 0) { num =    50; den =    1; }
	else if (lstrcmpiA(p, "pal_quad"         ) == 0) { num =   100; den =    1; }

	else if (lstrcmpiA(p, "drop24"           ) == 0) { num = 24000; den = 1001; }
	else if (lstrcmpiA(p, "drop30"           ) == 0) { num = 30000; den = 1001; }
	else if (lstrcmpiA(p, "drop60"           ) == 0) { num = 60000; den = 1001; }
	else if (lstrcmpiA(p, "drop120"          ) == 0) { num =120000; den = 1001; }
/*
	else if (lstrcmpiA(p, "drop25"           ) == 0) { num = 25000; den = 1001; }
	else if (lstrcmpiA(p, "drop50"           ) == 0) { num = 50000; den = 1001; }
	else if (lstrcmpiA(p, "drop100"          ) == 0) { num =100000; den = 1001; }
	
	else if (lstrcmpiA(p, "nondrop24"        ) == 0) { num =    24; den =    1; }
	else if (lstrcmpiA(p, "nondrop25"        ) == 0) { num =    25; den =    1; }
	else if (lstrcmpiA(p, "nondrop30"        ) == 0) { num =    30; den =    1; }
	else if (lstrcmpiA(p, "nondrop50"        ) == 0) { num =    50; den =    1; }
	else if (lstrcmpiA(p, "nondrop60"        ) == 0) { num =    60; den =    1; }
	else if (lstrcmpiA(p, "nondrop100"       ) == 0) { num =   100; den =    1; }
	else if (lstrcmpiA(p, "nondrop120"       ) == 0) { num =   120; den =    1; }

	else if (lstrcmpiA(p, "23.976"           ) == 0) { num = 24000; den = 1001; }
	else if (lstrcmpiA(p, "23.976!"          ) == 0) { num =  2997; den =  125; }
	else if (lstrcmpiA(p, "24.0"             ) == 0) { num =    24; den =    1; }
	else if (lstrcmpiA(p, "25.0"             ) == 0) { num =    25; den =    1; }
	else if (lstrcmpiA(p, "29.97"            ) == 0) { num = 30000; den = 1001; }
	else if (lstrcmpiA(p, "29.97!"           ) == 0) { num =  2997; den =  100; }
	else if (lstrcmpiA(p, "30.0"             ) == 0) { num =    30; den =    1; }
	else if (lstrcmpiA(p, "59.94"            ) == 0) { num = 60000; den = 1001; }
	else if (lstrcmpiA(p, "59.94!"           ) == 0) { num =  2997; den =   50; }
	else if (lstrcmpiA(p, "60.0"             ) == 0) { num =    60; den =    1; }
	else if (lstrcmpiA(p, "100.0"            ) == 0) { num =   100; den =    1; }
	else if (lstrcmpiA(p, "119.88"           ) == 0) { num =120000; den = 1001; }
	else if (lstrcmpiA(p, "119.88!"          ) == 0) { num =  2997; den =   25; }
	else if (lstrcmpiA(p, "120.0"            ) == 0) { num =   120; den =    1; }
*/
	else {
...
ChangeFPS::ChangeFPS(PClip _child, unsigned new_numerator, unsigned new_denominator, bool _linear, IScriptEnvironment* env)
  : GenericVideoFilter(_child,__FUNCTION__ ), linear(_linear)
{
...
PVideoFrame __stdcall ChangeFPS::GetFrame(int n, IScriptEnvironment* env)
{
  ...
  lastframe = getframe;
  PVideoFrame pDst =  child->GetFrame(getframe , env );
  return pDst;
}
...
ConvertFPS::ConvertFPS( PClip _child, unsigned new_numerator, unsigned new_denominator, int _zone,
	...
	: GenericVideoFilter(_child,__FUNCTION__ ), zone(_zone), vbi(_vbi), lps(0)
{
