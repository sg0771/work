...
extern const AVSFunction Edit_filters[] = {  
	{ "AudioTrim1", "cff",          Trim::CreateA, (void*)Trim::Default}, // start time, end time
  ...
  { "AudioTrim", "cf[end]f",     Trim::CreateA, (void*)Trim::End},     // start time, end time
	...
	{ "Trim1", "cii[pad]b",         Trim::Create,  (void*)Trim::Default}, // first frame, last frame[, pad audio]
  ...
  { "AudioDub", "ccccc", AudioDub::Create, (void*)0},          // video src, audio src
 ...
 : GenericVideoFilter(_child,__FUNCTION__ ) 
{
  ...
  audio_offset = std::min(std::max(__int64(starttime*vi.audio_samples_per_second + 0.5), 0LL), (__int64)vi.num_audio_samples);
...
{
  return new Trim(args[1].AsFloat(), args[2].AsFloat(), args[0].AsClip(), (intptr_t)mode, env);
}
...
{
  return new Trim(args[1].AsInt(), args[2].AsInt(), args[3].AsBool(true), args[0].AsClip(), (intptr_t)mode, env);
}
...
FreezeFrame::FreezeFrame(int _first, int _last, int _source, PClip _child)
 : GenericVideoFilter(_child,__FUNCTION__ ), first(_first), last(_last), source(_source) {}
...
DeleteFrame::DeleteFrame(int _frame, PClip _child)
 : GenericVideoFilter(_child,__FUNCTION__ ), frame(_frame) { --vi.num_frames; }
...
Splice::Splice(PClip _child1, PClip _child2, bool realign_sound, bool _passCache, IScriptEnvironment* env)
 : GenericVideoFilter(_child1, __FUNCTION__), child2(_child2), passCache(_passCache)
{
...
intptr_t Splice::SetCacheHints(int cachehints,int frame_range)
{
...
Dissolve::Dissolve(PClip _child1, PClip _child2, int _overlap, double fps, IScriptEnvironment* env)
 : GenericVideoFilter(ConvertAudio::Create(_child1,SAMPLE_INT16|SAMPLE_FLOAT,SAMPLE_FLOAT),__FUNCTION__),
   child2(_child2),
   overlap(_overlap),
   audbuffer(0),
   audbufsize(0)
...
AudioDub::AudioDub(PClip child1, PClip child2, PClip child3, PClip child4, PClip child5, int mode, IScriptEnvironment* env) : IClip(__FUNCTION__){
...
{
  return new AudioDub(args[0].AsClip(), args[1].AsClip(),
      args[2].AsClip(), args[3].AsClip(), args[4].AsClip(),
...
Reverse::Reverse(PClip _child) : GenericVideoFilter(_child,__FUNCTION__ ) {}
...
void Reverse::GetAudio(void* buf, __int64 start, __int64 count, IScriptEnvironment* env) 
{
  ...
  int _xor = vi.BytesPerAudioSample() - 1;
  char* buf2 = (char*)buf;
  const int count_bytes = (int)vi.BytesFromAudioSamples(count);
  for (int i=0; i<(count_bytes>>1); ++i) {
    char temp = buf2[i]; buf2[i] = buf2[count_bytes-1-(i^ _xor)]; buf2[count_bytes-1-(i^ _xor)] = temp;
  }
}
...
Loop::Loop(PClip _child, int times, int _start, int _end, IScriptEnvironment* env)
 : GenericVideoFilter(_child,__FUNCTION__ ), start(_start), end(_end)
{
