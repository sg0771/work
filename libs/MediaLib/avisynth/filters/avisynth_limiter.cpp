...
AVSValue __cdecl Limiter::Create(AVSValue args, void* user_data, IScriptEnvironment* env)
{
	...
	if (option) {
	  if      (lstrcmpiA(option, "luma") == 0)
			show = show_luma;
	  else if (lstrcmpiA(option, "luma_grey") == 0)
			show = show_luma_grey;
	  else if (lstrcmpiA(option, "chroma") == 0)
			show = show_chroma;
	  else if (lstrcmpiA(option, "chroma_grey") == 0)
