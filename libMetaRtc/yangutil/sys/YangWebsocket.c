...
int yang_websocket_init(void* pcontext) {
	...
	yang_memset(&context->info, 0, sizeof(context->info));
	...
	yang_memset(&context->conn_info, 0, sizeof(struct lws_client_connect_info));
	 char addr_port[256] = { 0 };
	 yang_sprintf(addr_port, "%s:%u", context->urlData.server, context->urlData.port & 65535 );
	//初始化连接信息
...
void yang_websocket_start(void* pcontext) {
	if(pcontext==NULL) return;
	YangWebsocketContext* context=(YangWebsocketContext*)pcontext;
	if (yang_thread_create(&context->threadId, 0, yang_run_websocket_thread, pcontext)) {
...
int32_t yang_websocket_connectServer(void* pcontext,char* url) {
	if(pcontext==NULL) return 1;
	YangWebsocketContext* context=(YangWebsocketContext*)pcontext;
	if(yang_ws_url_parse(context->familyType,url,&context->urlData)) return 1;
...
int yang_websocket_sendData(void* pcontext,uint8_t *p, int nb) {
	...
	YangWebsocketContext *context = (YangWebsocketContext*) pcontext;
	char buf[2000];
	yang_memset(buf, 0, 2000);
	//LWS_SEND_BUFFER_PRE_PADDING
	yang_memcpy(buf + LWS_PRE, p, nb);
...
void yang_create_websocket(YangIpFamilyType familyType,YangWebsocket* web,YangWebsocketCallback* callback){
	if(web==NULL) return;
    web->context=yang_calloc(sizeof(YangWebsocketContext),1);
