...
void yang_ma_init(void* pcontext,int32_t sample,int32_t channel,int32_t echopath){
	...
	if(aec->buf==NULL) aec->buf=(uint8_t*)yang_malloc(aec->outRes.context->inBytes);
}
...
void yang_ma_echo_capture(void* pcontext,short *rec, short *out){
	...
	yang_memcpy((char*)out,aec->audioFrame.payload,aec->outRes.context->outBytes);
...
void yang_create_aec(YangAec* aec){
	if(aec==NULL) return;
	if(aec->session==NULL) {
		aec->session=(YangAecContext*)yang_calloc(sizeof(YangAecContext),1);
