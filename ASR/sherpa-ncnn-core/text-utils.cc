...
static std::vector<std::string> MergeCharactersIntoWords(
  ...
  while (i < n) {
    ...
    SHERPA_NCNN_LOGE("Ignore %s", w.c_str());
    ++i;
  }
...
std::vector<std::string> SplitUtf8(const std::string &text) {
  ...
  while (start < end) {
    ...
    if (num_bytes == 0) {
      ...
      SHERPA_NCNN_LOGE("Invalid byte at position: %d",
...
std::string RemoveInvalidUtf8Sequences(const std::string &text,
  ...
  while (i < n) {
    ...
    if (show_debug_msg) {
      SHERPA_NCNN_LOGE("Ignore invalid utf8 sequence at pos: %d, value: %02x",
                       i, p[i]);
    }
...
std::string Gb2312ToUtf8(const std::string &text) {
  ...
  SHERPA_NCNN_LOGE("num of wchars: %d", num_wchars);
  if (num_wchars == 0) {
    return {};
  }
