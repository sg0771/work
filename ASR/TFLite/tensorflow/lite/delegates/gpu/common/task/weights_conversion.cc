...
namespace tflite {
namespace gpu {
uint32_t GetTotalElementsCountForLayout(const WeightsDescription& weight_desc,
                                    const OHWI& shape) {
  if (weight_desc.layout == WeightsLayout::kOHWIOGroupI4O4 ||
    ...
    uint32_t i_aligned = AlignByN(shape.i, 4);
    uint32_t o_aligned = AlignByN(shape.o, 4 * weight_desc.output_group_size);
    ...
    uint32_t i_aligned = AlignByN(shape.i, 4);
    uint32_t o_aligned = AlignByN(shape.o, 4);
