    ...
    const union xnn_qs8_add_params params[XNN_MIN_ELEMENTS(1)]) XNN_DISABLE_TSAN
{
  ...
  for (; n >= 32 * sizeof(int8_t); n -= 32 * sizeof(int8_t)) {
    ...
    __m128i vaccOPQR = _mm_macc_epi32(vxOPQR, vx_multiplier, vzero_point_product);
    __m128i vaccSTUV = _mm_macc_epi32(vxSTUV, vx_multiplier, vzero_point_product);
    ...
    vaccOPQR = _mm_macc_epi32(vyOPQR, vy_multiplier, vaccOPQR);
    vaccSTUV = _mm_macc_epi32(vySTUV, vy_multiplier, vaccSTUV);
