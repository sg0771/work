...
void xnn_qs8_vadd_minmax_ukernel__sse41_mul32_ld32_x32(
    ...
    int8_t* output,
    const union xnn_qs8_add_params params[XNN_MIN_ELEMENTS(1)]) XNN_DISABLE_TSAN
{
  ...
  for (; n >= 32 * sizeof(int8_t); n -= 32 * sizeof(int8_t)) {
    ...
    const __m128i vyOPQR = _mm_cvtepi8_epi32(_mm_loadu_si32(input_y + 24));
    const __m128i vxSTUV = _mm_cvtepi8_epi32(_mm_loadu_si32(input_x + 28));
    const __m128i vySTUV = _mm_cvtepi8_epi32(_mm_loadu_si32(input_y + 28));
    input_x += 32;
    input_y += 32;
    ...
    vaccGHIJ = _mm_add_epi32(vaccGHIJ, _mm_mullo_epi32(vyGHIJ, vy_multiplier));
    vaccKLMN = _mm_add_epi32(vaccKLMN, _mm_mullo_epi32(vyKLMN, vy_multiplier));
    vaccOPQR = _mm_add_epi32(vaccOPQR, _mm_mullo_epi32(vyOPQR, vy_multiplier));
    vaccSTUV = _mm_add_epi32(vaccSTUV, _mm_mullo_epi32(vySTUV, vy_multiplier));
