...
void xnn_f32_dwconv2d_chw_ukernel_5x5p2__sse_2x4_acc3(
    size_t input_height,
    size_t input_width,
    const float* input,
    const float* weights,
    ...
    const union xnn_f32_chw_params params[XNN_MIN_ELEMENTS(1)])
{
  ...
  do {
    ...
    for (; w > 8 * sizeof(float); w -= 4 * sizeof(float)) {
      ...
      __m128 vo1p2 = _mm_mul_ps(vi3x4567, vk22);
      ...
      vo1p1 = _mm_add_ps(vo1p1, _mm_mul_ps(vi5x4567, vk42));
      ...
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi1x3456, vk01));
      ...
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi4x3456, vk31));
      ...
      vo1p1 = _mm_add_ps(vo1p1, _mm_mul_ps(vi1x2345, vk00));
      vo0p2 = _mm_add_ps(vo0p2, _mm_mul_ps(vi1x2345, vk10));
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi2x2345, vk10));
      ...
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi5x2345, vk40));
      ...
      vo1p1 = _mm_add_ps(vo1p1, _mm_mul_ps(vi2x5678, vk13));
      vo0p2 = _mm_add_ps(vo0p2, _mm_mul_ps(vi2x5678, vk23));
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi3x5678, vk23));
      ...
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi1x6789, vk04));
      ...
      vo1p1 = _mm_add_ps(vo1p1, _mm_mul_ps(vi3x6789, vk24));
      vo0p2 = _mm_add_ps(vo0p2, _mm_mul_ps(vi3x6789, vk34));
      vo1p2 = _mm_add_ps(vo1p2, _mm_mul_ps(vi4x6789, vk34));
