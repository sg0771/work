...
//Test  -i input.jpg -o output.png -n realesr-animevideov3 -s 2 -m D:\work\MyWork2\ASR\Real-ESRGAN-ncnn-vulkan\models
//OK
...
// ncnn
#include "../libncnn/cpu.h"
#include "../libncnn/gpu.h"
#include "../libncnn/platform.h"
...
#endif
{
    ...
    {
        swprintf(parampath, 256, L"%ws/%ws-x%ws.param", model.c_str(), modelname.c_str(), std::to_wstring(scale).c_str());
        swprintf(modelpath, 256, L"%ws/%ws-x%ws.bin", model.c_str(), modelname.c_str(), std::to_wstring(scale).c_str());
    }
    else{
        swprintf(parampath, 256, L"%ws/%ws.param", model.c_str(), modelname.c_str());
        swprintf(modelpath, 256, L"%ws/%ws.bin", model.c_str(), modelname.c_str());
    }
    ...
    path_t paramfullpath = parampath;
    path_t modelfullpath = modelpath;
