...
#include "../buffer/backend/VulkanBackend.hpp"

extern"C" MNN_PUBLIC   int MNNSupportVulkan();
//extern"C" MNN_PUBLIC   int WXGetSystemVersion();
//extern"C" MNN_PUBLIC   int WXIsVM();
namespace MNN {
...
bool MNN_SupportVulkan() {

    //if (WXIsVM() || WXGetSystemVersion() == 7) {
    //    return false;
    //}
...
void registerVULKANRuntimeCreator() {
    if (MNN_SupportVulkan()) {
...
//extern "C" int WXGetIniValue(const wchar_t* wszReg, const wchar_t* wszKey, int nDefaultValue);

extern "C" MNN_PUBLIC int MNNSupportVulkan() {
    //int nVulkan = WXGetIniValue(L"WXMedia", L"CheckVulkan", -1);
    //if (nVulkan <= 0) {
    //    return 0;
    //}
    return MNN::MNN_SupportVulkan() ? 1 : 0;
}